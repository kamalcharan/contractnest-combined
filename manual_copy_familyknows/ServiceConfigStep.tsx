...
const ServiceConfigStep: React.FC<ServiceConfigStepProps> = ({
  ...
  // ✅ PRODUCTION FIX: Add resource requirement with full resource details (name, description)
  const addResourceRequirement = useCallback((resourceId: string, resourceTypeId: string) => {
    // Find the full resource object to get name and description
    const resource = actualResources().find(r => r.id === resourceId);

    const newRequirement: ServiceResourceForm = {
      resource_id: resourceId,
      resource_type_id: resourceTypeId,
      resource_name: resource?.display_name || resource?.name || undefined,
      quantity: 1,
      is_required: true,
      description: resource?.description || undefined,
      // Store the full resource object for reference
      resource: resource ? {
        id: resource.id,
        name: resource.name,
  ...
  }, [resourceRequirements, onChange, actualResources]);
