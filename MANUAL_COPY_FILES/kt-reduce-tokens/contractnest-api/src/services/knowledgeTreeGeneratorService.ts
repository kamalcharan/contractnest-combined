...
service_activity: ${serviceActivity}`;
    ...
    console.log(`🔍 Raw start: ${rawText.substring(0, 200)}`);
    console.log(`🔍 Raw end: ${rawText.substring(Math.max(0, rawText.length - 200))}`);
    ...
    // Debug: log top-level keys and array lengths to confirm structure
    const topKeys = Object.keys(parsed);
    const arraySizes = topKeys
      .filter(k => Array.isArray(parsed[k]))
      .map(k => `${k}:${parsed[k].length}`)
      .join(', ');
    console.log(`🔍 Parsed keys: [${topKeys.join(', ')}]`);
    console.log(`🔍 Array sizes: ${arraySizes || 'none'}`);
