...
serve(async (req: Request) => {
  ...
  try {
    ...
    const isEventAssetsRequest = pathSegments.includes('event-assets');
    ...
    if (isStatusRequest || isInvoicesRequest || isNotifyRequest || isBuyerEquipmentRequest || isSellerEquipmentRequest || isEventAssetsRequest) {
    ...
    switch (method) {
      case 'GET':
        ...
        } else if (isEventAssetsRequest && contractId) {
          response = await handleGetEventAssets(supabase, contractId, tenantId);
        } else if (contractId) {
...
// ==========================================================
// HANDLER: GET per-asset progress rows (Sprint 3)
// One row per (service event x covered asset) — see
// generate_contract_event_assets(). Read-only, grouped by event_id so the
// UI can show "n/m assets proven" per event without a client-side join.
// ==========================================================
async function handleGetEventAssets(
  supabase: any,
  contractId: string,
  tenantId: string
): Promise<Response> {
  ...
  if (error) {
    console.error('[handleGetEventAssets] query error:', error);
  ...
  return jsonResponse({ success: true, data: byEvent });
}
