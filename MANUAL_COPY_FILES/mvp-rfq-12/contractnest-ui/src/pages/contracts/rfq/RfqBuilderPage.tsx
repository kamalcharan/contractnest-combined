...
interface FlyByLine {
  id: string;
  type: FlyByBlockType;      // service | spare | text | document
  name: string;
  cycle?: string;           // service — BILLING cycle (prepaid … custom)
  serviceCycleDays?: number; // service — SERVICE cycle: performed every N days (frequency)
...
const FLYBY_ADD_ORDER: FlyByBlockType[] = ['service', 'spare', 'text', 'document'];
...
const RfqBuilderPage: React.FC = () => {
  ...
  const addFlyby = (type: FlyByBlockType) =>
    setServices((prev) => [...prev, {
      ...
      serviceCycleDays: type === 'service' ? 30 : undefined,
  ...
  const submit = useCallback(async () => {
    ...
    const blocks = services.map((s, i) => ({
      ...
      category_name: FLYBY_TYPE_CONFIG[s.type].label,
      unit_price: 0,
      ...
      custom_fields: {
        config: {
          ...
          ...(s.type === 'service' && s.serviceCycleDays ? { serviceCycles: { days: s.serviceCycleDays, enabled: true } } : {}),
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '28px 20px 0' }}>
        ...
        {stepId === 'services' && (
          <>
            ...
            {services.map((s) => {
              const cfg = FLYBY_TYPE_CONFIG[s.type];
              const Icon = cfg.icon;
              return (
                <div key={s.id} style={{ ...card, marginBottom: 10, borderColor: `${cfg.color}44` }}>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    ...
                    <input value={s.name} onChange={(e) => updateFlyby(s.id, { name: e.target.value })}
                      placeholder={s.type === 'service' ? 'e.g. Preventive maintenance visit' : s.type === 'spare' ? 'e.g. HVAC filter kit' : s.type === 'text' ? 'e.g. Scope note / terms' : 'e.g. Compliance certificate'}
              ...
              {services.map((s) => (
                <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: `1px solid ${line}`, fontSize: 13.5 }}>
                  <span style={{ color: ink }}>
                    <span style={{ color: FLYBY_TYPE_CONFIG[s.type].color, fontWeight: 700, fontSize: 11, marginRight: 6, textTransform: 'uppercase' }}>{FLYBY_TYPE_CONFIG[s.type].label}</span>
                    {s.name || `Unnamed ${FLYBY_TYPE_CONFIG[s.type].label}`}
                  </span>
