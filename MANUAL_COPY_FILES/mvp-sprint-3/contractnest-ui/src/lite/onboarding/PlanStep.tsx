...
export const PlanStep: React.FC = () => {
  ...
  const leaveOnboarding = (planId: string | null) => {
    ...
    completeVaniStep('done', {
      ...
      first_contract_id: routeState.contractId || null,
    });
      ...
      {routeState.contractNumber && (
        <div className="cnx-aside" style={{ boxShadow: 'none' }}>
          ...
          <span className="cnx-asidetitle">
            Contract {routeState.contractNumber} is ready
          </span>
          <p className="cnx-asidebody">
            It&apos;s in test mode, so nothing has reached a real customer. Open it from
            Contracts when you want to send it for real.
          </p>
        </div>
      )}
