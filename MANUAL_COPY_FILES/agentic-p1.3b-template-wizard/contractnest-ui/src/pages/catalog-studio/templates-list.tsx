...
// =====================================================
// MY TEMPLATES (real tenant templates from t_cat_templates)
// =====================================================
...
const formatTemplateValue = (total: number | null | undefined, currency?: string): string | null => {
  ...
  const symbol = (currency && CURRENCY_CONFIG[currency as CurrencyCode]?.symbol) || currency || '₹';
  return `${symbol}${Number(total).toLocaleString('en-IN', { maximumFractionDigits: 0 })}`;
};
...
const TemplatesList: React.FC = () => {
  ...
  // State — default to My Templates (real tenant data)
  ...
  // ===== MY TEMPLATES: real tenant data (t_cat_templates) =====
  const {
    data: myTemplatesResponse,
  ...
  const myTemplates: CatTemplate[] = myTemplatesResponse?.data?.templates || [];

  const filteredMyTemplates = useMemo(() => {
    if (!searchQuery) return myTemplates;
  ...
  }, [myTemplates, searchQuery]);

  // Delete confirmation target (two-step delete)
  const [deleteTarget, setDeleteTarget] = useState<CatTemplate | null>(null);

  // Template wizard (same ContractWizard as contract creation, in template mode)
  ...
  const handleCopyMyTemplate = useCallback(
    (t: CatTemplate) => {
      if (copyTemplateMutation.isPending) return;
      copyTemplateMutation.mutate({ id: t.id, data: { name: `${t.name} (Copy)` } });
    },
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: colors.utility.primaryBackground }}>
      ...
      {/* ===== MY TEMPLATES TAB (real tenant templates) ===== */}
      {activeTab === 'my' && (
        <div className="px-6 py-6">
          {/* Loading */}
          {loadingMyTemplates && (
            <div className="flex flex-col items-center justify-center py-20">
              <Loader2 className="w-10 h-10 animate-spin mb-4" style={{ color: tenantPrimary }} />
              <p style={{ color: colors.utility.secondaryText }}>Loading your templates…</p>
            </div>
          )}

          {/* Error */}
          {!loadingMyTemplates && myTemplatesError && (
            <div
            ...
            >
              ...
              <p className="text-sm mb-4" style={{ color: colors.utility.secondaryText }}>
                {(myTemplatesError as Error)?.message || 'Something went wrong.'}
              </p>
              <button
                onClick={() => refetchMyTemplates()}
                className="px-4 py-2 rounded-lg text-sm font-medium"
                style={{ backgroundColor: tenantPrimary, color: themeColors.textOnPrimary }}
              >
                Try again
          ...
          {/* Empty state */}
          {!loadingMyTemplates && !myTemplatesError && myTemplates.length === 0 && (
            <div className="py-12">
              <div className="text-center max-w-md mx-auto">
                ...
                <p className="mb-6" style={{ color: colors.utility.secondaryText }}>
                  Build a reusable template from your catalog blocks. Signed-off templates can be
                  used to create contracts in seconds.
                </p>
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <span
                    ...
                    style={{ backgroundColor: themeColors.accentLight, color: tenantPrimary }}
                  >
                    {filteredMyTemplates.length} of {myTemplates.length}
                  </span>
                </div>
                <button
                  onClick={handleCreateNew}
                  ...
                  {filteredMyTemplates.map((template) => {
                    ...
                    return (
                      <div
                        ...
                        style={{
                          backgroundColor: colors.utility.secondaryBackground,
                          borderColor: signedOff
                            ? withOpacity(tenantPrimary, 0.4)
                        ...
                        >
                          <div className="flex items-start justify-between mb-3">
                            ...
                            {signedOff ? (
                              <span
                                className="flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium"
                                style={{ backgroundColor: withOpacity('#10B981', 0.15), color: '#10B981' }}
                              >
                          ...
                          <h3
                            className="font-bold text-lg mb-1.5 line-clamp-1"
                            style={{ color: colors.utility.primaryText }}
                          >
                          ...
                          <button
                            ...
                            style={
                              signedOff
                                ...
                                : { backgroundColor: withOpacity('#10B981', 0.15), color: '#10B981' }
                            }
                          >
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm mb-1" style={{ color: colors.utility.secondaryText }}>
                    "{deleteTarget.display_name || deleteTarget.name}" will be removed from your
                    templates. Contracts already created from it are not affected.
                  </p>
                </div>
              </div>
              <div className="flex items-center justify-end gap-3 mt-6">
                ...
                <button
                  ...
                  disabled={deleteTemplateMutation.isPending}
                  className="px-5 py-2.5 rounded-xl font-medium flex items-center gap-2 transition-colors disabled:opacity-70"
      ...
      <ContractWizard
        ...
        onTemplateSaved={() => refetchMyTemplates()}
      />
    </div>
  );
};
