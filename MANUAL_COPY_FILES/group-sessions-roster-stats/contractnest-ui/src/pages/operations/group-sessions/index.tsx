            ...
            <Card style={cardStyle}>
              ...
              {rosterQuery.isLoading ? (
                ...
                <div key={m.contact_id} className="flex items-center gap-3 px-4 py-2.5 border-b cursor-pointer hover:bg-black/5" style={{ borderColor: line }} onClick={() => openMember(m)}>
                  ...
                  {overCap && (
                    <span className="text-[9.5px] font-bold rounded px-1.5 py-0.5" style={{ backgroundColor: colors.semantic.error + '1e', color: colors.semantic.error }}>
                      {m.over_no_show_cap ? `⚠ ${m.missed}/${m.max_no_shows} no-shows` : `⚠ ${m.substituted}/${m.max_substitutes} subs`}
                    </span>
                  )}
                  <span className="text-[11.5px] tabular-nums" style={sub}>{m.attended}/{m.overall}</span>
                  <ChevronRight size={15} style={sub} className="flex-none" />
                </div>
              );})}
            </Card>
        ...
        {memberQuery.isLoading ? (
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-3">
              ...
              <Kpi icon={<CalendarClock size={12} />} label="Attended" value={`${d?.attended ?? 0} / ${d?.overall ?? 0}`} sub="sessions" />
              <Kpi icon={<Clock size={12} />} label="Last seen" value={d?.last_seen ? fmtShort(d.last_seen) : '—'} />
              <Kpi icon={<AlertTriangle size={12} />} label="Missed" value={d?.missed ?? 0} sub="sessions" tone={d?.over_no_show_cap ? 'warn' : undefined} />
              <Kpi icon={<UserCog size={12} />} label="Substituted" value={d?.substituted ?? 0} sub="sessions" tone={d?.over_substitute_cap ? 'warn' : undefined} />
