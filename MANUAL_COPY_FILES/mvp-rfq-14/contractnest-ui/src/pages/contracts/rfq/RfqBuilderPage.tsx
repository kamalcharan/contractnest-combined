  ...
  return (
    <div style={{ minHeight: '100vh', background: bg, padding: '0 0 120px' }}>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '28px 20px 0' }}>
        ...
        {/* ── STEP: scope — the REAL ServiceBlocksStep in RFQ mode (no library, no pricing) ── */}
        {stepId === 'services' && (
          <>
            ...
            <ServiceBlocksStep
              selectedBlocks={services}
              currency={currency}
              onBlocksChange={setServices}
              contractName={name || 'Request'}
              contractStatus="draft"
              contractDuration={durationUnit === 'years' ? durationValue * 12 : durationUnit === 'days' ? Math.max(1, Math.round(durationValue / 30)) : durationValue}
              contractStartDate={startDate ? new Date(`${startDate}T00:00:00`) : null}
              rfqMode
              coverageTypes={[]}
            />
          </>
