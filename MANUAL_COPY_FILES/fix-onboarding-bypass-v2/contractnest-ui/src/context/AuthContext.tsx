...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const checkOnboardingStatus = async (tenantOverride?: Tenant): Promise<boolean> => {
    try {
      ...
      // If no token or tenant, allow through (will be caught by other auth checks)
      if (!token || !tenant) {
        console.log('[Onboarding] No token or tenant, skipping onboarding check');
        return true;
      }

      // Check if we already have the status in storage (cached)
      const storedStatus = sessionStorage.getItem(STORAGE_KEYS.ONBOARDING_COMPLETE);
      if (storedStatus === 'true') {
        console.log('[Onboarding] Found cached status: complete');
      ...
      if (response.data) {
        // API returns: { needs_onboarding: boolean, data: { is_complete: boolean, ... } }
        const needsOnboarding = response.data.needs_onboarding;
        const isComplete = response.data.data?.is_complete || false;

        console.log('[Onboarding] needs_onboarding:', needsOnboarding, 'is_complete:', isComplete);

        // If onboarding is complete, cache it and return true
        if (isComplete || needsOnboarding === false) {
        ...
        console.log('[Onboarding] Onboarding NOT complete');
        setHasCompletedOnboarding(false);
        return false;
      }

      // If response format is unexpected, allow through (don't block existing users)
      console.log('[Onboarding] Unexpected response format, allowing through');
      return true;
    } catch (error: any) {
      console.error('[Onboarding] Error checking status:', error?.message || error);
      // On error, allow through to not block existing users
      // New users will be caught by the register flow which always goes to onboarding
      return true;
    }
  };
  ...
  // Function to mark onboarding as complete (called when onboarding finishes)
  const markOnboardingComplete = () => {
    console.log('[Onboarding] Marking onboarding as complete in AuthContext');
    setHasCompletedOnboarding(true);
    sessionStorage.setItem(STORAGE_KEYS.ONBOARDING_COMPLETE, 'true');
  };
  ...
  const login = async (email: string, password: string, remember: boolean = false) => {
    ...
    try {
      ...
      if (!data.tenants || data.tenants.length === 0) {
        ...
        console.log('[Login] Checking onboarding for tenant:', tenant.id, 'is_owner:', tenant.is_owner);
        ...
        if (!isOnboardingComplete) {
          ...
          if (tenant.is_owner) {
            console.log('[Login] Owner needs onboarding - redirecting to /onboarding');
            navigate('/onboarding');
          } else {
            console.log('[Login] Non-owner waiting for onboarding - redirecting to /onboarding-pending');
  ...
  const register = async (userData: RegisterFormData) => {
    ...
    try {
      ...
      if (!data.tenant) {
        ...
        setHasCompletedOnboarding(false);
        // Clear any cached onboarding status from previous sessions
