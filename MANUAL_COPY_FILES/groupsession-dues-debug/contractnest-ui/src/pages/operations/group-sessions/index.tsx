...
import { useAuth } from '@/context/AuthContext'; // TEMP DEBUG — remove with the debug block below
...
const GroupSessionsPage: React.FC = () => {
  ...
  const { currentTenant } = useAuth(); // TEMP DEBUG — remove with the debug block below
      ...
      <>
        ...
        {duesQuery.isLoading ? (
          ...
          <div className="flex flex-col items-center py-12 gap-2">
            ...
            {/* TEMP DEBUG — remove once the Dues load failure is diagnosed. */}
            <pre className="text-[10px] text-left whitespace-pre-wrap max-w-lg p-3 rounded-lg border"
              style={{ borderColor: colors.semantic.error + '40', color: colors.semantic.error, backgroundColor: colors.semantic.error + '0d' }}>
{JSON.stringify({
  error_message: (duesQuery.error as any)?.message ?? null,
  error_status: (duesQuery.error as any)?.response?.status ?? null,
  error_body: (duesQuery.error as any)?.response?.data ?? null,
  activeDuesBlock,
  duesBlock,
  currentTenantId: currentTenant?.id ?? null,
  sessions_length: sessions.length,
  sessions_first_block_id: sessions[0]?.block_id ?? null,
  api_base_url: (import.meta as any).env?.VITE_API_URL ?? '(unset — falls back to prod default)',
}, null, 2)}
            </pre>
