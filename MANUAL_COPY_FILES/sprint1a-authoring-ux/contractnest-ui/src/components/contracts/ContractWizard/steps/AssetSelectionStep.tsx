...
const AssetSelectionStep: React.FC<AssetSelectionStepProps> = ({
  ...
  const [attachmentMode, setAttachmentMode] = useState<AttachmentMode>(
    ...
    allowBuyerToAdd ? 'buyer' : (equipmentDetails.filter((d) => !isPlaceholderDetail(d)).length > 0 ? 'existing' : null)
  );
  ...
  React.useEffect(() => {
    ...
    for (const ct of coverageTypes) {
      ...
      // Reuse existing placeholder rows for this type (stable ids), trim/extend to `needed`
    ...
    // Only write when the placeholder set actually changed (loop guard)
  ...
  const stepStatus: 'empty' | 'coverage_only' | 'complete' = useMemo(() => {
    ...
    if (attachmentMode === 'existing' && realDetails.length > 0) return 'complete';
    if (attachmentMode === 'buyer' || attachmentMode === 'later') return 'complete';
    return 'coverage_only';
  }, [hasCoverage, attachmentMode, realDetails.length]);
      ...
      <div className="flex-1 min-w-0 overflow-y-auto">
        <div className="px-5 pt-5 pb-4">
          ...
          <div className="mb-1">
            ...
            <p className="text-[11px] mb-1" style={{ color: colors.utility.secondaryText, marginLeft: 30 }}>
              Select {config.label.toLowerCase()} types this contract will service, and how many units of each
            </p>
            <p
              className="text-[10px] mb-3 inline-flex items-center gap-1 px-2 py-0.5 rounded-full"
              style={{ marginLeft: 30, backgroundColor: `${colors.brand.primary}10`, color: colors.brand.primary }}
            >
              <ArrowRight size={10} />
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {resourcesInCat.map((resource) => {
                            ...
                            return (
                              <span
                                key={resource.id}
                                className="inline-flex items-center rounded-lg text-[11px] font-semibold transition-all border overflow-hidden"
                                style={{
                              ...
                              >
                                ...
                                {isSelected && (
                                  /* Declared fleet size — drives placeholders + per-asset fan-out */
                                  <span
                                    className="inline-flex items-center gap-0.5 pl-1 pr-1.5 py-1 border-l"
                                    style={{ borderColor: iconColor + '30' }}
                                    title="How many units of this type the contract covers"
                                  >
                                    <button
                                      type="button"
                                      aria-label="Fewer units"
                                      className="w-4 h-4 rounded flex items-center justify-center font-bold hover:opacity-70"
                                      style={{ backgroundColor: iconColor + '15' }}
                                      onClick={(e) => { e.stopPropagation(); setCoverageUnitCount(resource.id, units - 1); }}
                                    >
                                      −
                                    </button>
                                    <span className="min-w-[18px] text-center tabular-nums text-[10px] font-bold">
                                      ×{units}
                                    </span>
                                    <button
                                      type="button"
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2.5 mb-4">
                  <OptionCard
                    ...
                    badge={realDetails.length > 0 ? `${realDetails.length}` : undefined}
                  />
                  <OptionCard
                  ...
                  <OptionCard
                    ...
                    label={`${buyerName || 'Client'} lists them after signing`}
                    hint="You can still send the contract today"
                    isSelected={attachmentMode === 'buyer'}
                    onClick={() => handleAttachmentModeChange(attachmentMode === 'buyer' ? null : 'buyer')}
                    accentColor="#0EA5E9"
                    colors={colors}
                    isDarkMode={isDarkMode}
                    badge={placeholderDetails.length > 0 ? `${placeholderDetails.length} to attach` : undefined}
                  />
                  <OptionCard
                    icon={<Clock size={16} />}
                    label="Attach Later"
                      ...
                      <div className="flex-1 overflow-y-auto px-3 py-2.5">
                        ...
                        <div className="flex items-center gap-2 mb-2.5">
                          ...
                          <span className="text-[10px] ml-auto" style={{ color: colors.utility.secondaryText }}>
                            <strong>{realDetails.length}</strong> / <strong>{displayAssets.length}</strong>
                            {placeholderDetails.length > 0 && (
                              <span style={{ color: '#F59E0B' }}> · {placeholderDetails.length} to attach</span>
                            )}
                          </span>
                        </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-1 p-2.5">
                        {items.map((ct) => {
                          const units = Math.max(1, ct.unit_count ?? 1);
                          const attached = realDetails.filter((d) => d.category_id === ct.resource_id).length;
                          return (
                            <span
                              key={ct.id}
                              ...
                              title={`${attached} attached · ${Math.max(0, units - attached)} to attach`}
                            >
                              {ct.resource_name} <span className="tabular-nums">×{units}</span>
                            </span>
                          );
                        })}
                      </div>
              ...
              {attachmentMode === 'existing' && (
                ...
                >
                  ...
                  <div className="p-2.5">
                    ...
                    {placeholderDetails.length > 0 && (
                      <div
                        className="flex items-center gap-1.5 text-[10px] mt-1.5 py-1.5 px-2 rounded-lg border border-dashed"
                        style={{ borderColor: '#F59E0B50', color: '#F59E0B', backgroundColor: '#F59E0B0A' }}
                        title="Declared units not yet attached — these become serviceable placeholders; visits can be assigned but not closed until the real asset is attached"
                      >
                        <Clock size={10} className="flex-shrink-0" />
                        <span className="font-medium">{placeholderDetails.length} unit{placeholderDetails.length > 1 ? 's' : ''} to attach</span>
                      </div>
                    )}
                  </div>
                </div>
