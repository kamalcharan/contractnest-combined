          ...
          return (
            ...
            >
              ...
              {(cycleMismatch?.offenders.has(block.id) ||
                (typeof block.listPrice === 'number' && block.listPrice > 0 && block.price !== block.listPrice)) && (
                <div className="flex flex-wrap gap-1.5 px-1 pt-1">
                  ...
                  {typeof block.listPrice === 'number' && block.listPrice > 0 && block.price < block.listPrice && (
                    ...
                    >
                      list <s>{formatCurrency(block.listPrice, block.currency)}</s> → {formatCurrency(block.price, block.currency)}
                      {' '}(−{Math.round((1 - block.price / block.listPrice) * 1000) / 10}%)
                    </span>
                  )}
                  {typeof block.listPrice === 'number' && block.listPrice > 0 && block.price > block.listPrice && (
                    <span
