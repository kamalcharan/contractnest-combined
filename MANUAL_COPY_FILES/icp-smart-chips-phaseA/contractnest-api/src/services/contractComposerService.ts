...
class ContractComposerService {
  ...
  async fetchTenantIcp(ctx: ComposerCallContext): Promise<{
    ...
    const empty = { vocabulary: [] as string[], clusters: [] as Array<{ primary_term: string; category: string }>, persona: '', resources: [] as Array<{ name: string; type: string }> };
    const supabase = this.supabaseServiceRead();
    if (!supabase) return empty;
    try {
      const [profileRes, clusterRes, resourceRes] = await Promise.all([
        supabase
  ...
  async buildSuggestions(
    ...
    try {
      ...
      // so the chip that reads most "like their business" surfaces first.
      ...
      // 1. Published templates → guaranteed fast-path phrasing, now RANKED by
      // vocabulary overlap. Billing suffix is derived from the template's actual
      // blocks — never invented.
      const rankedTemplates = [...templates].sort((a, b) =>
      ...
      for (const t of rankedTemplates.slice(0, 2)) {
      ...
      // 2. The tenant's REAL resources (materialized working set from onboarding),
      // in their OWN display names, phrased by type and RANKED by vocabulary
      // overlap. Equipment/consumable→AMC, facility(asset)→FMC, service→package.
      const equipNom = nomFor('equipment_maintenance');
      const facNom = nomFor('facility_property');
      const rankedResources = [...icp.resources].sort((a, b) => icpScore(b.name) - icpScore(a.name));
      for (const r of rankedResources) {
        if (suggestions.length >= (mode === 'contract' ? 3 : 4)) break;
        const type = r.type;
        if (type === 'asset') {
          const nom = facNom || 'FMC';
          if (mode === 'contract') push(`${r.name} ${nom} for 1 year, billed monthly`);
          else push(`1 year ${r.name} ${nom} with monthly service visits and sign-off acceptance`);
        } else if (type === 'equipment' || type === 'consumable') {
          const nom = equipNom || 'AMC';
          if (mode === 'contract') push(`${r.name} ${nom} for 1 year, billed quarterly`);
          else push(`1 year ${r.name} ${nom} with quarterly PM visits and sign-off acceptance`);
        } else if (type === 'service') {
          if (mode === 'contract') push(`1 year ${r.name} service package${recentClient ? ` for ${recentClient}` : ''}, billed quarterly`);
          else push(`1 year ${r.name} service package with quarterly reviews and sign-off acceptance`);
        }
      }

      // 3. Vocabulary chip — for service-led tenants with no equipment/facility
      // to sell, phrase a package from their STRONGEST semantic cluster (a real
      // ICP term, never invented). Falls back to the service nomenclature if no
      // clusters exist. This is what makes a chip read as "made for me".
      const hasHardResource = icp.resources.some((r) => ['equipment', 'consumable', 'asset'].includes(r.type));
      const serviceLed = !hasHardResource && icp.persona !== 'buyer';
      if (suggestions.length < 4 && serviceLed) {
        const cycle = rankedTemplates[0]?.settings?.wizard_state?.selectedBlocks?.[0]?.cycle;
        const billing = ['monthly', 'fortnightly', 'quarterly'].includes(cycle) ? cycle : 'quarterly';
        const serviceNoun = icp.clusters[0]?.primary_term
          ? titleCase(icp.clusters[0].primary_term)
          : nomFor('service_delivery');
        if (serviceNoun) {
          if (mode === 'contract') {
            push(`1 year ${serviceNoun} service package${recentClient ? ` for ${recentClient}` : ''}, billed ${billing}`);
          } else {
            push(`1 year ${serviceNoun} service package with ${billing} billing and sign-off acceptance`);
          }
        }
      }
    } catch (e: any) {
