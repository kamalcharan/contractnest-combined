...
const SmartProfilePage: React.FC = () => {
  ...
  const handleSaveClusters = async () => {
    ...
    try {
      ...
      // The view reads clusters from the PROFILE query cache, so refresh it
      // (not just the clusters query) — otherwise navigating back shows the
      // stale empty list from before these clusters were generated.
      queryClient.invalidateQueries({
        queryKey: smartProfileQueryKeys.profile(currentTenant.id)
      });
