...
const ReviewSendStep: React.FC<ReviewSendStepProps> = ({
  ...
  const handleDownloadPDF = useCallback(async () => {
    if (!paperRef.current || isExportingPDF) return;

    setIsExportingPDF(true);
    try {
      const safeName = (contractName || 'contract').toLowerCase().replace(/[^a-z0-9]+/g, '-');
      await generatePDF(paperRef.current, {
        filename: `${safeName}-${viewMode}.pdf`,
        margin: 8,
        quality: 0.92,
        scale: 2,
      });
      vaniToast.success('PDF downloaded successfully');
    } catch (error) {
      console.error('PDF export failed:', error);
      vaniToast.error('Failed to generate PDF. Please try again.');
    } finally {
      setIsExportingPDF(false);
    }
  }, [contractName, viewMode, isExportingPDF]);
        ...
        <div className="max-w-[850px] mx-auto flex items-center justify-between px-2 pt-6 pb-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-semibold transition-all hover:opacity-80 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
              style={{
