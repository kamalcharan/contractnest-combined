...
interface EquipmentFormDialogProps {
  ...
  categories?: Array<{ id: string; name: string; resource_type_id?: string }>;
...
const EquipmentFormDialog: React.FC<EquipmentFormDialogProps> = ({
  ...
  // Helper: resolve master resource_type_id from a category UUID
  const resolveMasterType = (categoryId: string | undefined): string => {
    if (!categoryId) return '';
    const cat = categories.find((c) => c.id === categoryId);
    return cat?.resource_type_id || '';
  };
  ...
  useEffect(() => {
    ...
    if (mode === 'edit' && asset) {
      ...
      // For create: resource_type_id = master type ('equipment'/'asset'), asset_type_id = tenant resource UUID
      const masterType = resolveMasterType(resourceTypeId);
      setFormData({
        ...DEFAULT_FORM_DATA,
        resource_type_id: masterType || 'equipment',
        asset_type_id: resourceTypeId || undefined,
      });
      setSpecRows([]);
    }
    setErrors({});
  ...
  // When user picks a category from the dropdown, set both fields
  const handleCategoryChange = (categoryId: string) => {
    const cat = categories.find((c) => c.id === categoryId);
    setFormData((prev) => ({
      ...prev,
      asset_type_id: categoryId,
      resource_type_id: cat?.resource_type_id || prev.resource_type_id || 'equipment',
    }));
  ...
  const validate = (): boolean => {
    ...
    if (!formData.asset_type_id && !formData.resource_type_id) errs.resource_type_id = 'Equipment type is required';
