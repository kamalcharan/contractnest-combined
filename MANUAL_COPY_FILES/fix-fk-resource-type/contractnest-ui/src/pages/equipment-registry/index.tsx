...
const EquipmentPage: React.FC = () => {
  ...
  const filters: AssetRegistryFilters = useMemo(() => ({
    asset_type_id: selectedCategoryId || undefined,
  ...
  const handleCreateSubmit = async (data: AssetFormData) => {
    try {
      // Form already sets resource_type_id = master type ('equipment'/'asset')
      // and asset_type_id = tenant resource UUID for filtering
  ...
  const handleEditSubmit = async (data: AssetFormData) => {
    ...
    try {
      // Form already sets resource_type_id = master type and asset_type_id = UUID
    ...
    for (const name of names) {
      try {
        await createMutation.mutateAsync({
          name,
          resource_type_id: selectedCat?.resource_type_id || 'equipment',
          asset_type_id: selectedCategoryId,
          status: 'active',
          condition: 'good',
          criticality: 'medium',
          specifications: {},
