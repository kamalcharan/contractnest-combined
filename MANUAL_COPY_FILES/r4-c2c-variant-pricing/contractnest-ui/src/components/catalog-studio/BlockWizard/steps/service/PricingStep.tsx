...
const PricingStep: React.FC<PricingStepProps> = ({ formData, onChange }) => {
  ...
  // Variant pricing — per-variant price lines (from KT)
  ...
  const [variantPrices, setVariantPrices] = useState<Record<string, number>>(() => {
    ...
    selectedVariants.forEach(v => { init[v.variant_id] = 0; });
    return init;
  });
            ...
            <div className="p-4 rounded-xl border" style={cardStyle}>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs mt-0.5" style={{ color: colors.utility.secondaryText }}>
                    Set price per variant. Default: {currencyOptions.find(c => c.code === (pricingRecords[0]?.currency || 'INR'))?.symbol || '₹'}0 (edit later in Catalog Studio)
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3 px-3 py-1.5">
                  ...
                  <span className="w-32 text-xs font-medium text-right" style={{ color: colors.utility.secondaryText }}>
                    Price ({pricingRecords[0]?.currency || 'INR'})
                  </span>
                </div>
                  ...
                  >
                    ...
                    <div className="w-32">
                      <input
                        type="number"
                        min={0}
                        value={variantPrices[variant.variant_id] || 0}
                        onChange={(e) => handleVariantPriceChange(variant.variant_id, parseFloat(e.target.value) || 0)}
                        className="w-full text-right text-sm px-3 py-2 rounded-lg border"
                        style={inputStyle}
                        placeholder="0"
                      />
                    </div>
