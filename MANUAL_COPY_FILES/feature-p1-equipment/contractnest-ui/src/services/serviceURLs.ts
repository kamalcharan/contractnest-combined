...
// Asset Registry filter types (P1 — Equipment & Entity)
...
export const API_ENDPOINTS = {
  ...
  // =================================================================
  // ASSET REGISTRY ENDPOINTS (P1 — Equipment & Entity)
  // =================================================================
  ASSET_REGISTRY: {
    // Main CRUD operations
    LIST: '/api/asset-registry',
    CREATE: '/api/asset-registry',
    GET: (id: string) => `/api/asset-registry?id=${id}`,
    UPDATE: (id: string) => `/api/asset-registry?id=${id}`,
    DELETE: (id: string) => `/api/asset-registry?id=${id}`,

    // Hierarchy
    CHILDREN: (parentAssetId: string) => `/api/asset-registry/children?parent_asset_id=${parentAssetId}`,

    // Contract-asset linking
    CONTRACT_ASSETS: {
      LIST: (contractId: string) => `/api/asset-registry/contract-assets?contract_id=${contractId}`,
      LINK: '/api/asset-registry/contract-assets',
      UNLINK: (contractId: string, assetId: string) =>
        `/api/asset-registry/contract-assets?contract_id=${contractId}&asset_id=${assetId}`,
    },

    // Helper: build list URL with filters
    LIST_WITH_FILTERS: (filters: AssetRegistryFilters = {}) => {
      const params = new URLSearchParams();
      ...
      if (filters.search) params.append('search', filters.search);
      if (filters.is_live !== undefined) params.append('is_live', filters.is_live.toString());
      if (filters.limit !== undefined) params.append('limit', filters.limit.toString());
      if (filters.offset !== undefined) params.append('offset', filters.offset.toString());

      const queryString = params.toString();
      return queryString ? `/api/asset-registry?${queryString}` : '/api/asset-registry';
    },
  },
};
...
export type AssetRegistryEndpoints = typeof API_ENDPOINTS.ASSET_REGISTRY;
...
/**
 * Build asset registry list URL with filters
 */
export const buildAssetRegistryListURL = (filters: AssetRegistryFilters = {}): string => {
  return API_ENDPOINTS.ASSET_REGISTRY.LIST_WITH_FILTERS(filters);
};
