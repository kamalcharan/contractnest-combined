...
--
-- Creates:
--   1. form_templates          — Global form definitions (support team)
--   2. tenant_form_selections  — Tenant selects which forms to use
--   3. form_template_mappings  — Tenant maps forms to contracts/events
--   4. form_submissions        — Completed form responses
--   5. form_attachments        — File uploads linked to submissions
--   6. Storage bucket          — form-attachments (private)
-- ============================================================================

-- ============================================================================
-- 1. form_templates — Global Form Definitions
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_form_templates_category
  ON public.form_templates(category);

CREATE INDEX IF NOT EXISTS idx_form_templates_parent
  ON public.form_templates(parent_template_id);

CREATE INDEX IF NOT EXISTS idx_form_templates_tags
  ON public.form_templates USING GIN(tags);

CREATE INDEX IF NOT EXISTS idx_form_templates_schema
  ON public.form_templates USING GIN(schema);
...
CREATE TABLE IF NOT EXISTS public.tenant_form_selections (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
ALTER TABLE public.tenant_form_selections ENABLE ROW LEVEL SECURITY;

-- Indexes
CREATE INDEX IF NOT EXISTS idx_tenant_form_selections_tenant
  ON public.tenant_form_selections(tenant_id);

CREATE INDEX IF NOT EXISTS idx_tenant_form_selections_template
  ON public.tenant_form_selections(form_template_id);

CREATE INDEX IF NOT EXISTS idx_tenant_form_selections_active
  ON public.tenant_form_selections(tenant_id, is_active)
...
CREATE TABLE IF NOT EXISTS public.form_template_mappings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
...
ALTER TABLE public.form_template_mappings ENABLE ROW LEVEL SECURITY;

-- Indexes
CREATE INDEX IF NOT EXISTS idx_form_template_mappings_tenant
  ON public.form_template_mappings(tenant_id);

CREATE INDEX IF NOT EXISTS idx_form_template_mappings_contract
  ON public.form_template_mappings(contract_id);

CREATE INDEX IF NOT EXISTS idx_form_template_mappings_template
  ON public.form_template_mappings(form_template_id);
...
CREATE TABLE IF NOT EXISTS public.form_submissions (
  ...
  contract_id UUID NOT NULL,
  mapping_id UUID REFERENCES public.form_template_mappings(id),
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_form_submissions_tenant
  ON public.form_submissions(tenant_id);

CREATE INDEX IF NOT EXISTS idx_form_submissions_event
  ON public.form_submissions(service_event_id);
...
CREATE INDEX IF NOT EXISTS idx_form_submissions_responses
  ON public.form_submissions USING GIN(responses);


-- ============================================================================
-- 5. form_attachments — File Uploads (Documents, Images, Video)
-- ============================================================================
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_form_attachments_submission
  ON public.form_attachments(form_submission_id);

CREATE INDEX IF NOT EXISTS idx_form_attachments_field
  ON public.form_attachments(field_id);

CREATE INDEX IF NOT EXISTS idx_form_attachments_tenant
  ON public.form_attachments(tenant_id);
...
-- tenant_form_selections: tenant isolation
CREATE POLICY "tenant_form_selections_tenant_isolation"
  ON public.tenant_form_selections
  FOR ALL
  USING (tenant_id::text = current_setting('request.jwt.claims', true)::json->>'tenant_id');

-- form_template_mappings: tenant isolation
CREATE POLICY "form_template_mappings_tenant_isolation"
  ON public.form_template_mappings
  FOR ALL
  USING (tenant_id::text = current_setting('request.jwt.claims', true)::json->>'tenant_id');

-- form_submissions: tenant isolation
CREATE POLICY "form_submissions_tenant_isolation"
  ON public.form_submissions
  FOR ALL
  USING (tenant_id::text = current_setting('request.jwt.claims', true)::json->>'tenant_id');

-- form_attachments: tenant isolation
CREATE POLICY "form_attachments_tenant_isolation"
