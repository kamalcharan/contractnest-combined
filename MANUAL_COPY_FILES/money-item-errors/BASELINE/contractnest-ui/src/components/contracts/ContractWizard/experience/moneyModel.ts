...
export function moneyPreview(s: ContractWizardState) {
  ...
  if (errors.length) return {totals,errors,events:[]};
  ...
  if (Math.abs(total - totals.grandTotal) > 0.001) errors.push('The payment schedule does not add up to the contract total. Review the quantity, cycles and term before saving Money.');
