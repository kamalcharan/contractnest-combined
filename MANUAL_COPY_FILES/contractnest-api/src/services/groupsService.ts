...
export const groupsService = {
  ...
  async groupDiscovery(
    ...
    try {
      const n8nEnv = VaNiN8NConfig.mapEnvironment(environment);
      const n8nUrl = VaNiN8NConfig.getWebhookUrl('GROUP_DISCOVERY', n8nEnv);

      console.log(`🔍 Group Discovery: Calling N8N webhook [${n8nEnv}]:`, n8nUrl);
      console.log(`🔍 Group Discovery: Request:`, {
        intent: request.intent,
        channel: request.channel,
        groupId: request.group_id,
        hasQuery: !!request.query,
        hasSegment: !!request.segment,
        hasMembershipId: !!request.membership_id
      });

      const n8nRequest: GroupDiscoveryRequest = {
        intent: request.intent,
        group_id: request.group_id,
        channel: request.channel,
        ...
        ...(request.membership_id && { membership_id: request.membership_id }),
        ...(request.business_name && { business_name: request.business_name }),
        ...(request.limit && { limit: request.limit }),
      };

      const response = await axios.post<GroupDiscoveryResponse>(
        n8nUrl,
        n8nRequest,
        {
          headers: { 'Content-Type': 'application/json' },
      ...
      console.log(`🔍 Group Discovery: Response received:`, {
        ...
        responseType: n8nData.response_type,
        resultsCount: n8nData.results_count,
      ...
      return {
        ...
        response_type: n8nData.response_type,
        detail_level: n8nData.detail_level,
      ...
      captureException(error instanceof Error ? error : new Error(String(error)), {
        tags: { source: 'groupsService', action: 'groupDiscovery', via: 'n8n' },
        extra: { intent: request.intent, groupId: request.group_id }
      });
