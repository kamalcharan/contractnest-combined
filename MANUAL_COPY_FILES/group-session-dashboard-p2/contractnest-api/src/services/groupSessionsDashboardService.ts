...
class GroupSessionsDashboardService {
  ...
  dashOccurrences(tenantId: string, contractId: string) {
    return this.call('gs_dash_occurrences', { p_tenant: tenantId, p_contract: contractId });
  }

  /** Roster enumeration + per-member attendance + dues for one session. */
  dashRoster(tenantId: string, contractId: string) {
    return this.call('gs_dash_roster', { p_tenant: tenantId, p_contract: contractId });
  }
