...
const GroupSessionsPage: React.FC = () => {
  ...
  const occurrencesQuery = useGroupSessionOccurrences(selectedSession?.contract_id, {
    enabled: view === 'session' && sessionTab === 'occurrences',
  });
  const rosterQuery = useGroupSessionRoster(selectedSession?.contract_id, {
        ...
        <Card style={cardStyle}>
          <CardContent className="flex flex-col items-center gap-3 py-12">
            ...
            <span className="text-sm text-center max-w-md" style={subText}>
              A group session appears here once a contract is marked as the session owner (it holds the
              shared schedule that members attend).
            </span>
          </CardContent>
        </Card>
      ) : (
  ...
  const OccurrencesTab = () => {
    ...
    if (rows.length === 0) return <EmptyRow label="No occurrences scheduled for this session." />;
    return (
      <Card style={cardStyle}>
            ...
            <tbody>
              {rows.map((o) => (
                <tr key={o.event_id} style={{ borderBottom: `1px solid ${colors.utility.primaryText}0d` }}>
                  ...
                  <Td>
                    {o.status === 'past' ? (
                      <span className="inline-flex items-center gap-1" style={{ color: colors.utility.secondaryText }}>
                        <CheckCircle2 size={13} /> Held
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1" style={{ color: colors.semantic.warning }}>
                        <Clock size={13} /> Upcoming
                      </span>
                    )}
                  </Td>
                  <Td align="right">
                    <span style={primaryText}>{o.present}</span>
                  </Td>
                </tr>
              ))}
            </tbody>
