...
import type { KnowledgeTreeCycle } from '@/pages/service-contracts/templates/admin/knowledge-tree/types';
...
const RfqBuilderPage: React.FC = () => {
  ...
  // Already-added KT suggestions — a block's config.ktCycleId marks it as
  // created from a specific m_service_cycles row, so that suggestion stops
  // showing once added (dedupe keyed by the KT cycle, not the block).
  const addedKtCycleIds = useMemo(
    () => new Set(services.map((b) => b.config?.ktCycleId).filter(Boolean) as string[]),
    [services]
  );

  // Clicking a suggestion adds the real thing immediately, in the same shape
  // as any manually-added Service block — no preview/staging step. Price
  // stays 0 (vendor quotes it); the KT range is carried as a reference only.
  // description comes from the checkpoint names the KT package covers
  // (built in KTSuggestionsForCoverage) — was previously hardcoded to '',
  // which is why suggestions were landing with no description at all.
  const addFromKT = (c: CoverageLine, { cycle, description }: KTSuggestion) => {
    ...
    const newBlock: ConfigurableBlock = {
      id,
      name: cycle.catalog_name || cycle.checkpoint_name || 'Service',
      description,
...
interface KTSuggestion {
  cycle: KnowledgeTreeCycle;
  description: string;
}
interface KTSuggestionsForCoverageProps {
  coverageLine: CoverageLine;
  resourceTemplateId: string;
  addedCycleIds: Set<string>;
  colors: any;
...
const KTSuggestionsForCoverage: React.FC<KTSuggestionsForCoverageProps> = ({
  ...
  const suggestions = useMemo(() => {
    ...
    // checkpoint.name is the granular technical detail ("Output Voltage —
    // L-N (V)", "Load Current — Phase R (A)") — this is what actually makes
    // a useful description; the cycle object itself carries no description
    // field (confirmed by reading the edge function's query), and the
    // checkpoint's own free-text `description` is populated on only ~4% of
    // rows across the whole KT dataset, so it can't be relied on alone.
    ...
    const built: KTSuggestion[] = Array.from(groups.values()).map(({ rep, checkpointIds }) => {
      const labels = Array.from(checkpointIds)
      ...
      const description = shown.length
        ? `Covers: ${shown.join(', ')}${more > 0 ? ` (+${more} more)` : ''}`
        : '';
      return { cycle: rep, description };
    });

    return built
      .filter(({ cycle }) => !addedCycleIds.has(cycle.id))
      .sort((a, b) => a.cycle.frequency_value - b.cycle.frequency_value);
  }, [summary, addedCycleIds]);
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      {suggestions.map(({ cycle: c, description }) => (
        <div key={c.id} style={{ border: `1px solid ${line}`, borderRadius: 10, padding: '9px 11px', marginBottom: 8 }}>
          <div style={{ fontSize: 13, fontWeight: 650, color: ink, marginBottom: 3 }}>
            {c.catalog_name || c.checkpoint_name || 'Service'}
          </div>
          <div style={{ fontSize: 11.5, color: sub, marginBottom: 4 }}>
            Every {c.frequency_value} days
          </div>
          {description && (
            <div style={{ fontSize: 11, color: sub, marginBottom: 4, lineHeight: 1.4 }}>
              {description}
            </div>
          )}
          {typeof c.price_median === 'number' && (
            <div style={{ fontSize: 11, color: sub, marginBottom: 8 }}>
              {getCurrencySymbol(c.price_currency || 'INR')}
              {(c.price_min ?? 0).toLocaleString()}–{(c.price_max ?? 0).toLocaleString()} (market reference)
            </div>
          )}
          <button
            type="button"
            onClick={() => onAdd({ cycle: c, description })}
