...
export function canGoNextForStep(
  ...
  switch (currentStepId) {
    ...
    case 'blocks':
      if (state.selectedBlocks.length === 0) return false;
      // RFQ: every line is free-text the buyer typed, so an unnamed line is a
      // line vendors can't quote. Require a name on each before continuing.
      if (ctx.isRfqMode) {
        return state.selectedBlocks.every((b) => (b.name || '').trim() !== '');
      }
      return true;
    case 'billingView':
      return true;
    case 'assetSelection':
      // RFQ: equipment/facility come from the tenant's own registry and are
      // OPTIONAL — a pure-service request ("10 guards, 1 year") has none. The
      // services step still enforces at least one line, so an RFQ is never empty.
      if (ctx.isRfqMode) return true;
      // Contract: coverage types are mandatory — pick at least one type.
