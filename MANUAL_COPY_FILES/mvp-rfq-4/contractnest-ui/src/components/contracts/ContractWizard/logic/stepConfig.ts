...
export const RFQ_STEPS: StepConfig[] = [
  ...
  // Equipment / facilities the request covers, drawn from the buyer's OWN
  // registry. Optional — a pure-service request skips straight past it.
  { id: 'assetSelection', label: 'Equipment', heading: { title: 'What does this cover?', subtitle: 'Pick the equipment or facilities from your registry — or skip for a pure service' } },
...
export function blockedHintFor(stepId: StepId, isRfqMode: boolean): string {
  switch (stepId) {
    ...
    case 'blocks': return isRfqMode ? 'Name every line you want quoted' : 'Add at least one service block';
    case 'assetSelection': return isRfqMode ? 'Pick equipment, or continue for a pure service' : 'Select at least one coverage type';
