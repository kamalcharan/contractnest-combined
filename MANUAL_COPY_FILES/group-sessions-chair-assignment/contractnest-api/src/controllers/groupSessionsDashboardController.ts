...
class GroupSessionsDashboardController {
  ...
  assignChair = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    const result = await groupSessionsDashboardService.assignChair(tenantId, id, assigned_to ?? null, assigned_to_name ?? null);
    if (!result.success) {
  ...
  assignDefaultChair = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    const result = await groupSessionsDashboardService.assignDefaultChair(tenantId, blockId, this.isLive(req), assigned_to, assigned_to_name ?? null);
    if (!result.success) {
