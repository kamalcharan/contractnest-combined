...
// SCALE-OPTIMIZED VERSION with Idempotency + In-Memory Caching
// Updated: January 2025
//
// Features:
// - Standard idempotency framework for POST/PUT operations
// - 15-second in-memory caching for GET operations
// - RLS bypass using service role key (Authorization header removed)
...
} from "../_shared/common/idempotency.ts";
import {
  getCacheKey,
  getFromCache,
  setCache,
  invalidateCacheByPrefix
} from "../_shared/common/cache.ts";

// CORS headers with idempotency-key support
const corsHeaders = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type, x-tenant-id, x-user-id, x-product, x-idempotency-key',
...
serve(async (req) => {
  ...
  try {
    ...
    const productCode = req.headers.get('x-product') || 'contractnest';
    const idempotencyKey = getIdempotencyKeyFromRequest(req);

    console.log(`[Plans] ${req.method} - Product: ${productCode}, Tenant: ${tenantHeader?.substring(0, 8)}...`);
    ...
    // IMPORTANT: When using SUPABASE_SERVICE_ROLE_KEY, do NOT pass Authorization header
    // Passing the user's JWT causes Supabase to evaluate RLS with user context instead of bypassing RLS
    // The service role key is designed to bypass RLS for admin operations like plan management
    const supabase = createClient(supabaseUrl, supabaseKey, {
      global: {
        headers: {
          'x-tenant-id': tenantHeader
          // Note: Authorization header intentionally omitted to use service role's RLS bypass
        }
      },
      auth: {
    ...
    if (req.method === 'GET') {
      ...
      if (pathSegments.length > 1 && pathSegments[1]) {
        ...
        // Cache key for single plan
        const cacheKey = getCacheKey('plans', tenantHeader, 'single', planId, isEdit ? 'edit' : 'view');
        const cachedData = getFromCache(cacheKey);

        if (cachedData) {
          return new Response(
            JSON.stringify({ ...cachedData, _cached: true, _duration_ms: Date.now() - startTime }),
            { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }

        console.log(`[Plans] Fetching plan ${planId}${isEdit ? ' for edit' : ''}`);
        ...
        if (isEdit) {
          ...
          responseData = { ...plan, activeVersion, product_name: productName };
        }

        // Cache the response
        setCache(cacheKey, responseData, { ttlSeconds: CACHE_TTL_SECONDS });

        return new Response(
          JSON.stringify({ ...responseData, _duration_ms: Date.now() - startTime }),
          { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }

      // List plans (with caching)
      const showArchived = url.searchParams.get('showArchived') === 'true';
      const planType = url.searchParams.get('planType');
      const filterProduct = url.searchParams.get('product_code') || productCode;

      // Cache key for list
      const listCacheKey = getCacheKey('plans', tenantHeader, 'list', filterProduct, planType || 'all', showArchived ? 'archived' : 'active');
      const cachedList = getFromCache(listCacheKey);

      if (cachedList) {
        return new Response(
          JSON.stringify({ data: cachedList, _cached: true, _duration_ms: Date.now() - startTime }),
          { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      ...
      console.log(`[Plans] Listing plans for product: ${filterProduct}`);
      ...
      setCache(listCacheKey, enrichedPlans, { ttlSeconds: CACHE_TTL_SECONDS });

      return new Response(
        JSON.stringify({ data: enrichedPlans, _duration_ms: Date.now() - startTime }),
        { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (req.method === 'POST') {
      ...
      if (idempotencyKey) {
        const idempotencyResult = await checkIdempotency(supabase, idempotencyKey, tenantHeader);
        if (idempotencyResult.exists) {
          console.log(`[Plans] Idempotent request - returning cached response`);
          return new Response(
            JSON.stringify({ ...idempotencyResult.response, _idempotent: true }),
      ...
      if (requestData.plan_id) {
        console.log('[Plans] Creating new version from edit');
        ...
        if (!validation.valid) {
          console.error('[Plans] Validation errors:', validation.errors);
          return new Response(
        ...
        if (versionError) {
          console.error('[Plans] Database error creating version:', versionError);
          throw versionError;
        }
        ...
        // Invalidate cache for this tenant's plans
        invalidateCacheByPrefix(`plans:${tenantHeader}`);

        // Save idempotency
        if (idempotencyKey) {
          await saveIdempotency(supabase, idempotencyKey, tenantHeader, responseData);
        }
      ...
      console.log(`[Plans] Creating plan for product: ${planProductCode}`);
      ...
      if (requestData.initial_version) {
        ...
        responseData = { ...plan, activeVersion: version };
      }

      responseData._duration_ms = Date.now() - startTime;
    ...
    // ==========================================
    // PUT - Update operations (with idempotency)
    // ==========================================
    if (req.method === 'PUT') {
      // Check idempotency first
      if (idempotencyKey) {
        const idempotencyResult = await checkIdempotency(supabase, idempotencyKey, tenantHeader);
        if (idempotencyResult.exists) {
          console.log(`[Plans] Idempotent PUT request - returning cached response`);
          return new Response(
            JSON.stringify({ ...idempotencyResult.response, _idempotent: true }),
            { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
      ...
      if (pathSegments.length > 2) {
        ...
        if (operation === 'visibility') {
          ...
          const responseData = { ...data, _duration_ms: Date.now() - startTime };
