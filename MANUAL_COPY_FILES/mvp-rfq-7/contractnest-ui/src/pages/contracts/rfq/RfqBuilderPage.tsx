...
const RfqBuilderPage: React.FC = () => {
  ...
  const canAdvance = useMemo(() => {
    switch (stepId) {
      ...
      case 'type': return nomenclatureId !== null;
  ...
  }, [stepId, assetKind, nomenclatureId, name, durationValue, services, vendors]);
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '28px 20px 0' }}>
        ...
        {stepId === 'type' && (
          <>
            ...
            {nomLoading ? (
              ...
              <div style={{ ...card, color: sub, fontSize: 13 }}>No contract types configured yet.</div>
            ) : (
        ...
        {stepId === 'services' && (
          <>
            ...
            <p style={{ fontSize: 14, color: sub, margin: '0 0 20px' }}>Add each service on the fly — free-form lines you define here, not tied to a catalog — with how often it happens.</p>
            {services.map((s) => (
