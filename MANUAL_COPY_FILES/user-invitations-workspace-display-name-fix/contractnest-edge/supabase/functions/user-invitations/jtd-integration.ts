...
/**
 * Send invitation via JTD.
 *
 * Owner decision (2026-07-22): user invitations are an identity-message
 * exception — they must ALWAYS go out regardless of the tenant's
 * rules/channel configuration, otherwise a tenant that has turned off
 * messaging (or is working in test mode) could never bring a real teammate
 * onto the platform. The channel-enabled check that used to gate this was
 * removed; jtd-worker independently exempts source_type_code='user_invite'
 * from its own TEST-env guardrail and tenant channel kill switch, so this
 * is enforced at both the enqueue and dispatch layers.
 *
...
export async function sendMultiChannelInvitation(
  ...
  // Process each channel. Identity-message exception (owner decision
  // 2026-07-22): invitations always send regardless of the tenant's channel
  // config — no isChannelEnabled() check here; jtd-worker independently
  // exempts source_type_code='user_invite' from its own gates.
