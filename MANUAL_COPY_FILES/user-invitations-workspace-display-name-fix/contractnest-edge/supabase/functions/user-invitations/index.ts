...
// Find an auth.users record by email, paginating through the FULL user
// list. Supabase's admin listUsers() only returns one page per call — a
// bare page:1 lookup silently misses any account that isn't on that first
// page. This project already has more users than a single default page,
// so that bug wrongly treated existing accounts as brand new, sending
// them down the "create account" path where auth.admin.createUser then
// correctly (but confusingly) rejects the duplicate email.
async function findAuthUserByEmail(supabase: any, normalizedEmail: string): Promise<{ id: string; email: string } | null> {
  const perPage = 1000;
  const maxPages = 50; // safety cap, not a realistic ceiling
...
async function createInvitation(supabase: any, tenantId: string, userId: string, body: any, isLive: boolean) {
 try {
   ...
   // Get tenant's business display name (falls back to t_tenants.name if
   // onboarding/profile isn't complete yet)
   const { data: tenantProfile } = await supabase
     .from('t_tenant_profiles')
     .select('business_name')
     .eq('tenant_id', tenantId)
     .single();
   const workspaceDisplayName = tenantProfile?.business_name || tenant?.name || 'Workspace';
  ...
  try {
    ...
    if (invitation.email) {
      ...
      if (profiles && profiles.length > 0) {
        ...
        // Fallback: Check in auth.users (case-insensitive), paginated
        // across the full user list — not just the first page.
        const matchingUser = await findAuthUserByEmail(supabase, normalizedInviteEmail);
        if (matchingUser) {
          userExists = true;
  ...
  try {
    ...
    if (!actualUserId && email) {
      ...
      if (profiles && profiles.length > 0) {
        ...
        // paginated across the full user list — not just the first page.
        const matchingUser = await findAuthUserByEmail(supabase, normalizedEmail);
        if (matchingUser) {
