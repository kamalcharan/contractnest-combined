...
// Import team member dropdown hook for manager assignment
...
type ModalType = 'channels' | 'classification' | 'persons' | 'tags' | 'address' | 'compliance' | 'manager' | null;
...
const ProfileDrawer: React.FC<ProfileDrawerProps> = ({
  ...
  const transformFormDataForAPI = (modalType: ModalType, data: any): any => {
    ...
    switch (modalType) {
      ...
      case 'manager':
        transformed.manager_id = data.manager_id || null;
        transformed.manager_name = data.manager_name || null;
        break;
  ...
  const openModal = (type: ModalType) => {
    switch (type) {
      ...
      case 'compliance':
        ...
        break;
      case 'manager':
        setFormData({ manager_id: contact.manager_id || '', manager_name: contact.manager_name || '' });
        break;
    }
    setActiveModal(type);
  };
  ...
  const getModalTitle = () => {
    switch (activeModal) {
      ...
      case 'manager': return 'Assign Account Manager';
      default: return 'Edit';
    }
  };
        ...
        return (
          <div className="space-y-4">
            <p className="text-sm" style={{ color: colors.utility.secondaryText }}>
              Assign a team member as the account manager for this contact.
            </p>
            <select
              value={formData.manager_id || ''}
              onChange={(e) => {
                const selectedId = e.target.value;
                const selectedMember = teamMembers.find(m => m.value === selectedId);
                setFormData({
                  ...formData,
                  manager_id: selectedId || null,
                  manager_name: selectedMember?.label || null,
                });
              }}
            ...
            >
              ...
              {loadingTeam && <option disabled>Loading team members...</option>}
              {teamMembers.map(m => (
                <option key={m.value} value={m.value}>{m.label}{m.subLabel ? ` (${m.subLabel})` : ''}</option>
              ))}
            </select>
            {formData.manager_name && (
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ backgroundColor: colors.brand.primary + '10' }}>
                <Shield className="h-4 w-4" style={{ color: colors.brand.primary }} />
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4" style={{ height: 'calc(100% - 72px)' }}>
          ...
          <ViewCard
            ...
            icon={<Shield className="h-3.5 w-3.5" style={{ color: colors.brand.primary }} />}
            iconBg={colors.brand.primary + '20'}
          ...
          >
            {contact.manager_name ? (
              <div className="flex items-center gap-2">
                ...
                >
                  {contact.manager_name.charAt(0).toUpperCase()}
                </div>
              ...
              <span className="text-xs italic" style={{ color: colors.utility.secondaryText }}>No manager assigned</span>
            )}
          </ViewCard>
