...
const RfqBuilderPage: React.FC = () => {
  ...
  // (m_kt_service_definitions, via KTSuggestionsForCoverage) — was
  // previously hardcoded to '', then a client-built "Covers: <checkpoint
  // names>" stand-in; now uses what KT actually generates for this.
  const addFromKT = (c: CoverageLine, { cycle, title, description }: KTSuggestion) => {
...
const KTSuggestionsForCoverage: React.FC<KTSuggestionsForCoverageProps> = ({
  ...
  const suggestions = useMemo(() => {
    ...
    // checkpoint names (the earlier "Covers: ..." approach), which only
    ...
    const built: KTSuggestion[] = Array.from(groups.values()).map(({ rep, checkpointIds }) => {
      ...
      return { cycle: rep, title, description };
    });
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      {suggestions.map(({ cycle: c, title, description }) => (
        <div key={c.id} style={{ border: `1px solid ${line}`, borderRadius: 10, padding: '9px 11px', marginBottom: 8 }}>
          ...
          <button
            type="button"
            onClick={() => onAdd({ cycle: c, title, description })}
