 ...
 */
export const useCatBlocksTest = (options?: { strict?: boolean }) => {
      ...
      try {
        if (options?.strict) {
          ...
          for (let page = 1; page <= 100; page++) {
            const response = await api.get(`/api/catalog-studio/blocks?page=${page}&limit=100`);
            const data = response.data?.data;
            if (response.data?.success !== true || !Array.isArray(data?.blocks) || !Number.isInteger(data.total) || data.total < 0) throw new Error('Catalogue response is incomplete.');
            ...
            if (blocks.length >= data.total) return { success: true, data: { blocks, total: data.total } };
