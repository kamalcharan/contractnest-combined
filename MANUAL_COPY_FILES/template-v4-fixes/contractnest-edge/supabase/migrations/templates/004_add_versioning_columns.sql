...
-- Adds copy-on-write versioning support to m_cat_templates
...
CREATE INDEX IF NOT EXISTS idx_cat_templates_is_latest
  ON m_cat_templates (is_latest)
  WHERE is_latest = true;

-- Index for version chain lookups
CREATE INDEX IF NOT EXISTS idx_cat_templates_parent
  ON m_cat_templates (parent_template_id);

-- Composite index for common query: latest + active
CREATE INDEX IF NOT EXISTS idx_cat_templates_latest_active
  ON m_cat_templates (is_latest, is_active)
  WHERE is_latest = true AND is_active = true;

COMMENT ON COLUMN m_cat_templates.is_latest IS 'True for the current version, false for historical/legacy versions';
COMMENT ON COLUMN m_cat_templates.parent_template_id IS 'Links all versions of the same template. Points to the original template ID.';
