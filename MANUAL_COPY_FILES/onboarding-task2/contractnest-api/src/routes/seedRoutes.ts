...
// =================================================================
// POST /tenant/industry-confirmed - Seed catalog blocks + facility
// hierarchy when tenant confirms their industry during onboarding.
//
// Body: { industryId: string, businessType: 'buyer' | 'seller' | 'both' }
...
router.post('/tenant/industry-confirmed', async (req: Request, res: Response) => {
  try {
    ...
    console.log('[SeedRoutes] POST /tenant/industry-confirmed', {
      tenantId,
      industryId,
      businessType,
    });
    ...
    console.error('[SeedRoutes] Error in /tenant/industry-confirmed:', error.message);
    return res.status(500).json({
      success: false,
      error: error.message || 'Failed to seed tenant industry data',
    });
  }
});
