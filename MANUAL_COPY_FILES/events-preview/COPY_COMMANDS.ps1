...
$files = @(
 ...
 'contractnest-ui\src\components\contracts\ContractWizard\logic\state.ts',
 ...
 'contractnest-ui\src\components\contracts\ContractWizard\experience\useEventCalendar.ts',
 'contractnest-ui\src\components\contracts\ContractWizard\experience\events.css'
)
function Read-Normalized([string]$path) { return ((Get-Content -LiteralPath $path -Raw) -replace "`r", '') }
# Preflight every target before copying ANY file; reapplying is safe.
...
Write-Host 'Experience draft > Save Delivery & acceptance > Continue to Events Preview.'
