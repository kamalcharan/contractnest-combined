...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const checkOnboardingStatus = async (): Promise<boolean> => {
    try {
      // FIX: Check API headers instead of React state (which may be stale during login/init)
      ...
      if (!hasAuth || !hasTenant) {
        return true; // Don't block if not properly configured
      }
      ...
      // FIX: API returns { needs_onboarding, onboarding: { is_completed }, ... } directly (no data wrapper)
      const data = response.data;

      if (data) {
        // Primary check: needs_onboarding flag
        if (typeof data.needs_onboarding === 'boolean') {
          const isComplete = !data.needs_onboarding;
        ...
        // Fallback: check onboarding.is_completed
        const isComplete = data.onboarding?.is_completed || false;
        setHasCompletedOnboarding(isComplete);
        if (isComplete) {
          sessionStorage.setItem(STORAGE_KEYS.ONBOARDING_COMPLETE, 'true');
        }
  ...
  // Function to mark onboarding as complete (used by CompleteStep)
