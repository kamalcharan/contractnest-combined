...
class VaniComposerController {
  ...
  /**
   * GET /entitlement — is this tenant allowed to see/use VaNi?
   * (Business rule: the Agent is a VaNi-subscription feature; the UI hides
   * the entry point when not entitled.)
   */
  ...
  compose = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      // Buyer is OPTIONAL: when not found, VaNi composes anyway and the user
      // selects or quick-creates the buyer in the wizard (audit finding 4a).
      const buyer = buyerId && buyerName ? { id: buyerId, name: buyerName } : null;

      const ctx = this.getContext(req);
      const result = await contractComposerService.compose(intent, buyer, ctx);
      sendSuccess(res, result);
    } catch (error: any) {
