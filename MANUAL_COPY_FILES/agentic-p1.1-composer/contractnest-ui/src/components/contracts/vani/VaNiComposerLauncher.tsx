...
const VaNiComposerLauncher: React.FC<VaNiComposerLauncherProps> = ({
  ...
  const handleParse = useCallback(async () => {
    ...
    try {
      ...
      if (result.buyer.status === 'resolved' && result.buyer.contact) {
        await runCompose(result, result.buyer.contact);
      } else if (result.buyer.status === 'ambiguous') {
        setStage('disambiguate');
      } else {
        // Buyer not found: compose anyway — the wizard's buyer step lets the
        // user pick or quick-create the contact. No dead ends.
        if (result.intent.buyer_text) {
          addToast({
            type: 'info',
            title: `"${result.intent.buyer_text}" not in your contacts`,
            message: 'Drafting anyway — you can select or create the buyer in the wizard.',
          });
        }
        await runCompose(result, null);
      }
    } catch (err: any) {
  ...
  const runCompose = useCallback(async (
    ...
    setStage('composing');
    try {
      const result = await vaniComposerService.compose(parsed.intent, buyer);
        ...
        <div className="p-5">
            ...
            <div>
              ...
              <div className="space-y-2">
                {parseResult.buyer.candidates.map((c) => (
                  <button
                    key={c.id}
                    onClick={() => runCompose(parseResult, { id: c.id, name: c.name })}
                ...
                <button
                  onClick={() => runCompose(parseResult, null)}
                  className="w-full px-4 py-2.5 rounded-xl border border-dashed text-left text-xs hover:opacity-80 transition-opacity"
                  style={{ borderColor: `${colors.utility.primaryText}25`, color: colors.utility.secondaryText }}
                >
                  None of these — draft anyway, I'll pick the buyer in the wizard
                </button>
              </div>
              <button
          ...
          {stage === 'result' && composeResult && (
            <div>
              ...
              {/* Readiness — every wizard step satisfied or explicitly flagged */}
              {composeResult.readiness && (
                <div
                  className="px-3.5 py-2.5 rounded-xl mb-3 border"
                  style={{
                    borderColor: composeResult.readiness.needsYou.length === 0
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <CheckCircle2
                      ...
                      style={{
                        color: composeResult.readiness.needsYou.length === 0
                          ? colors.semantic.success
                          : colors.semantic.warning,
                      }}
                    />
                    <span className="text-xs font-bold" style={{ color: colors.utility.primaryText }}>
                      Ready: {composeResult.readiness.readyCount} of {composeResult.readiness.totalCount} steps
                    </span>
                  </div>
                  {composeResult.readiness.needsYou.length > 0 && (
                    <p className="text-[11px] leading-relaxed pl-5.5" style={{ color: colors.utility.secondaryText }}>
                      Needs you: {composeResult.readiness.needsYou.join(' · ')}
                    </p>
                  )}
                  {composeResult.draft.nomenclatureName && (
                    <p className="text-[10px] mt-1 pl-5.5" style={{ color: colors.utility.secondaryText }}>
                      Contract type: {composeResult.draft.nomenclatureName}
                      {composeResult.draft.acceptanceMethod ? ` · Acceptance: ${composeResult.draft.acceptanceMethod}` : ''}
                    </p>
                  )}
                </div>
              )}
