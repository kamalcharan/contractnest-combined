...
export default function MoneyPage(p:Props) {
  ...
  const locked = p.busy || changedContext;
  ...
  const save = async(complete:boolean) => {
    if (locked || !totalsReady || !safeTotals) return;
    setAttempted(complete);setSaved(null);
    if (complete && errors.length) return;
    if (await p.onSave()) setSaved(complete ? snapshot : null);
  };
  ...
  return <div className="ag-page mn-page" style={{'--ag-bg':c.utility.primaryBackground,'--ag-panel':c.utility.secondaryBackground,'--ag-text':c.utility.primaryText,'--ag-muted':c.utility.secondaryText,'--ag-line':`${c.utility.primaryText}22`,'--ag-brand':c.brand.primary,'--ag-onbrand':textOnBrand(c.brand.primary),'--ag-error':c.semantic.error} as React.CSSProperties}>
    ...
    <footer className="ag-footer"><button disabled={locked} onClick={p.onBack}><ArrowLeft size={17}/>Services</button><button disabled={locked||!safeTotals||!totalsReady} onClick={()=>void save(false)}><Save size={17}/>Save draft</button><button className="ag-primary" disabled={locked||!safeTotals||!totalsReady} onClick={()=>void save(true)}>Save Money</button></footer>
  </div>;
}
