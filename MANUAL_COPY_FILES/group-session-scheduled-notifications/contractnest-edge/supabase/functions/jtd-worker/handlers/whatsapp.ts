  ...
  try {
    ...
    if (templateData && Object.keys(templateData).length > 0) {
      ...
      if (templateName === 'user_invitation') {
        ...
        // Body: Hi {{member_name}}, looking forward to seeing you at
        // {{session_name}} on {{occurrence_date}} at {{start_time}}...
        // Four variables — the only group-session template carrying a time.
        // Enqueue side (gs_run_session_notifications) only creates these for
        // blocks that have groupSession.timing.startTime set, so start_time is
        ...
        // Body: Hi {{member_name}}, we missed you at {{session_name}} on
        // {{occurrence_date}}...
        orderedValues = [
          String(templateData.member_name || ''),
          String(templateData.session_name || ''),
