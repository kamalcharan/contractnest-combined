...
const STEP_NARRATION: Record<StepId, { title: string; detail: string }> = {
  ...
  template: { title: 'Checking your templates…', detail: 'A signed-off template can answer this instantly' },
          ...
          pushCard({
            ...
            body: (
              <p className="text-xs">
                ...
                Continuing — compare them on the Templates page before signing off.
              </p>
            ),
          });
        } else {
        ...
        {stage === 'input' && (
          <div className="p-6 max-w-xl mx-auto w-full">
            ...
            <div className="mt-3 space-y-1.5">
              {(isTemplateMode ? TEMPLATE_EXAMPLES : EXAMPLES).map((ex) => (
                <button
                  key={ex}
                  onClick={() => setText(ex)}
