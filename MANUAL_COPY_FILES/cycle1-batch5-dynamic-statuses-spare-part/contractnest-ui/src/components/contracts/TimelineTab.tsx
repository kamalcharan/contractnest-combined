 ...
 * Falls back to hex_color from DB, with sensible icon defaults.
 */
...
export const EventCard: React.FC<EventCardProps> = ({ event, currency, colors, onStatusChange, isUpdating, hideActions, statusDefs, allowedTransitions }) => {
  ...
  const accent = isSparePart
    ...
    : isService ? colors.semantic.success : colors.semantic.warning;
  const statusConfig = getStatusConfig(event.status, colors, statusDefs);
  const StatusIcon = statusConfig.icon;

  const validTransitions = allowedTransitions || [];
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        {groupedEvents.map((group) => {
          ...
          const sparePartEvts = group.events.filter(e => e.event_type === 'spare_part');
              ...
              <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))' }}>
                ...
                {serviceEvts.map((event) => (
                  <EventCard
                    ...
                    statusDefs={statusDefsByType.service}
                    allowedTransitions={transitionsByType.service[event.status] || []}
                  />
                ))}

                {/* Spare Part Events */}
                {sparePartEvts.map((event) => (
                  <EventCard
                    key={event.id}
                    event={event}
                    currency={currency}
                    ...
                    onStatusChange={handleStatusChange}
                    isUpdating={isChangingStatus}
                    statusDefs={statusDefsByType.spare_part}
                    allowedTransitions={transitionsByType.spare_part[event.status] || []}
                  />
                ))}
                ...
                {billingEvts.map((event) => (
                  <EventCard
                    ...
                    onStatusChange={handleStatusChange}
                    isUpdating={isChangingStatus}
                    statusDefs={statusDefsByType.billing}
                    allowedTransitions={transitionsByType.billing[event.status] || []}
                  />
                ))}
              </div>
            </div>
