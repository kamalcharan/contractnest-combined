...
export default function AgreementPage(p: Props) {
  ...
  const groups = [...new Set(items.map(item => item.form_settings?.group_label || 'Other'))];
  ...
  const save = async (complete: boolean) => {
    ...
    if (await p.onSave()) setSavedSnapshot(complete ? captured : null);
  };
  ...
  return <div className="ag-page" style={{ '--ag-bg': c.utility.primaryBackground, '--ag-panel': c.utility.secondaryBackground, '--ag-text': c.utility.primaryText, '--ag-muted': c.utility.secondaryText, '--ag-line': `${c.utility.primaryText}22`, '--ag-brand': c.brand.primary, '--ag-onbrand': textOnBrand(c.brand.primary), '--ag-error': c.semantic.error } as React.CSSProperties}>
    <header className="ag-header"><div><FileText size={22}/><strong>ContractNest</strong><span>Create an agreement</span></div><div><span role="status">{p.saveStatus === 'saving' ? 'Saving…' : p.saveStatus === 'failed' ? 'Save failed' : p.saveStatus === 'saved' ? 'Draft saved' : p.hasDraft ? 'Draft exists · autosave on' : 'Not saved yet'}</span><button type="button" aria-label="Close agreement" disabled={p.busy} onClick={p.onClose}><X size={20}/></button></div></header>
    ...
    <div className="ag-layout"><main><div className="ag-intro"><span className="ag-eyebrow">LET’S MAKE IT OFFICIAL</span><h1>Every commitment starts<br/>with an agreement.</h1><p>Who it’s with. What you’re agreeing to. The rest follows.</p></div>
      <fieldset disabled={p.busy} className="ag-fields">
      <section className="ag-card"><div className="ag-card-heading"><div><span className="ag-eyebrow">01 · THE RELATIONSHIP</span><h2>Who is this agreement with?</h2></div><span className={`ag-badge ag-${p.relationship}`}>{relation}</span></div>
        <p>Your contacts are filtered to {relation.toLowerCase()}s. This relationship applies to this agreement.</p>
      ...
      <section className="ag-card"><span className="ag-eyebrow">02 · THE ESSENTIALS</span><h2>Give the agreement its shape.</h2><p>Just the essentials. Fine-tune the rest when you need to.</p>
        <label>Agreement name<input value={s.contractName} maxLength={CONTRACT_TITLE_MAX_LENGTH} placeholder="Give this commitment a clear name" onChange={e => p.onChange({ contractName: e.target.value })}/></label>
        <div className="ag-grid"><label>Starts on<input type="date" value={dateInput(new Date(s.startDate))} onChange={e => p.onChange({ startDate: e.target.value ? new Date(`${e.target.value}T00:00:00`) : new Date(NaN) })}/></label><label>Agreement label <small>optional</small><select aria-label="Agreement label" disabled={isLoading || !!labelsError} value={s.nomenclatureId || ''} onChange={e => { const item = items.find(x => x.id === e.target.value); const fs = item?.form_settings as any; p.onLabel(item?.id || null, item ? fs?.short_name || item.display_name || item.detail_name : null, fs?.group || null); }}><option value="">{isLoading ? 'Loading labels…' : 'Choose a label'}</option>{s.nomenclatureId && !items.some(i => i.id === s.nomenclatureId) && <option value={s.nomenclatureId}>{s.nomenclatureName} — unavailable</option>}{groups.map(group => <optgroup key={group} label={group}>{items.filter(item => ((item.form_settings as any)?.group_label || 'Other') === group).map(item => <option key={item.id} value={item.id}>{(item.form_settings as any)?.short_name || item.display_name || item.detail_name}</option>)}</optgroup>)}</select></label></div>
        ...
        <div className="ag-grid"><label>Runs for<div className="ag-duration"><input aria-label="Duration" type="number" min="1" step="1" value={s.durationValue || ''} onChange={e => p.onChange({ durationValue: Number(e.target.value) })}/><select aria-label="Duration unit" value={s.durationUnit} onChange={e => p.onChange({ durationUnit: e.target.value })}>{['days', 'months', 'years'].map(unit => <option key={unit}>{unit}</option>)}</select></div></label><label>Currency<select aria-label="Currency" value={s.currency} onChange={e => p.onChange({ currency: e.target.value })}><option value="">Choose currency</option>{currencyOptions.map(currency => <option key={currency.code} value={currency.code}>{currency.code} · {currency.name}</option>)}</select></label></div>
        <div className="ag-presets"><span>Common terms</span><button type="button" onClick={() => p.onChange({ durationValue: 6, durationUnit: 'months' })}>6 months</button><button type="button" onClick={() => p.onChange({ durationValue: 1, durationUnit: 'years' })}>1 year</button></div>
        ...
        <details><summary>Description & grace period</summary><RichTextEditor value={s.description} onChange={description => p.onChange({ description })} label="Scope note · optional" maxLength={CONTRACT_DESCRIPTION_MAX_LENGTH}/><label>Grace period<div className="ag-duration"><input aria-label="Grace period" type="number" min="0" step="1" value={s.gracePeriodValue} onChange={e => p.onChange({ gracePeriodValue: Number(e.target.value) })}/><select aria-label="Grace unit" value={s.gracePeriodUnit} onChange={e => p.onChange({ gracePeriodUnit: e.target.value })}>{['days', 'months', 'years'].map(unit => <option key={unit}>{unit}</option>)}</select></div></label><p>New agreements remain drafts. Acceptance is decided after scope and price.</p></details>
      </section></fieldset>
      ...
      {ready && <section className="ag-card ag-ready" role="status"><Check size={24}/><h2>Agreement saved.</h2><p>Your essentials are safely in a draft. Coverage & services is the next build; this review release ends here. No acceptance request has been sent.</p><button type="button" onClick={() => setSavedSnapshot(null)}>Keep refining</button></section>}
      <details className="ag-mobile-summary"><summary>Your agreement so far</summary>{summary}</details>
    </main><aside>{summary}</aside></div>
    <footer className="ag-footer"><span>Agreement · 1 of 5</span><button type="button" disabled={p.busy} onClick={() => void save(false)}><Save size={17}/> Save draft</button><button type="button" className="ag-primary" disabled={p.busy} onClick={() => void save(true)}>{p.busy ? 'Saving…' : 'Save agreement'}<ArrowRight size={18}/></button></footer>
  </div>;
}
