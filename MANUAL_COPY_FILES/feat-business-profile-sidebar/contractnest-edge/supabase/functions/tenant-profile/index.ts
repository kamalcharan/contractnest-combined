...
serve(async (req) => {
  ...
  try {
    ...
    // Exclude sub-paths like /served-industries and /unlock-preview which have their own GET handlers below
    if (req.method === 'GET' && !url.pathname.endsWith('/served-industries') && !url.pathname.endsWith('/unlock-preview')) {
      try {
    ...
    // Exclude sub-paths like /served-industries and /logo which have their own POST handlers below
    if (req.method === 'POST' && !url.pathname.endsWith('/served-industries') && !url.pathname.endsWith('/logo')) {
      try {
        // Parse request body
    ...
    // =========================================================================
    // SERVED INDUSTRIES ENDPOINTS
    // =========================================================================

    // GET /served-industries - List industries this tenant serves
    if (req.method === 'GET' && url.pathname.endsWith('/served-industries')) {
      try {
        const { data, error } = await supabase
    ...
    if (req.method === 'POST' && url.pathname.endsWith('/served-industries')) {
      try {
        const requestData = await req.json();
    ...
    if (req.method === 'DELETE' && url.pathname.includes('/served-industries')) {
      try {
    ...
    if (req.method === 'GET' && url.pathname.endsWith('/served-industries/unlock-preview')) {
      try {
