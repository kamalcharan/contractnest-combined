...
interface TicketSummaryProps {
  ...
  colors: any;
  currency: string;
  onViewTicket?: (ticketNumber: string, assignedTo: string, completedAt: string, events: ContractEvent[]) => void;
}

const TicketSummaryCard: React.FC<TicketSummaryProps> = ({ date, deliverables, billing, colors, currency, onViewTicket }) => {
      ...
      >
        ...
        {onViewTicket && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              onViewTicket(ticket.ticketNumber, ticket.assignedTo, date, [...deliverables, ...billing]);
            }}
...
const OperationsTab: React.FC<OperationsTabProps> = ({
  ...
  const handleViewTicket = useCallback((ticketNumber: string, assignedTo: string, completedAt: string, events: ContractEvent[]) => {
    setTicketDetail({ isOpen: true, ticketNumber, assignedTo, completedAt, events });
  }, []);
  ...
  return (
    <div>
      ...
      <ServiceTicketDetail
        state={ticketDetail}
        onClose={() => setTicketDetail({ isOpen: false, ticketNumber: '', assignedTo: '', completedAt: '', events: [] })}
        colors={colors}
        currency={currency}
      />
    </div>
