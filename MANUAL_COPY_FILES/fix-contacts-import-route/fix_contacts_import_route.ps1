...
# Restores the /contacts/import route that was accidentally removed from
# contractnest-ui/src/App.tsx during the Sandbox wiring hand-merge (commit b1113d8).
#
...
#
# Idempotent: safe to run multiple times. Does NOT touch Sandbox or any other
# lines. Only inserts the two missing lines if they are absent.
#
...
if (-not (Test-Path $appTsx)) {
    ...
    Write-Host "Run this script from the contractnest-combined\ root." -ForegroundColor Yellow
    exit 1
}
...
if ($content -notmatch "import\s+ContactsImportPage\s+from\s+'\./pages/contacts/import'") {
    ...
    if ($content.Contains($importAnchor)) {
        $importInsert = $importAnchor + "`r`nimport ContactsImportPage from './pages/contacts/import';"
...
# --- 2. Restore the route (before the /contacts/:id route, after /contacts/create) ---
if ($content -notmatch '<Route\s+path="import"\s+element=\{<ContactsImportPage\s*/>\}') {
    ...
    if ($content.Contains($routeAnchor)) {
        $routeInsert = $routeAnchor + "`r`n            <Route path=`"import`" element={<ContactsImportPage />} />"
        $content = $content.Replace($routeAnchor, $routeInsert)
        $changed = $true
        Write-Host "[+] Added <Route path=`"import`"> before the :id route" -ForegroundColor Green
    } else {
...
if ($changed) {
    ...
    Write-Host "`n✅ App.tsx patched. Restart the UI dev server and hard-refresh (Ctrl+F5)." -ForegroundColor Green
} else {
    Write-Host "`nℹ️  No changes needed - App.tsx already has the import route." -ForegroundColor Cyan
}
