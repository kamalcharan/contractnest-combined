...
const handleApiServerDown = (error: any) => {
  ...
  console.log('[API] API Server Down detected - NOT redirecting (disabled to prevent loop)');
  ...
  // DISABLED: This was causing infinite redirect loops
...
api.interceptors.response.use(
  (response) => {
    ...
    if (response.headers['x-maintenance-mode'] === 'true') {
      ...
      // DISABLED: Redirect was causing loops
      // window.location.href = '/misc/maintenance';
      console.warn('[API] Maintenance mode detected but redirect disabled');
    ...
    if (response.headers['x-session-conflict'] === 'true') {
      ...
      console.warn('[API] Session conflict detected but redirect disabled');
  ...
  (error) => {
    ...
    if (!error.response && (error.message === 'Network Error' || error.code === 'ERR_NETWORK')) {
      ...
      if (!API_HEALTH_CHECK_ENABLED) {
        ...
        console.warn('[API] Server error but redirect disabled');
      }
    ...
    if (error.response?.status === 503) {
      ...
      // DISABLED: Redirect was causing loops
      // window.location.href = '/misc/maintenance';
      console.warn('[API] 503 Maintenance but redirect disabled');
      return Promise.reject(error);
    }
    ...
    if (error.response?.status === 401 && error.response?.data?.code === 'SESSION_CONFLICT') {
      ...
      // DISABLED: Redirect was causing loops
      // window.location.href = '/misc/session-conflict';
      console.warn('[API] Session conflict but redirect disabled');
      return Promise.reject(error);
    }
    ...
    if (error.response?.status === 403) {
      ...
      console.warn('[API] 403 Forbidden but redirect disabled');
      return Promise.reject(error);
    }
