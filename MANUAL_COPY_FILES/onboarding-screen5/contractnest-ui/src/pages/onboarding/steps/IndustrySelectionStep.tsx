...
const IndustrySelectionStep: React.FC = () => {
  ...
  useEffect(() => {
    if (!servedLoading && !hydrated && servedIndustries.length > 0) {
      const existingIds = new Set(servedIndustries.map(si => si.industry_id));
      setSelectedIds(existingIds);
      setHydrated(true);
    } else if (!servedLoading && !hydrated) {
      setHydrated(true);
    }
  }, [servedLoading, servedIndustries, hydrated]);
