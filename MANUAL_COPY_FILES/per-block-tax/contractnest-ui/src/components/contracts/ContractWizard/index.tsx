...
function mapWizardToRequest(
  ...
  // Build blocks array — flattened to match t_contract_blocks columns (with per-block tax)
  const blocks = state.selectedBlocks.map((block, idx) => {
    const isFlyBy = !isValidUUID(block.id);

    // Build per-block tax breakdown snapshot
    const blockTaxBreakdown = (block.taxes || []).map((tax) => {
      ...
      const baseForTax = block.taxInclusive
        ? lineTotal / (1 + (block.taxRate || 0) / 100)
        : lineTotal;
      return {
        tax_id: tax.id,
        name: tax.name,
        rate: tax.rate,
        amount: Math.round(baseForTax * (tax.rate / 100) * 100) / 100,
      };
    });
    ...
    return {
      ...
      unit_price: block.config?.customPrice || block.price,
      ...
      tax_inclusive: block.taxInclusive || false,
      tax_rate: block.taxRate || 0,
      tax_amount: block.taxAmount || 0,
      hsn_sac_code: block.hsnSacCode || '',
      tax_breakdown: blockTaxBreakdown,
      custom_fields: {
