...
const ServiceBlocksStep: React.FC<ServiceBlocksStepProps> = ({
  ...
  const handleAddBlock = useCallback(
    (block: Block) => {
      ...
      // Extract per-block tax data from catalog block meta / pricingRecords
      const blockMeta = block.meta as Record<string, any> | undefined;
      const pricingRecords = blockMeta?.pricingRecords as Array<any> | undefined;
      const primaryPricing = pricingRecords?.[0];

      const taxInclusive = primaryPricing?.tax_inclusion === 'inclusive'
        || blockMeta?.taxInclusive === true;
      const taxes = (primaryPricing?.taxes || []) as Array<{ id: string; name: string; rate: number }>;
      const taxRate = taxes.length > 0
        ? taxes.reduce((sum: number, t: { rate: number }) => sum + t.rate, 0)
        : (blockMeta?.taxRate || 0);
      const hsnSacCode = blockMeta?.hsn || blockMeta?.hsnSacCode || '';

      // Calculate initial tax amount
      const basePrice = block.price || 0;
      let taxAmount = 0;
      if (taxRate > 0) {
        if (taxInclusive) {
          // Price already includes tax: extract tax from inclusive price
          taxAmount = basePrice - (basePrice / (1 + taxRate / 100));
        } else {
      ...
      onBlocksChange(
        selectedBlocks.map((block) => {
          if (block.id === blockId) {
            ...
            // Recalculate per-block tax when price or quantity changes
