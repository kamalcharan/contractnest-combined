...
const OpsCockpitPage: React.FC = () => {
  ...
  const { data: dateSummary, isLoading: summaryLoading, isFetching: summaryFetching, refetch: refetchSummary } = useTenantDateSummary();

  // D2 data hooks
  const { data: pendingAcceptanceData, isLoading: acceptanceLoading } = useContracts({ status: 'pending_acceptance' as any, per_page: 10 });
  const { data: draftContractsData, isLoading: draftsLoading } = useContracts({ status: 'draft' as any, per_page: 10 });
  const { data: pendingReviewData, isLoading: pendingLoading } = useContracts({ status: 'pending_review' as any, per_page: 10 });
  ...
  const { data: eventsData, isLoading: eventsLoading } = useContractEvents({
    date_from: dateRange.from, date_to: dateRange.to, per_page: 20,
    sort_by: 'scheduled_date', sort_order: 'asc',
  });
  const { data: overdueEventsData, isLoading: overdueLoading } = useContractEvents({
    status: 'overdue' as any, per_page: 10, sort_by: 'scheduled_date', sort_order: 'asc',
  });
  ...
  const urgency = useMemo(() => {
    ...
    const safeBucket = (b: any) => ({
      count: Number(b?.count) || 0, service_count: Number(b?.service_count) || 0,
      billing_count: Number(b?.billing_count) || 0, billing_amount: Number(b?.billing_amount) || 0,
    ...
    return {
      overdue: safeBucket(dateSummary.overdue), today: safeBucket(dateSummary.today),
      tomorrow: safeBucket(dateSummary.tomorrow), this_week: safeBucket(dateSummary.this_week),
      next_week: safeBucket(dateSummary.next_week), later: safeBucket(dateSummary.later),
      totals: {
  ...
  // Handlers
  const handleRefresh = useCallback(() => { refetchStats(); refetchSummary(); }, [refetchStats, refetchSummary]);
