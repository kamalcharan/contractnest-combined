...
export const API_ENDPOINTS = {
  ...
  // Extend — customer touchpoints (Website / WhatsApp / Email). A touchpoint
  // is one published template exposed on one route behind a public storefront
  // key; buyers land on /buy/:storefrontKey.
  EXTEND: {
    TOUCHPOINTS: '/api/extend/touchpoints',
    TOUCHPOINT: (id: string) => `/api/extend/touchpoints/${id}`,
    // Public, unauthenticated — used by the hosted checkout page only.
    STOREFRONT: (key: string) => `/api/storefront/${key}`,
    STOREFRONT_PURCHASE: (key: string) => `/api/storefront/${key}/purchase`,
  },
