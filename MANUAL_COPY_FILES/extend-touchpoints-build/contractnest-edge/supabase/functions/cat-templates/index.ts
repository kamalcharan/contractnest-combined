...
async function handleSubscribeToPlan(
  ...
  // Real billing schedule derived by the API layer from the template's
  // wizard_state (same engine every contract uses). Bounded and shape-checked
  // here; NULL falls back to the RPC's single-upfront-event behaviour.
  const rawEvents = body?.computed_events;
  const computedEvents =
    Array.isArray(rawEvents) &&
    rawEvents.length > 0 &&
    rawEvents.length <= 366 &&
    rawEvents.every((e: any) => e && typeof e === 'object' && typeof e.event_type === 'string')
      ? rawEvents
      : null;
  ...
  const { data, error } = await supabase.rpc('subscribe_tenant_to_plan', {
    ...
    p_computed_events: computedEvents,
  });
...
async function handleGetPlanTemplates(
  ...
  const plans = (data || []).map((t: any) => {
    ...
    return {
      ...
      // The full authored wizard state, passed through for the API layer's
      // event derivation on subscribe (contractEventsDerivationService needs
      // selectedBlocks/paymentMode/duration to build the REAL billing
      // schedule). Display consumers are free to ignore it.
      wizard_state: t.settings?.wizard_state ?? null,
      defaults,
    };
  });
