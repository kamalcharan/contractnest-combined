...
// Two-phase: POST /api/knowledge-tree/generate → POST edge /save
// Exposes phase state so the UI can show animated progress.
...
export interface KTGenerateInput {
  ...
  existingKT?: boolean;
  resourceType?: 'equipment' | 'facility'; // routes to the correct LLM skill
}
  ...
  const generate = async (input: KTGenerateInput): Promise<string | null> => {
    ...
    try {
      ...
      const response = await api.post('/api/knowledge-tree/generate', {
        ...
        existingKT,
        resourceType,
      }, { timeout: 300_000 }); // 5 min — LLM generation takes 60–180s
