...
const ContractWizard: React.FC<ContractWizardProps> = ({
  ...
  // Wizard data state
  const [experienceStep, setExperienceStep] = useState<'agreement' | 'coverage' | 'services' | 'money' | 'delivery'>('agreement');
  ...
  useEffect(() => {
    if (draftContractData?.metadata?.wizard_state && isOpen) {
      ...
      if (agreementOnly) {
        setExperienceStep(draftContractData.metadata.experience_step === 'delivery' ? 'delivery' : draftContractData.metadata.experience_step === 'money' ? 'money' : draftContractData.metadata.experience_step === 'services' ? 'services' : draftContractData.metadata.experience_step === 'coverage' ? 'coverage' : 'agreement');
