...
$files = @(
 ...
 'contractnest-ui\src\components\contracts\ContractWizard\experience\DeliveryPage.tsx',
 'contractnest-ui\src\components\contracts\ContractWizard\experience\useDeliveryData.ts',
 'contractnest-ui\src\components\contracts\ContractWizard\experience\delivery.css'
)
function Read-Normalized([string]$path) { return ((Get-Content -LiteralPath $path -Raw) -replace "`r", '') }
# Validate ALL targets before copying any file. Preserve divergent local edits.
...
if ($CheckOnly) { Write-Host 'Preflight passed for all 5 UI files. No files changed.' -ForegroundColor Green; exit 0 }
...
Write-Host 'Copied and verified 5 UI files. Restart UI, then Ctrl+F5.' -ForegroundColor Green
Write-Host 'Open the experience draft. Money > Save Money > Continue to Delivery.'
