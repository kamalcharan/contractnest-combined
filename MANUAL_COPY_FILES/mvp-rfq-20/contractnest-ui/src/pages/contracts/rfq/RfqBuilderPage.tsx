...
const RfqBuilderPage: React.FC = () => {
  ...
  const addFromKT = (c: CoverageLine, cycle: KnowledgeTreeCycle) => {
...
interface KTSuggestionsForCoverageProps {
  ...
  colors: any;
  onAdd: (cycle: KnowledgeTreeCycle) => void;
}
const KTSuggestionsForCoverage: React.FC<KTSuggestionsForCoverageProps> = ({
  ...
  const suggestions = useMemo(() => {
    ...
    const deduped: KnowledgeTreeCycle[] = [];
    ...
    return deduped
      .filter((c) => !addedCycleIds.has(c.id))
    ...
    <div style={{ marginBottom: 14 }}>
      ...
      {suggestions.map((c) => (
        <div key={c.id} style={{ border: `1px solid ${line}`, borderRadius: 10, padding: '9px 11px', marginBottom: 8 }}>
          ...
          <div style={{ fontSize: 11.5, color: sub, marginBottom: typeof c.price_median === 'number' ? 4 : 8 }}>
