...
import { ArrowRight, ArrowUpRight, BriefcaseBusiness, Check, FileText, Layers, Plus, RefreshCw } from 'lucide-react';
...
function WorkspaceContent() {
  ...
  const showStart = empty || startOpen;
  const closeStart = () => { setStartOpen(false); startButton.current?.focus(); };
  ...
  return <main id="experience-main" className="xp-main">
    <section className={`xp-heading ${empty ? 'xp-heading-start' : ''}`}>
      ...
      <div className="xp-heading-actions"><div className="xp-context"><span className="xp-environment">{isLive ? 'Live' : 'Test'} environment</span><span>{revenue ? 'Revenue' : 'Expense'} perspective</span></div>
        {!empty && <button ref={startButton} className="xp-button xp-button-brand" aria-expanded={showStart} aria-controls="experience-start" onClick={() => { if (startOpen) closeStart(); else { measureStart('chooser_open', perspective, isLive); setStartOpen(true); } }}><Plus size={17} />Start something new</button>}</div>
    </section>
    {showStart && <StartChooser perspective={perspective} isLive={isLive} onAction={handleStart} onClose={empty ? undefined : closeStart} />}
...
export default function ExperiencePage() {
  ...
  return <div className="xp-root" style={style}>
    ...
    <footer className="xp-release"><span>Workspace experience · Sprint 1.3</span><Link to="/ops/cockpit">Return to existing cockpit<ArrowUpRight size={14} /></Link></footer>
  </div>;
}
