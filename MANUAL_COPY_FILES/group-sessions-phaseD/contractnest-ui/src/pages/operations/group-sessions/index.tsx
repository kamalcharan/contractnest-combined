...
import CheckinQrModal from '@/components/group-sessions/CheckinQrModal';
...
const GroupSessionsPage: React.FC = () => {
  ...
  const [checkinModal, setCheckinModal] = useState<{ url: string; title: string } | null>(null);

  // mint the block's static check-in token and open the QR modal
  const openCheckin = async (blockId?: string | null, title?: string) => {
    if (!blockId) return;
    try {
      const res = await ensureToken.mutateAsync({ blockId });
      if (res?.token) {
        const url = `${window.location.origin}/checkin/${res.token}`;
        setCheckinModal({ url, title: title || 'Group Session' });
      }
  ...
  // Chair: members' declared UPI payments awaiting confirmation → mark paid
  const PaymentsToConfirm = () => {
    const rows = declarationsQuery.data ?? [];
    if (declarationsQuery.isLoading || rows.length === 0) return null;
    return (
      <Card style={cardStyle}>
        <CardContent className="p-4">
          <div className="flex items-center gap-2 mb-3">
            <CircleDollarSign size={16} style={{ color: colors.brand.primary }} />
            <span className="font-semibold" style={primaryText}>Payments to confirm</span>
            <span
              className="text-xs px-2 py-0.5 rounded-full"
              style={{ backgroundColor: colors.semantic.warning + '1e', color: colors.semantic.warning }}
            >
              {rows.length} pending
            </span>
          </div>
          <div className="flex flex-col">
            {rows.map((d) => (
              <div
                key={d.id}
                className="flex items-center justify-between gap-3 py-2.5 border-t"
                style={{ borderColor: colors.utility.primaryText + '0d' }}
              >
                <div>
                  <div style={primaryText}>{d.member_name || '—'}</div>
                  <div className="text-xs" style={subText}>
                    {d.label || 'Due'} · {money(d.amount, d.currency)}
                    {d.upi_reference ? ` · UPI ${d.upi_reference}` : ''}
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button
                    ...
                    style={{ ...subText, borderColor: colors.utility.primaryText + '22' }}
                  >
                    Reject
                  </button>
                </div>
            ...
            >
              ...
              >
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    void openCheckin(s.block_id, s.name);
                  }}
                  className="text-xs font-medium inline-flex items-center gap-1"
                  style={{ color: colors.brand.primary }}
                >
  ...
  const SessionView = () => (
    <>
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <button
          onClick={() => void openCheckin(selectedSession?.block_id, selectedSession?.name)}
