...
export interface BlockLibraryMiniProps {
  ...
  // Hide group-session blocks from the library (direct-contract path: group
  // sessions may only be assigned via templates).
...
// A block is a Group Session if it carries audience='group' or the legacy
// 'session' category. Mirrors ServiceBlocksStep / the events engine.
const isGroupSessionBlock = (block: Block): boolean => {
  const aud = (block.meta as any)?.audience || (block.config as any)?.audience;
  return aud === 'group' || block.categoryId === 'session';
};
...
const BlockLibraryMini: React.FC<BlockLibraryMiniProps> = ({
  ...
  const allBlocks: Block[] = useMemo(() => {
    ...
    return hideGroupBlocks ? fanned.filter((b) => !isGroupSessionBlock(b)) : fanned;
  }, [blocksResponse, hideGroupBlocks]);
