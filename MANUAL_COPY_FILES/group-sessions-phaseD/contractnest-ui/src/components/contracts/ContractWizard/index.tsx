...
const ContractWizard: React.FC<ContractWizardProps> = ({
  ...
  // Group-session blocks already used by OTHER templates (rule: a group session
  // may belong to only one template). Excludes the template being edited.
  const usedGroupSessionBlockIds = React.useMemo(() => {
    const currentId = templateRecordId || editTemplate?.id || null;
    const set = new Set<string>();
    (templatesResponse?.data?.templates || []).forEach((t) => {
      if (t.id === currentId) return;
      (t.blocks || []).forEach((b) => {
        const co = (b.config_overrides || {}) as any;
        if (co?.config?.audience === 'group' || co?.category_id === 'session') set.add(b.block_id);
      });
    });
    return Array.from(set);
  }, [templatesResponse, templateRecordId, editTemplate]);
      ...
      case 'blocks': {
        ...
        return (
          <ServiceBlocksStep
            ...
            hideGroupBlocks={!isTemplateMode}
            usedGroupSessionBlockIds={usedGroupSessionBlockIds}
          />
        );
      }
      case 'billingView': {
