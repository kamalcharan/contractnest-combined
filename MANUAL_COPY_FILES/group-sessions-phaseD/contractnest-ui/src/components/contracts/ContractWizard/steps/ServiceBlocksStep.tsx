...
export interface ServiceBlocksStepProps {
  ...
  // Enforcement (group sessions): hide group blocks on the direct-contract path;
  // block group blocks already used by another template (uniqueness).
  hideGroupBlocks?: boolean;
  usedGroupSessionBlockIds?: string[];
}
...
const ServiceBlocksStep: React.FC<ServiceBlocksStepProps> = ({
  ...
  rfqMode = false,
  coverageTypes = [],
  hideGroupBlocks = false,
  usedGroupSessionBlockIds = [],
}) => {
  ...
  const handleAddBlock = useCallback(
    (block: Block) => {
      ...
      // A group session may live in only ONE template — reject if used elsewhere.
      const audience = (block.meta as any)?.audience || (block.config as any)?.audience;
      const isGroup = audience === 'group' || block.categoryId === 'session';
      if (isGroup && usedGroupSessionBlockIds.includes(block.id)) {
        addToast({
          type: 'warning',
          title: 'Already in another template',
          message: `"${block.name}" is a group session already assigned to a different template. A group session can belong to only one template.`,
        });
        return;
      }
    ...
    [selectedBlocks, blocksForActiveTab, onBlocksChange, addToast, currency, activeCoverageType, activeCoverageTabId, hasCoverageTypes, contractDuration, usedGroupSessionBlockIds]
  );
  ...
  const handleAddBlocks = useCallback(
    (blocks: Block[]) => {
      ...
      for (const block of blocks) {
        const audience = (block.meta as any)?.audience || (block.config as any)?.audience;
        const isGroup = audience === 'group' || block.categoryId === 'session';
        if (isGroup && usedGroupSessionBlockIds.includes(block.id)) { skippedGroup++; continue; }
      ...
      if (skippedGroup > 0) {
        addToast({ type: 'warning', title: 'Group session skipped',
          message: `${skippedGroup} group session${skippedGroup === 1 ? '' : 's'} already belong to another template and were not added.` });
      }
      if (toAdd.length === 0) {
      ...
      <div className="flex-1 flex gap-4 px-4 pb-6 min-h-0 overflow-hidden">
        ...
        <div className="w-[280px] flex-shrink-0" style={{ height: 'calc(100vh - 200px)' }}>
          <BlockLibraryMini
            ...
            hideGroupBlocks={hideGroupBlocks}
