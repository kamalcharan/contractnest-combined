...
/**
 * @route POST /api/contracts/:id/invoices/receipts/cancel
 * @description Cancel a single receipt (payment record) with a reason (seller-only)
 * @param {string} id - Contract UUID
 * @body {CancelReceiptRequest} - { receipt_id, reason? }
 * @returns {CancelReceiptResponse}
 */
router.post(
  '/:id/invoices/receipts/cancel',
  contractController.cancelReceipt
);
