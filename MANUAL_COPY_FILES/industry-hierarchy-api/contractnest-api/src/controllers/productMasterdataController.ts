...
class ProductMasterdataController {
  ...
  /**
   * Get industries with pagination, search, and hierarchy support
   * ✅ UPDATED: Added level and parent_id parameters for hierarchy filtering
   */
  getIndustries = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // ✅ UPDATED: Added level and parent_id extraction
      ...
      console.log(`📊 Industries request params - Page: ${parsedPage}, Limit: ${parsedLimit}, Search: "${sanitizedSearch}", Active: ${isActive}, Level: ${level}, ParentId: ${parent_id}`);

      // Call service with hierarchy parameters
   ...
   */
  getSubSegments = async (req: Request, res: Response): Promise<void> => {
    try {
      console.log('🔍 Product Master Data - Sub-segments requested');

      // Extract and validate parent_id
      const { parent_id, page, limit, search, is_active = 'true' } = req.query;
      ...
      console.log(`📊 Sub-segments request params - ParentId: ${parent_id}, Page: ${parsedPage}, Limit: ${parsedLimit}, Search: "${sanitizedSearch}", Active: ${isActive}`);

      // Call service
      const result = await this.productMasterdataService.getSubSegments(
        parent_id as string,
        parsedPage,
        parsedLimit,
        sanitizedSearch,
      ...
      console.log(`✅ Product Master Data - Sub-segments retrieved for ${parent_id} (${result.data?.length || 0} records)`);
      res.status(result.success ? 200 : 400).json({
        ...result,
        timestamp: new Date().toISOString()
      });
    } catch (error) {
      console.error('❌ Product Master Data - Sub-segments error:', error);

      res.status(500).json({
        success: false,
        error: 'Failed to get sub-segments',
        code: MasterDataErrorCodes.INTERNAL_ERROR,
        timestamp: new Date().toISOString()
      });
    }
