...
class ProductMasterdataService {
   ...
   * ✅ UPDATED: Added level and parentId parameters for hierarchy filtering
   */
  async getIndustries(
    page: number = 1,
    limit: number = 50,
    ...
    try {
      ...
      // ✅ NEW: Add hierarchy parameters if provided
  ...
  async getSubSegments(
    ...
    try {
      ...
      const url = `${this.edgeFunctionUrl}/sub-segments?${queryParams.toString()}`;

      console.log(`🔍 Fetching sub-segments for parent: ${parentId}, Page: ${page}, Limit: ${limit}, Search: "${search}"`);

      return await this.makeRequest('GET', url, null, userJWT) as IndustryResponse;
    } catch (error) {
      console.error('Error in getSubSegments:', error);
      return {
        success: false,
