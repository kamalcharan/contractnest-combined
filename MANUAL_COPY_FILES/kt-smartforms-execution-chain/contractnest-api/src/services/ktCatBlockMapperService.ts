...
export class KtCatBlockMapperService {
  ...
  async buildBlocksForTemplate(resourceTemplateId: string, authToken?: string): Promise<{
    ...
    const sb = this.clientFor(authToken);
    const [resourceTemplate, variants, serviceRows, spareRows, variantMap, serviceDefs, serviceFormMap] = await Promise.all([
      ...
      this.fetchServiceFormMap(sb, resourceTemplateId),
    ]);
    ...
    const { blocks: serviceBlocks, skipped: skippedServiceGroups } =
      this.buildServiceBlocks(serviceRows, variants, variantMap, serviceDefs, multipliers, resourceTemplateId, selectedResource, serviceFormMap);
  ...
  async syncBlocksForTemplate(
    ...
    authToken?: string,
  ): Promise<{ added: number; repriced: number; formsAttached: number }> {
    const sb = this.clientFor(authToken);

    const { blocks: freshBlocks } = await this.buildBlocksForTemplate(resourceTemplateId, authToken);
    if (freshBlocks.length === 0) return { added: 0, repriced: 0, formsAttached: 0 };
    ...
    for (const isLive of [false, true]) {
      const { data: existing, error } = await sb
        .from('m_cat_blocks')
        .select('id, name, base_price, config, form_template_id')
      ...
      // Backfill KT-generated Smart Form onto existing blocks that don't have
      // one yet -- independent of pricing/customization, so this runs even
      // for blocks the tenant already priced/edited.
      for (const row of existing || []) {
        if ((row as any).form_template_id) continue; // already attached, don't override a tenant's own choice
        const fresh = freshByName.get((row as any).name);
        if (!fresh?.form_template_id) continue;

        const { error: formUpdateError } = await sb
          .from('m_cat_blocks')
          .update({ form_template_id: fresh.form_template_id, updated_at: new Date().toISOString() })
          .eq('id', (row as any).id);

        if (formUpdateError) {
          console.error(`KtCatBlockMapper: form attach failed for block ${(row as any).id}:`, formUpdateError.message);
          continue;
        }
        formsAttached++;
      }
    ...
    return { added, repriced, formsAttached };
  }
  ...
  private buildServiceBlocks(
    ...
    serviceFormMap:   Map<string, string>,
    ...
    for (const [serviceName, groupRows] of groups) {
      ...
      blocks.push({
        ...
        // KT auto-generates one Smart Form per service group
        // (ktServiceFormGeneratorService → m_kt_service_form_map); attach it
        // here so the block is execution-ready the moment it's seeded,
        // instead of sitting unlinked in the admin Smart Forms list.
        form_template_id:     serviceFormMap.get(serviceName) || null,
  ...
  // service_name -> form_template_id, from the KT service-form generator
  // (ktServiceFormGeneratorService writes these into m_kt_service_form_map).
  private async fetchServiceFormMap(sb: SupabaseClient, resourceTemplateId: string): Promise<Map<string, string>> {
    const { data, error } = await sb
      .from('m_kt_service_form_map')
      .select('service_name, form_template_id')
      .eq('resource_template_id', resourceTemplateId);
    if (error) {
      console.error('KtCatBlockMapper: fetchServiceFormMap error', error.message);
      return new Map();
    }
    return new Map((data || []).map((r: any) => [r.service_name, r.form_template_id]));
  }
