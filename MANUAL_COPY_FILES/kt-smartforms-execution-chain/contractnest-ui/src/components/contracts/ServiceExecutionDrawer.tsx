...
const ServiceExecutionDrawer: React.FC<ServiceExecutionDrawerProps> = ({
  ...
  const primaryEvent = useMemo(
    () => deliverables.find((e) => e.event_type === 'service') || deliverables[0],
    [deliverables]
  );

  // ─── Smart-form schema for the form being filled ───
  const { data: activeFormTemplate, isLoading: loadingFormTemplate } = useQuery({
  ...
  const handleFormSubmit = useCallback(
    async (responses: Record<string, unknown>, computedValues: Record<string, unknown>) => {
      if (!activeForm || isSubmittingForm) return;
      setIsSubmittingForm(true);
      try {
        const submission = await createSubmission({
          form_template_id: activeForm.form_template_id,
          service_event_id: primaryEvent?.id || contractId,
    ...
    [activeForm, isSubmittingForm, createSubmission, updateSubmission, primaryEvent, contractId]
  );
          ...
          >
            ...
            <div className="p-4">
              {loadingFormTemplate ? (
                ...
                <FormRenderer
                  schema={activeFormTemplate.schema}
