...
const IndustrySelectionStep: React.FC = () => {
  ...
  // Served industries (pre-populate selections)
  const {
    servedIndustries,
  ...
  // Guard: useServedIndustries is enabled only when currentTenant?.id exists.
  // Without the guard, the query is disabled → isLoading=false immediately with empty
  // data → hydrated becomes true before the real fetch ever runs.
  useEffect(() => {
    if (!currentTenant?.id) return; // query not enabled yet — wait
    if (servedLoading) return;      // fetching — wait
  ...
  }, [currentTenant?.id, servedLoading, servedIndustries, hydrated]);
