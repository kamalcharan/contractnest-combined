...
// ── Mutation: Save Context Overlays (on-demand) ────────────────────────────────────
...
// ── Mutation: Delete Knowledge Tree ──────────────────────────────────────────────
...
// ── Mutation: Save Knowledge Tree (full payload) ────────────────────────────────────
...
// Each step: calls API to generate → calls edge function to save → invalidates queries
  ...
  return useMutation({
    mutationFn: async (payload: {
      ...
      if (!response.data?.success) {
        throw new Error(response.data?.error?.message || 'Variant generation failed');
      }
      // save_mode:'variants' ensures only variant rows are wiped — checkpoints/parts unaffected
...
export const useKTGenerateSpareParts = () => {
  ...
  return useMutation({
    mutationFn: async (payload: {
      ...
      if (!response.data?.success) {
        throw new Error(response.data?.error?.message || 'Spare parts generation failed');
      }
      // save_mode:'spare_parts' ensures only spare part rows are wiped — variants/checkpoints unaffected
...
export const useKTGenerateCheckpoints = () => {
  ...
  return useMutation({
    mutationFn: async (payload: {
      ...
      const { default: api } = await import('@/services/api');
      const response = await api.post('/api/knowledge-tree/generate-checkpoints', payload, { timeout: 180000 });
      if (!response.data?.success) {
        throw new Error(response.data?.error?.message || 'Checkpoint generation failed');
      }
      // save_mode:'checkpoints' ensures only checkpoint rows are wiped — variants/parts unaffected
      await postKnowledgeTreeEdge('save', { ...response.data.data, save_mode: 'checkpoints' });
      return response.data.data as { resource_template_id: string; checkpoints: any[]; service_cycles: any[] };
    },
...
// ── Hook: Generate Knowledge Tree via LLM (legacy — used for + Install / + Decomm) ──
