...
const CheckpointsTab: React.FC<Props> = ({ summary, colors }) => {
  ...
  const unitStyle: React.CSSProperties = {
    fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: colors.utility.secondaryText + '80',
  };

  return (
    <div>
      <VaNiBubble colors={colors}>
        <p><strong style={{ color: colors.utility.primaryText }}>{allCheckpoints.length} checkpoints</strong> — what the technician checks and records. Two types:{' '}
        ...
        <div key={cp.id} style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
            ...
            <span style={{
              ...
              background: cp.checkpoint_type === 'condition' ? colors.semantic.warning + '15' : (colors.semantic.info || '#2563eb') + '15',
              color: cp.checkpoint_type === 'condition' ? colors.semantic.warning : (colors.semantic.info || '#2563eb'),
            }}>
              {cp.checkpoint_type}
            </span>
          </div>
                ...
                {(cp.values || []).map((v: any) => {
                  ...
                  return (
                    <span key={v.id} style={{
                      ...
                      color: colors.utility.primaryText, display: 'flex', alignItems: 'center', gap: '4px',
                    }}>
              ...
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '6px', fontSize: '12px' }}>
                ...
                {cp.amber_threshold != null && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                    ...
                    <input defaultValue={cp.amber_threshold} style={{ ...inputStyle, borderColor: colors.semantic.warning }} />
                    <span style={unitStyle}>{cp.unit}</span>
                  </div>
                )}
                {cp.red_threshold != null && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                    ...
                    <input defaultValue={cp.red_threshold} style={{ ...inputStyle, borderColor: colors.semantic.error }} />
                    <span style={unitStyle}>{cp.unit}</span>
                  </div>
                )}
              </div>
