        ...
        {variants.map((v) => {
          ...
          return (
            <div key={v.id} style={{ background: sel ? brandPrimary + '06' : colors.utility.secondaryBackground, border: `1px solid ${sel ? brandPrimary : borderColor}`, borderRadius: '10px', padding: '13px 16px', display: 'flex', gap: '10px', cursor: 'pointer', transition: '.2s', position: 'relative' }}>
              ...
              {isCustom && (
                <button onClick={(e) => { e.stopPropagation(); onRemove(v.id); }} style={{ position: 'absolute', top: '8px', right: '8px', width: '24px', height: '24px', borderRadius: '6px', border: 'none', background: colors.semantic.error + '10', color: colors.semantic.error, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title="Remove variant">
