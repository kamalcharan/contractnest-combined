    ...
    .custom((channels) => {
      ...
      for (let i = 0; i < channels.length; i++) {
        ...
        if (['mobile', 'phone', 'whatsapp'].includes(channel.channel_type)) {
          ...
          // Country code is required for phone channels
          if (!channel.country_code) {
            throw new Error(`Channel ${i + 1}: Country code is required for ${channel.channel_type}`);
          }

          // Validate phone length based on country
          const phoneValidation = validatePhoneLengthForCountry(digitsOnly, channel.country_code);
