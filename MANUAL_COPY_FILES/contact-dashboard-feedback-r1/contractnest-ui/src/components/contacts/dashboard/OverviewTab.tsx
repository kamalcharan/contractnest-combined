...
// Overview tab: Health Ring + Pillars + Needs Attention + Quick Financials + Recent Activity
    ...
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex gap-6 items-start">
        ...
        <div className="flex-1">
          <HealthPillars cockpitData={cockpitData} colors={colors} />

          {/* Quick Stats Row */}
          <div className="grid grid-cols-3 gap-3 mt-3">
            <div
              className="p-3 rounded-xl border text-center cursor-pointer transition-colors hover:opacity-80"
              style={{
                backgroundColor: colors.utility.secondaryBackground,
                borderColor: colors.utility.primaryText + '10',
              }}
            ...
            >
              <div className="text-lg font-bold" style={{ color: colors.brand.primary }}>
                {cockpitData.contracts?.by_status
                  ? Object.values(cockpitData.contracts.by_status).reduce((s: number, c) => s + (c as number), 0)
                  : (cockpitData.contracts?.total || 0)}
              </div>
              <div className="text-xs" style={{ color: colors.utility.secondaryText }}>Contracts</div>
            </div>
            <div
              className="p-3 rounded-xl border text-center cursor-pointer transition-colors hover:opacity-80"
              style={{
            ...
            >
              <div className="text-lg font-bold" style={{ color: '#3b82f6' }}>
                {cockpitData.events?.total || 0}
              </div>
              <div className="text-xs" style={{ color: colors.utility.secondaryText }}>Events</div>
            </div>
            <div
              className="p-3 rounded-xl border text-center cursor-pointer transition-colors hover:opacity-80"
              style={{
            ...
            >
              <div className="text-lg font-bold" style={{ color: '#06b6d4' }}>
                {cockpitData.invoices?.length || 0}
              </div>
              <div className="text-xs" style={{ color: colors.utility.secondaryText }}>Invoices</div>
            </div>
          </div>
        </div>
      </div>
