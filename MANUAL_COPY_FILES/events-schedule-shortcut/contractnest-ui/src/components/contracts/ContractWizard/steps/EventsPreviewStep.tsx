...
export interface EventsPreviewStepProps {
  ...
  /** Template mode: schedule is illustrative — dates are not editable/persisted */
  readOnly?: boolean;
}
    ...
    return (
      <div key={event.id} className="w-full max-w-[272px]">
        ...
        <div className="flex items-center gap-1 mt-2">
          {readOnly ? (
            <span
              ...
              style={{
                ...
                border: `1px dashed ${isDarkMode ? colors.utility.secondaryBackground : '#E5E7EB'}`,
              }}
            >
              <CalendarDays className="w-3.5 h-3.5 flex-shrink-0" style={{ color: colors.brand.secondary }} />
          ...
          {isOverridden && !isEditing && !readOnly && (
            <button
          ...
          >
            ...
            <div>
              <p className="text-[11px] font-semibold mb-0.5" style={{ color: colors.brand.secondary }}>
                {readOnly ? 'Illustrative schedule' : 'Dates are editable'}
              </p>
              <p className="text-[10px] leading-relaxed" style={{ color: colors.utility.secondaryText }}>
                {readOnly
                  ? 'This preview assumes a start today. Real dates are set on each contract created from this template.'
                  : 'Click any date on the timeline cards to adjust the schedule. Changed dates are highlighted and can be reset anytime.'}
              </p>
            </div>
          </div>
        </div>
