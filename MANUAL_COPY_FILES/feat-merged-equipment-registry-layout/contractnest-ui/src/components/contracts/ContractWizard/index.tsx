...
// Contract flow — full static list; assetSelection is conditionally filtered at runtime
// Order: path → nomenclature → acceptance → counterparty → details → [assets] → billingCycle → blocks → billingView → evidencePolicy → events → review
const CONTRACT_STEPS_ALL: StepConfig[] = [
...
function mapWizardToRequest(
  ...
  const blocks = state.selectedBlocks.map((block, idx) => {
    // Resource-based blocks have composite IDs (uuid__res__resourceId)
    // Use originalBlockId to get the actual catalog block UUID
    const catalogBlockId = block.originalBlockId || block.id;
    const isFlyBy = !isValidUUID(catalogBlockId);
    return {
      position: idx,
      source_type: isFlyBy ? 'flyby' : 'catalog',
      source_block_id: isFlyBy ? undefined : catalogBlockId,
      ...
      custom_fields: {
        ...
        // Resource-based block metadata
        resourceId: block.resourceId || undefined,
        resourceName: block.resourceName || undefined,
      },
    };
  });
...
const ContractWizard: React.FC<ContractWizardProps> = ({
  ...
  // Safety: clamp currentStep if step array shrank (e.g., asset step removed mid-wizard)
  const safeCurrentStep = Math.min(currentStep, totalSteps - 1);

  // Get current step ID
  const currentStepId = activeSteps[safeCurrentStep]?.id || 'path';
  ...
  const canGoNext = useCallback((): boolean => {
    ...
    switch (currentStepId) {
      ...
      case 'billingView':
        return true;
      case 'assetSelection':
        return true; // Asset selection is optional — seller can skip (buyer adds later)
      case 'evidencePolicy':
