...
const ContractWizard: React.FC<ContractWizardProps> = ({
  ...
  const handleNext = useCallback(() => {
    if (isLastStep) {
      if (wizardState.acceptanceMethod === 'signoff') {
        // Show animated confirmation screen — onComplete deferred to Done button
  ...
  // Checked before !isOpen so the success animation persists
  // even if parent sets isOpen=false after onComplete.
  if (isContractSent) {
    return (
