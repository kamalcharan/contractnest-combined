...
const SubscriptionPage: React.FC = () => {
  ...
  // ── B8 ────────────────────────────────────────────────────────────────
  //
  // The countdown used to be `term.left` — days to the CONTRACT END DATE. On
  // a quarterly plan sold on a 12-month contract that reads "365 days left",
  // which is true about the contract and useless about the money: the next
  // ₹5,999 is due in 3 months, not in a year. The rhythm comes from the
  // plan's BILLING EVENTS (get_subscription_billing_rhythm), so whatever
  // cadence a plan is authored with, the page counts to that plan's own next
  // payment without knowing anything about cadences itself.
  const rhythm = ctx?.subscription?.rhythm ?? null;

  // Nothing has been paid and the contract is still gated. The plan is NOT
  // live — which is why every allowance on the page reads 0. Saying "your
  // plan" over a full term countdown while the allowances are all zero is
  // the specific contradiction this state removes.
  const awaitingPayment = !!rhythm?.awaiting_first_payment;

  // Prefer the rhythm; fall back to the term only when there is no billing
  // schedule at all (a free plan, or an API build predating migration 037).
  const countdown = useMemo(() => {
    if (rhythm && rhythm.source !== 'none' && typeof rhythm.days_to_next === 'number') {
      const d = rhythm.days_to_next;
      return {
        value: Math.abs(d),
        overdue: !!rhythm.is_overdue,
        label: rhythm.is_overdue
          ? (Math.abs(d) === 1 ? 'day overdue' : 'days overdue')
          : (d === 0 ? 'due today' : d === 1 ? 'day to next payment' : 'days to next payment'),
        showNumber: !(d === 0 && !rhythm.is_overdue),
        fromRhythm: true,
      };
    }
    if (term) {
      return { value: term.left, overdue: false, label: 'days left', showNumber: true, fromRhythm: false };
    }
    return null;
  }, [rhythm, term]);

  // Progress across the plan's INSTALMENTS, not across elapsed calendar
  // time. "2 of 4 paid" is a fact about the money; a term bar is not.
  const paidPct = rhythm && rhythm.total_installments > 0
    ? Math.min(100, (rhythm.paid_installments / rhythm.total_installments) * 100)
    : (term?.elapsedPct ?? 0);
  ...
  return (
    <div className="p-6 max-w-5xl">
      ...
      <div
        style={{
          ...surface,
          borderColor: onPlan ? (awaitingPayment ? `${warn}55` : `${ok}55`) : `${ink}18`,
        }}
        className="p-6 mb-5"
      >
        {onPlan ? (
          <>
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="text-sm mt-1" style={{ color: dim }}>
                  ...
                  {/* What the plan bills, and how often — the term total on
                      its own made a 4 × ₹5,999 plan look like one ₹23,996
                      charge, which is exactly what it was mistaken for. */}
                  {rhythm && rhythm.total_installments > 1 && rhythm.next_due_amount != null && (
                    <> {`(${rhythm.total_installments} × ${symbol}${Number(rhythm.next_due_amount).toLocaleString()}${rhythm.cycle ? `, ${rhythm.cycle}` : ''})`}</>
                  )}
              ...
              {/* The number that actually matters day to day. While the first
                  payment is outstanding there is no countdown to show — the
                  plan has not started — so the amount owed takes the slot. */}
              {awaitingPayment ? (
                <div className="text-right">
                  <div className="text-4xl font-extrabold leading-none" style={{ color: warn }}>
                    {symbol}{Number(rhythm?.next_due_amount ?? sub.amount ?? 0).toLocaleString()}
                  </div>
                  <div className="text-xs font-semibold uppercase tracking-wider mt-1" style={{ color: dim }}>
                    due to activate
                  </div>
                </div>
              ) : countdown && countdown.showNumber ? (
                <div className="text-right">
                  <div
                    className="text-4xl font-extrabold leading-none"
                    style={{ color: countdown.overdue || countdown.value <= 7 ? warn : ink }}
                  >
                    {countdown.value}
                  </div>
                  <div className="text-xs font-semibold uppercase tracking-wider mt-1" style={{ color: dim }}>
                ...
                <div className="text-right">
                  <div className="text-2xl font-extrabold leading-none" style={{ color: warn }}>
                    Due today
                  </div>
                </div>
              ) : null}
            ...
            {/* Nothing is live until the first payment clears — said plainly,
                because otherwise the zeros in the allowance cards below read
                as "your plan gives you nothing" rather than "your plan has
                not started". */}
            {awaitingPayment && (
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3 p-3 rounded-xl"
                ...
                <div className="flex items-start gap-2 text-sm" style={{ color: ink }}>
                  ...
                  <span>
                    ...
                    <span style={{ color: dim }}>
                      {' '}Nothing below is active yet — that is why the counts read zero.
                    </span>
                  </span>
                </div>
                <button
                ...
                >
                  Complete payment <ArrowUpRight className="w-4 h-4" />
                </button>
              </div>
            )}

            {!awaitingPayment && (rhythm?.total_installments ?? 0) > 0 && (
              <div className="mt-4">
                {/* Progress through the INSTALMENTS, not through elapsed
                    calendar time. */}
                <div className="h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: `${ink}12` }}>
                  <div className="h-full rounded-full transition-all"
                       style={{ width: `${paidPct}%`, backgroundColor: countdown?.overdue ? warn : ok }} />
                </div>
                <p className="text-xs mt-2 flex flex-wrap items-center gap-x-3 gap-y-1" style={{ color: dim }}>
                  <span className="flex items-center gap-1.5">
                    <CalendarClock className="w-3.5 h-3.5" />
                    {countdown?.overdue ? 'Was due ' : 'Next payment '}
                    {formatDate(rhythm?.next_due_date ?? null)}
                    {rhythm?.next_due_amount != null && ` · ${symbol}${Number(rhythm.next_due_amount).toLocaleString()}`}
                  </span>
                  {rhythm && rhythm.total_installments > 1 && (
                    <span>{rhythm.paid_installments} of {rhythm.total_installments} paid</span>
                  )}
                </p>
              </div>
            )}

            {/* Free plan, or an API build with no rhythm yet: the contract
                term is all there is to show, and for a free plan it is also
                the right thing to show. */}
            {!awaitingPayment && (rhythm?.total_installments ?? 0) === 0 && term && (
              <div className="mt-4">
                <div className="h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: `${ink}12` }}>
                  <div className="h-full rounded-full transition-all"
