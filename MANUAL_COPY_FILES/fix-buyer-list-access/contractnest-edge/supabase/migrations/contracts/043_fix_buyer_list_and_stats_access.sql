-- ============================================================================
-- Migration: contracts/043_fix_buyer_list_and_stats_access.sql
-- Purpose: Fix get_contracts_list and get_contract_stats to include contracts
--          accessible to the requesting tenant via t_contract_access (buyer/vendor/partner).
--
-- Root Cause:
--   Both functions only check `c.tenant_id = p_tenant_id`, meaning a buyer
--   (whose tenant_id differs from the seller/owner) sees ZERO contracts.
--   The get_contract_by_id function already handles this correctly via
--   t_contract_access fallback (since migration 031), but the list & stats
--   functions were never updated.
--
-- Fix:
--   Use a CTE or subquery that unions:
--     (a) Contracts where c.tenant_id = p_tenant_id  (owner/seller path)
--     (b) Contracts where t_contract_access.accessor_tenant_id = p_tenant_id  (buyer path)
--
-- Depends on: 042_seller_buyer_tenant_columns.sql
-- ============================================================================
...
BEGIN
    ...
    --   Owner path: c.tenant_id = p_tenant_id
    --   Accessor path: EXISTS in t_contract_access
    -- ═══════════════════════════════════════════
    v_where := format(
...
BEGIN
    ...
    --   Include both owned contracts AND accessor-granted contracts
    -- ═══════════════════════════════════════════
    SELECT COALESCE(
