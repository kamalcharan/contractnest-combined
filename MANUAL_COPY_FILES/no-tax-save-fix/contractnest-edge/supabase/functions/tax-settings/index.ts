...
async function handleCreateUpdateSettings(
  ...
  try {
    ...
    // Validate input
    if (!requestData?.display_mode || !['including_tax', 'excluding_tax', 'no_tax'].includes(requestData.display_mode)) {
      return new Response(
        JSON.stringify({ error: 'Invalid display_mode. Must be "including_tax", "excluding_tax", or "no_tax"' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
