...
| B4 | Extend — WhatsApp/email touchpoints (T1→T2→T3) | 🔶 **B4.2 DONE (live) + B4.1 DRAFTED 2026-09-12** — owner directive: notifications-first, "close coordination with /integrations, /automation rules, /admin/jtd/templates". Migration 013 live: 5 source types + 5 platform n_jtd_templates (ordered `variables` → worker's GENERIC positional path, **no worker deploy needed**) + 3 new automation rules (report link base tenant-editable via rule config) + 4 enqueue functions + 3 triggers (exception-swallowing — notifications never break the transaction). Harness ALL GREEN incl. proof that a tenant's live rule switch-off is honored (signia has notif_service_completed OFF — respected). **Templates REGISTERED + Meta-APPROVED 2026-09-12** (after a copy rework — Meta forbids leading/trailing variables, 013b). **LIVE SEND FIRED**: signia's whatsapp channel was off in n_jtd_tenant_config (worker refused — the /integrations gate working) → enabled; service_visit_started for test ticket TKT-10002 dispatched to the owner-approved number, status 'sent' w/ provider id. 013c fixed V2 event_type 'service_visit' in the service-name lookup (test-caught). **HANDSET-CONFIRMED 2026-09-12 (owner: "i got message") — B4 notification part CLOSED.** B4.3 (delivery reports inbound) + B4.4 (Flows POC) parked until after 19 Sep. **B4 notification part: ✅ CLOSED** |
...
3. **Track A** — soak until **19 Sep BBB meeting** → post-meeting verification
   report → Phase 5 flip discussion → Phase 6 retirement. OPEN owner call:
   4 stale pending declarations (₹15,000) — reject before the 19th?
4. **UX review of the execution surface** — parked at owner request
   ("UX is bad, we will review UX later"): drawer, form modal, asset chips,
   plus offers on the table (tap-asset→form shortcut, upload UI in form
   modal, D10 mobile polish).
5. **B4.3 + B4.4** — delivery reports inbound (n8n forward or second MSG91
   destination) and the WhatsApp Flows POC; parked until after 19 Sep.
6. **Backlog (unscheduled)**: BBB 25-vs-26 meetings arithmetic; guest
   payments orphaned (waiting contract-less invoices); duplicate-declaration
   member alert; billing cadence calendar drift (engine-wide); IST hardcode;
   RFQ Award → vendor-created contract (5-point spec); standalone Create
   Invoice page; Block Wizard timing-in-description duplication.
