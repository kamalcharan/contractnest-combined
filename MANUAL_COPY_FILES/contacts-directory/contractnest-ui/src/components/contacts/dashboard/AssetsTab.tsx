...
// Reuses the Equipment Registry's EquipmentCard (owner call, 2026-09-16 —
// it already accepts ClientAsset via its CardAsset union, so registry and
// contact assets read as one family) and ClientAssetFormDialog
        ...
        <div className={viewMode === 'grid' ? 'grid grid-cols-3 gap-4' : 'space-y-3'}>
          {filteredAssets.map((asset) => (
            <EquipmentCard
              key={asset.id}
              asset={asset}
              onEdit={(a) => { setEditingAsset(a as ClientAsset); setIsEditOpen(true); }}
              onDelete={async (a) => {
