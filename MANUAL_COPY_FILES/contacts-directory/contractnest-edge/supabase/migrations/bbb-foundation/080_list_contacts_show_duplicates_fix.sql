-- 080_list_contacts_show_duplicates_fix.sql
-- ALREADY APPLIED LIVE (2026-09-16) — this file is a source-of-record copy.
-- DO NOT RE-RUN.
--
-- list_contacts_with_channels_v2 has accepted p_show_duplicates since it
-- was written but never referenced it in the query body — a silent
-- regression from the RPC it replaced (list_contacts_filtered DOES filter
-- on it correctly). This is the exact same "accepted, never read" bug
-- pattern as 078's p_contact_persons fix, just on a different RPC. The
-- already-shipped "Possible duplicates" checkbox on /contacts called this
-- with show_duplicates=true and always got back the full unfiltered list.
-- Now wired to 079's get_tenant_duplicate_contact_ids().
CREATE OR REPLACE FUNCTION public.list_contacts_with_channels_v2(p_tenant_id uuid, p_is_live boolean DEFAULT true, p_page integer DEFAULT 1, p_limit integer DEFAULT 20, p_type text DEFAULT NULL::text, p_status text DEFAULT NULL::text, p_search text DEFAULT NULL::text, p_classifications text[] DEFAULT NULL::text[], p_user_status text DEFAULT NULL::text, p_show_duplicates boolean DEFAULT false, p_include_inactive boolean DEFAULT false, p_include_archived boolean DEFAULT false, p_sort_by text DEFAULT 'created_at'::text, p_sort_order text DEFAULT 'desc'::text, p_tags text[] DEFAULT NULL::text[])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WHERE c.tenant_id = p_tenant_id
    ...
    -- 077: children join the population only under a tag filter
    AND (c.parent_contact_id IS NULL OR p_tags IS NOT NULL)
    ...
    AND (
      ...
      OR c.id IN (SELECT contact_id FROM get_tenant_duplicate_contact_ids(p_tenant_id, p_is_live))
    );
