...
export interface MenuItem {
  ...
  /** Render only while the Revenue perspective is active. An invoice is a
   *  seller's instrument — on the expense side, received invoices are
   *  payables and live in Finance/Subscription, so the workspace nav hides.
...
export const defaultMenuItems: MenuItem[] = [
{
  ...
  */
  ...
  // Invoices — the money document, ranked beside the contract document.
  // Revenue-only: sellers issue invoices; buyers meet theirs as payables in
  // Finance (AR/AP) and plan invoices under Subscription. Single item on
  // purpose — verbs (New invoice) are buttons on the hub, not nav rows, and
  // future rows (Receipts, Credit notes) join the section when they exist.
  {
    id: 'invoices',
    label: 'Invoices',
    icon: 'Receipt',
