...
import type { EquipmentDetailItem, CoverageTypeItem } from './AssetSelectionStep';
...
export interface ReviewSendStepProps {
  ...
  // Equipment addendum (from AssetSelectionStep)
    ...
    <div className="h-full flex flex-col" style={{ backgroundColor: canvasBg }}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        >
          ...
          {/* ═══ PAGE 2: EQUIPMENT ADDENDUM ═══ */}
          {(coverageTypes.length > 0 || equipmentDetails.length > 0) && (
            <>
              {/* Page break separator */}
              <div
                className="mx-12 my-0 border-t-2 border-dashed"
              ...
              <div className="px-12 py-10">
                ...
                <div className="flex items-center gap-3 mb-8">
                  <div
                    className="w-10 h-10 rounded-xl flex items-center justify-center"
                    style={{ background: `linear-gradient(135deg, ${brandSecondary}20, ${brandSecondary}08)` }}
                  >
                    <Wrench className="w-5 h-5" style={{ color: brandSecondary }} />
                  </div>
                  <div>
                    <h2
                      ...
                      style={{ color: colors.utility.primaryText }}
                    >
                      Equipment & Coverage Addendum
                    </h2>
                    <p className="text-xs mt-0.5" style={{ color: colors.utility.secondaryText }}>
                      Equipment types and assets covered under this agreement
                    </p>
                  </div>
                  <span
                    className="ml-auto text-[9px] px-2.5 py-1 rounded-full font-semibold uppercase"
                    style={{ backgroundColor: `${brandSecondary}10`, color: brandSecondary }}
                  >
                    Addendum
                  </span>
                </div>
                ...
                {coverageTypes.length > 0 && (
                  <div className="mb-8">
                    ...
                    >
                      ...
                      Equipment Types Covered ({coverageTypes.length})
                    </h3>
                    <div
                    ...
                    >
                      {/* Table header */}
                      <div
                        className="grid grid-cols-[1fr_1fr_auto] gap-4 px-4 py-2.5"
                        style={{ backgroundColor: `${colors.utility.primaryText}04` }}
                      >
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-wider text-right" style={{ color: colors.utility.secondaryText }}>
                          Assets
                        </span>
                      </div>
                      {/* Table rows */}
                      {coverageTypes.map((ct, idx) => {
                        ...
                        const CatIcon = scConfig?.icon || Package;
                        const catColor = scConfig?.color || '#6B7280';
                        // Count equipment items linked to this coverage type's resource
                        const linkedCount = equipmentDetails.filter(
                          (eq) => eq.category_name === ct.resource_name || eq.category_id === ct.resource_id
                        ).length;
                        ...
                        return (
                          <div
                            ...
                            style={{
                              borderTop: idx > 0 ? `1px solid ${borderColor}` : undefined,
                            }}
                          >
                            <div className="flex items-center gap-2.5">
                              <div
                            ...
                            <span className="text-xs font-bold text-right" style={{ color: colors.utility.primaryText }}>
                ...
                {equipmentDetails.length > 0 && (
                  <div>
                    ...
                    >
                      ...
                      Equipment Inventory ({equipmentDetails.length} item{equipmentDetails.length !== 1 ? 's' : ''})
                    </h3>

                    {/* Equipment cards — grouped by category */}
                    {(() => {
                      // Group by category_name
                      const groups: Record<string, EquipmentDetailItem[]> = {};
                      equipmentDetails.forEach((eq) => {
                        const cat = eq.category_name || 'Uncategorised';
                      ...
                      return Object.entries(groups).map(([catName, items]) => (
                        <div key={catName} className="mb-5 last:mb-0">
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            <Tag className="w-3 h-3" style={{ color: brandSecondary }} />
                            <span className="text-[11px] font-bold" style={{ color: colors.utility.primaryText }}>
                              {catName}
                            </span>
                            <span className="text-[10px]" style={{ color: colors.utility.secondaryText }}>
                              ({items.length})
                          ...
                          >
                            {items.map((eq, eIdx) => (
                              <div
                                ...
                                className="px-4 py-3"
                                style={{
                                  borderTop: eIdx > 0 ? `1px solid ${borderColor}` : undefined,
                                }}
                              >
                                {/* Row 1: Name + Serial + Qty */}
                                <div className="flex items-center gap-3 mb-1.5">
                                  ...
                                  {eq.serial_number && (
                                    ...
                                    >
                                      <Hash className="w-2.5 h-2.5 inline mr-0.5" style={{ verticalAlign: 'text-bottom' }} />
                                      {eq.serial_number}
                                    </span>
                                  )}
                                  {eq.quantity > 1 && (
                                  ...
                                  {eq.condition && (
                                    <span
                                      className="ml-auto text-[9px] px-2 py-0.5 rounded-full font-semibold uppercase"
                                      style={{
                                        backgroundColor: eq.condition === 'good' ? `${colors.semantic.success}10`
                                          : eq.condition === 'fair' ? `${colors.semantic.warning}10`
                                          : `${colors.semantic.error}10`,
                                        color: eq.condition === 'good' ? colors.semantic.success
                                          : eq.condition === 'fair' ? (colors.semantic.warning || '#F59E0B')
                                ...
                                {/* Row 2: Meta details (Make, Model, Location) */}
                                <div className="flex flex-wrap gap-x-4 gap-y-1">
                                  ...
                                  {eq.criticality && (
                                    <span className="text-[10px]" style={{ color: colors.utility.secondaryText }}>
                                      <strong>Criticality:</strong> {eq.criticality}
                                    </span>
                                  )}
                                  {eq.area_sqft && (
                                    <span className="text-[10px]" style={{ color: colors.utility.secondaryText }}>
                                      <strong>Area:</strong> {eq.area_sqft} sq ft
                                    </span>
                                  )}
                                  {eq.warranty_expiry && (
                                    <span className="text-[10px]" style={{ color: colors.utility.secondaryText }}>
                                      <strong>Warranty:</strong> {eq.warranty_expiry}
                                    </span>
                                  )}
                                </div>
                                ...
                                {/* Row 3: Notes (if any) */}
                                {eq.notes && (
                                  <p className="text-[10px] mt-1.5 italic" style={{ color: colors.utility.secondaryText }}>
                                    <Info className="w-2.5 h-2.5 inline mr-1" style={{ verticalAlign: 'text-bottom' }} />
                                    {eq.notes}
                                  </p>
                                )}
                              </div>
                ...
                >
                  <span className="text-[10px]" style={{ color: colors.utility.secondaryText }}>
                    This addendum is an integral part of the service agreement referenced above.
                  </span>
