...
import { Plus, Search, X, Layers, Package, UserPlus } from 'lucide-react';
...
const AssetSelectionStep: React.FC<AssetSelectionStepProps> = ({
  ...
  const handleToggle = useCallback(
    ...
    [selectedRegistryIds, equipmentDetails, onEquipmentDetailsChange, buildDetailFromAsset]
  );
  ...
  const handleCreateSubmit = useCallback(
    ...
    [contactId, config, createMutation, filteredResources, currentTenant?.id, equipmentDetails, onEquipmentDetailsChange]
  );
    ...
    <div className="flex flex-col h-full overflow-hidden" style={{ minHeight: 480 }}>
      ...
      >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {/* Let Buyer Add toggle */}
          <button
            type="button"
            onClick={() => onAllowBuyerToAddChange(!allowBuyerToAdd)}
            className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold transition-all border"
            style={{
              borderColor: allowBuyerToAdd ? colors.brand.primary : colors.utility.primaryText + '20',
              backgroundColor: allowBuyerToAdd ? colors.brand.primary + '10' : 'transparent',
              color: allowBuyerToAdd ? colors.brand.primary : colors.utility.secondaryText,
            }}
          >
            <UserPlus size={14} />
          ...
          {assetsLoading ? (
            ...
            <div className="text-center py-16 px-6">
              ...
              <p className="text-sm max-w-md mx-auto mb-7 leading-relaxed" style={{ color: colors.utility.secondaryText }}>
                Add {config.label.toLowerCase()} to track what this contract covers. You can also let the buyer add their own.
              </p>
              <Button
                onClick={() => setIsDrawerOpen(true)}
