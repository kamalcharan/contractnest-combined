...
interface FlyByLine {
  ...
  name: string;
  cycle?: string;         // service only — how often (monthly … onetime)
...
const RfqBuilderPage: React.FC = () => {
  ...
  const canAdvance = useMemo(() => {
    switch (stepId) {
      ...
      case 'type': return nomenclatureTypes.length === 0 ? true : nomenclatureId !== null;
  ...
  }, [stepId, assetKind, nomenclatureId, nomenclatureTypes, name, durationValue, services, vendors]);
  ...
  const addFlyby = (type: FlyByBlockType) =>
    setServices((prev) => [...prev, {
      ...
      type,
      name: '',
      cycle: type === 'service' ? 'monthly' : undefined,
        ...
        {stepId === 'type' && (
          <>
                ...
                {nomenclatureTypes.map((n: any) => {
                  ...
                  return (
                    <button key={n.id} onClick={() => {
                      ...
                      title={n.description}
                      style={{ fontSize: 14, fontWeight: 600, padding: '10px 16px', borderRadius: 12, cursor: 'pointer',
                        border: `1.5px solid ${on ? brand : ink + '22'}`, background: on ? brand : surface, color: on ? '#fff' : ink }}>
            ...
            {selectedNom?.description && (
              <p style={{ fontSize: 12.5, color: sub, margin: '14px 2px 0' }}>{selectedNom.description}</p>
            )}
          </>
        )}
              ...
              {services.map((s) => (
                <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: `1px solid ${line}`, fontSize: 13.5 }}>
                  ...
                  <span style={{ color: sub, fontFamily: 'monospace' }}>
                    {s.type === 'service' ? CYCLES.find((x) => x.id === s.cycle)?.label : s.type === 'spare' ? `×${s.quantity ?? 1}` : s.type === 'document' ? (s.fileType || '').toUpperCase() : ''}
                  </span>
                </div>
              ))}
