...
import { X, CheckCircle2, ArrowRight, Loader2, Copy, Check, Key, Mail, CreditCard, PenTool, Zap, Receipt, Monitor, MessageSquare, Link2, Globe, Wallet } from 'lucide-react';
...
import { useCreateOrder, useCreateLink, type CreateOrderResponse } from '@/hooks/queries/usePaymentGatewayQueries';
...
const ContractWizard: React.FC<ContractWizardProps> = ({
  ...
  const { hasActiveGateway, providerDisplayName } = useGatewayStatus();
  ...
  type OnlineCollectionMode = 'terminal' | 'email_link' | 'whatsapp_link';
  const [paymentChannel, setPaymentChannel] = useState<PaymentChannel>('offline');
  const [onlineCollectionMode, setOnlineCollectionMode] = useState<OnlineCollectionMode>('terminal');
  const [onlineCustomerName, setOnlineCustomerName] = useState('');
  const [onlineCustomerEmail, setOnlineCustomerEmail] = useState('');
  const [onlineCustomerPhone, setOnlineCustomerPhone] = useState('');
  const [onlineLinkResult, setOnlineLinkResult] = useState<{ url: string; expiresAt: string | null } | null>(null);
  ...
  const resetWizard = useCallback(() => {
    ...
    // Online payment resets
    setPaymentChannel(hasActiveGateway ? 'online' : 'offline');
    setOnlineCollectionMode('terminal');
    setOnlineCustomerName('');
    setOnlineCustomerEmail('');
    setOnlineCustomerPhone('');
    setOnlineLinkResult(null);
    setLinkCopied(false);
  }, [hasActiveGateway]);
  ...
  const handleNext = useCallback(async () => {
    if (isLastStep) {
      ...
      if (wizardState.acceptanceMethod === 'auto') {
        ...
        setPaymentNotes('');
        setPaymentEmiSequence(1);
        // Default to online if gateway is active
        setPaymentChannel(hasActiveGateway ? 'online' : 'offline');
        setOnlineCollectionMode('terminal');
        setOnlineCustomerName(wizardState.buyerName || '');
        setOnlineCustomerEmail('');
        setOnlineCustomerPhone('');
        setOnlineLinkResult(null);
        setLinkCopied(false);
  ...
  // Create contract + initiate online terminal payment (Razorpay popup)
  const handleCreateWithOnlineTerminal = useCallback(async () => {
    let contractResult: Record<string, any> | null = null;
    try {
      setIsProcessingPayment(true);
      ...
      // Step 4: Close the dialog and open Razorpay popup
      setShowPrePaymentDialog(false);
      setIsProcessingPayment(false);
      setProcessingStep('');
      ...
      // Open Razorpay Checkout — the hook handles verification and callbacks
      openCheckout(orderData as CreateOrderResponse);
    } catch (err: any) {
      if (contractResult) {
        setCreatedContractData(contractResult);
        setShowPrePaymentDialog(false);
        ...
        addToast({
          ...
          message: err.message || 'You can collect payment from the contract details page.',
        });
      } else {
        addToast({
          type: 'error',
  ...
  }, [wizardState, contractType, createContract, paymentAmount, addToast, openCheckout]);

  // Create contract + create payment link (email/WhatsApp)
  const handleCreateWithOnlineLink = useCallback(async () => {
    let contractResult: Record<string, any> | null = null;
    try {
      setIsProcessingPayment(true);

      // Validate customer details
      if (onlineCollectionMode === 'email_link' && !onlineCustomerEmail) {
        addToast({ type: 'error', title: 'Email required', message: 'Enter the buyer\'s email to send the payment link.' });
        setIsProcessingPayment(false);
        return;
      }
      if (onlineCollectionMode === 'whatsapp_link' && !onlineCustomerPhone) {
        addToast({ type: 'error', title: 'Phone required', message: 'Enter the buyer\'s phone number for WhatsApp link.' });
        setIsProcessingPayment(false);
        return;
      }
      ...
      setProcessingStep('Creating payment link...');
      const linkResponse = await api.post(API_ENDPOINTS.PAYMENTS.CREATE_LINK, {
        invoice_id: invoice.id,
        amount: parseFloat(paymentAmount),
        currency: wizardState.currency || 'INR',
        collection_mode: onlineCollectionMode,
        customer: {
          name: onlineCustomerName || wizardState.buyerName || undefined,
          email: onlineCustomerEmail || undefined,
          contact: onlineCustomerPhone || undefined,
        },
        expire_hours: 72,
      });
      const linkData = linkResponse.data?.data || linkResponse.data;
      ...
      // Show link result
      setOnlineLinkResult({
        url: linkData.gateway_short_url,
      ...
      addToast({
        ...
        message: `Link sent via ${onlineCollectionMode === 'email_link' ? 'email' : 'WhatsApp'}`,
      });
    } catch (err: any) {
      if (contractResult) {
        setCreatedContractData(contractResult);
        ...
        addToast({
          type: 'warning',
          title: 'Contract created, link creation failed',
          message: err.message || 'You can send a payment link from the contract details page.',
        });
      } else {
        addToast({
          type: 'error',
  ...
  }, [wizardState, contractType, createContract, paymentAmount, onlineCollectionMode, onlineCustomerName, onlineCustomerEmail, onlineCustomerPhone, addToast]);

  // Razorpay checkout hook — for terminal payments
  const { openCheckout, isCheckoutOpen, isVerifying } = useRazorpayCheckout({
    contractId: createdContractId,
    onPaymentVerified: (result) => {
      setRecordedReceipt({
        receipt_id: result.receipt?.receipt_id || '',
        receipt_number: result.receipt?.receipt_number || '',
        amount: result.receipt?.amount || 0,
        currency: wizardState.currency || 'INR',
        payment_method: 'online',
        payment_date: new Date().toISOString(),
        invoice_id: '',
        invoice_number: '',
        invoice_status: result.receipt?.invoice_status || 'paid',
        amount_paid: result.receipt?.amount || 0,
        balance: result.receipt?.balance || 0,
        receipts_count: 1,
      });
      setCnakCopied(false);
      setIsContractSent(true);
    ...
    onPaymentFailed: () => {
      // Contract already created, just show success screen
      setCnakCopied(false);
      setIsContractSent(true);
    },
    onDismiss: () => {
        ...
        <div className="relative z-10 w-full h-full flex items-center justify-center overflow-y-auto py-8">
          ...
          >
            ...
            <div className="p-4 pb-2" style={{ borderBottom: `1px solid ${colors.utility.border}` }}>
              ...
              <p className="text-[11px]" style={{ color: colors.utility.secondaryText }}>
                {paymentChannel === 'online'
                  ? `Collect online via ${providerDisplayName || 'payment gateway'}. Contract will be created first.`
                  : 'Record offline payment details before creating the contract.'}
              </p>
            </div>
            ...
            <div className="p-4 space-y-3">
              ...
              {hasActiveGateway && (
                <div>
                  <label style={pmtLabelStyle}>Payment Channel</label>
                  <div className="grid grid-cols-2 gap-1.5 p-1 rounded-lg" style={{ backgroundColor: colors.utility.secondaryBackground }}>
                    {(['online', 'offline'] as const).map((ch) => (
                      <button
                        key={ch}
                        onClick={() => { setPaymentChannel(ch); setOnlineLinkResult(null); setLinkCopied(false); }}
                        className="flex items-center justify-center gap-1.5 py-1.5 rounded-md text-[11px] font-medium transition-all"
                        style={{
                          backgroundColor: paymentChannel === ch ? colors.brand.primary : 'transparent',
                          color: paymentChannel === ch ? '#fff' : colors.utility.secondaryText,
                        }}
                      >
                        {ch === 'online' ? <Globe className="w-3 h-3" /> : <Wallet className="w-3 h-3" />}
              ...
              {paymentChannel === 'online' && (
                <div>
                  <label style={pmtLabelStyle}>Collection Mode</label>
                  <div className="grid grid-cols-3 gap-1.5">
                    {([
                      { key: 'terminal' as const, label: 'Pay Now', icon: <Monitor className="w-3.5 h-3.5" /> },
                      { key: 'email_link' as const, label: 'Email Link', icon: <Mail className="w-3.5 h-3.5" /> },
                      { key: 'whatsapp_link' as const, label: 'WhatsApp', icon: <MessageSquare className="w-3.5 h-3.5" /> },
                    ]).map(({ key, label, icon }) => (
                      <button
                        key={key}
                        onClick={() => { setOnlineCollectionMode(key); setOnlineLinkResult(null); setLinkCopied(false); }}
                        className="flex flex-col items-center gap-1 py-2 rounded-lg text-[10px] font-medium transition-all"
                        style={{
                          backgroundColor: onlineCollectionMode === key
                            ? `${colors.brand.primary}15`
                            : colors.utility.secondaryBackground,
                          border: `1px solid ${onlineCollectionMode === key ? colors.brand.primary : colors.utility.border}`,
                          color: onlineCollectionMode === key ? colors.brand.primary : colors.utility.secondaryText,
                        }}
                      >
                        {icon}
                        {label}
              ...
              {/* ─── Link Success State ─── */}
              {onlineLinkResult && (
                <div className="p-3 rounded-lg" style={{ backgroundColor: `${colors.semantic.success}10`, border: `1px solid ${colors.semantic.success}30` }}>
                  <p className="text-xs font-medium mb-2" style={{ color: colors.semantic.success }}>Payment link created</p>
                  <div className="flex items-center gap-2">
                    <input
                      readOnly
                      value={onlineLinkResult.url}
                      className="flex-1 text-[11px] px-2 py-1.5 rounded"
                      style={pmtInputStyle}
                    />
                    <button
                      onClick={() => { navigator.clipboard.writeText(onlineLinkResult.url); setLinkCopied(true); setTimeout(() => setLinkCopied(false), 2000); }}
                      className="shrink-0 px-2 py-1.5 rounded text-[10px] font-medium"
                      style={{ backgroundColor: colors.brand.primary, color: '#fff' }}
                    >
                      {linkCopied ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                    </button>
                  </div>
                  {onlineLinkResult.expiresAt && (
                    <p className="text-[10px] mt-1" style={{ color: colors.utility.secondaryText }}>
                      Expires: {new Date(onlineLinkResult.expiresAt).toLocaleString()}
                    </p>
                  )}
                  <button
                    onClick={() => { setShowPrePaymentDialog(false); setCnakCopied(false); setIsContractSent(true); }}
                    className="w-full mt-2 py-2 rounded-lg text-xs font-semibold text-white transition-all hover:opacity-90"
              ...
              {/* ─── Offline-Only Fields ─── */}
              {!onlineLinkResult && paymentChannel === 'offline' && (
                <>
                  <div>
              ...
              {/* ─── Online Link Fields (customer details) ─── */}
              {!onlineLinkResult && paymentChannel === 'online' && onlineCollectionMode !== 'terminal' && (
                <>
                  <div>
                    <label style={pmtLabelStyle}>Customer Name (optional)</label>
                    <input
                      type="text"
                      value={onlineCustomerName}
                      onChange={(e) => setOnlineCustomerName(e.target.value)}
                      placeholder={wizardState.buyerName || 'Buyer name'}
                      style={pmtInputStyle}
                    />
                  </div>
                  {onlineCollectionMode === 'email_link' && (
                    <div>
                      <label style={pmtLabelStyle}>Customer Email *</label>
                      <input
                        type="email"
                        value={onlineCustomerEmail}
                        onChange={(e) => setOnlineCustomerEmail(e.target.value)}
                        placeholder="buyer@example.com"
                        style={pmtInputStyle}
                      />
                    </div>
                  )}
                  {onlineCollectionMode === 'whatsapp_link' && (
                    <div>
                      <label style={pmtLabelStyle}>Customer Phone *</label>
                      <input
                        type="tel"
              ...
              >
                ...
                <button
                  onClick={
                    paymentChannel === 'offline'
                      ...
                      : onlineCollectionMode === 'terminal'
                        ? handleCreateWithOnlineTerminal
                        : handleCreateWithOnlineLink
                  }
                  ...
                  style={{
                    backgroundColor: paymentChannel === 'online' ? colors.brand.primary : colors.semantic.success,
                ...
                >
                  {isProcessingPayment ? (
                    ...
                    <><CreditCard className="w-3.5 h-3.5" /> Pay Now & Create Contract</>
                  ) : (
                    <><Link2 className="w-3.5 h-3.5" /> Send Link & Create Contract</>
                  )}
                </button>
                <button
