...
INSERT INTO kt_compliance_defaults (sub_category, compliance_standard, description) VALUES
  ('Medical Imaging', 'AERB', 'Atomic Energy Regulatory Board'),
  ('Medical Imaging', 'NABH', 'National Accreditation Board for Hospitals'),
  ('Medical Imaging', 'IEC 60601', 'International standard for medical electrical equipment safety'),
  ('Life Support', 'NABH', 'National Accreditation Board for Hospitals'),
  ...
  ('Diagnostic Equipment', 'NABH', 'National Accreditation Board for Hospitals'),
  ('Diagnostic Equipment', 'IEC 60601', 'International standard for medical electrical equipment safety'),
  ('Patient Monitoring', 'NABH', 'National Accreditation Board for Hospitals'),
  ...
  ('HVAC', 'BEE', 'Bureau of Energy Efficiency'),
  ...
  ('Water Treatment', 'CPCB', 'Central Pollution Control Board'),
  ('Water Treatment', 'FSSAI', 'Food Safety and Standards Authority'),
...
CREATE INDEX IF NOT EXISTS idx_kt_equipment_meta_rtemplate ON kt_equipment_meta (resource_template_id);
CREATE INDEX IF NOT EXISTS idx_kt_compliance_defaults_sub_category ON kt_compliance_defaults (sub_category);
CREATE INDEX IF NOT EXISTS idx_m_equipment_checkpoints_compliance ON m_equipment_checkpoints (compliance_standard) WHERE compliance_standard IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_m_equipment_checkpoints_mandatory ON m_equipment_checkpoints (is_mandatory) WHERE is_mandatory = TRUE;
