...
import {ArrowLeft,CheckCircle2,Save,Send,Eye,Building2} from 'lucide-react';
...
export default function ReviewPage(p:Props){
 ...
 const [documentView,setDocumentView]=useState<'self'|'client'>('self');
 ...
 return <div className="ag-page" style={{'--ag-brand':c.brand.primary,'--ag-on-brand':textOnBrand(c.brand.primary),'--ag-bg':c.utility.secondaryBackground,'--ag-card':c.utility.primaryBackground,'--ag-text':c.utility.primaryText,'--ag-muted':c.utility.secondaryText,'--ag-border':c.utility.border} as React.CSSProperties}>
  ...
  <main className="rv-main"><div className="ag-intro"><span className="ag-eyebrow">06 · FINAL REVIEW</span><h1>Ready for the other side of the table.</h1><p>Check the document as seller or buyer, then choose the next lifecycle action.</p></div>
   ...
   <section className="rv-viewbar"><div><span className="ag-eyebrow">DOCUMENT VIEW</span><h2>{documentView==='self'?'Your working copy':`${p.relationship==='partner'?'Partner':'Customer'} copy`}</h2><p>{documentView==='self'?'Review the agreement you are preparing.':`This is how ${s.buyerName||'the other party'} will read the shared agreement.`}</p></div><div className="rv-toggle" role="group" aria-label="Document perspective"><button aria-pressed={documentView==='self'} onClick={()=>setDocumentView('self')}><Building2 size={16}/>Seller view</button><button aria-pressed={documentView==='client'} onClick={()=>setDocumentView('client')}><Eye size={16}/>{p.relationship==='partner'?'Partner view':'Customer view'}</button></div></section>
   <section className="rv-document" data-view={documentView}><ReviewSendStep documentOnly forcedViewMode={documentView} contractName={s.contractName} contractStatus={s.status} startDate={s.startDate} description={s.description} durationValue={s.durationValue} durationUnit={s.durationUnit} buyerId={s.buyerId} buyerName={s.buyerName} contractType={p.relationship} acceptanceMethod={s.acceptanceMethod} billingCycleType={s.billingCycleType} currency={s.currency} selectedBlocks={s.selectedBlocks} paymentMode={s.paymentMode} emiMonths={s.emiMonths} perBlockPaymentType={s.perBlockPaymentType} selectedTaxRateIds={s.selectedTaxRateIds} nomenclatureName={s.nomenclatureName}/></section>
