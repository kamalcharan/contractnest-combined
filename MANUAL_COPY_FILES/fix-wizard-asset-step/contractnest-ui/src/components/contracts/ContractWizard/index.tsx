...
const createInitialWizardState = (): ContractWizardState => ({
  ...
  nomenclatureRequiresAssets: true, // Default true — show assets until we know otherwise
  // Acceptance
...
const ContractWizard: React.FC<ContractWizardProps> = ({
  ...
  // Dynamic step array based on wizard mode + nomenclature
  // Skip assetSelection when the selected nomenclature doesn't involve physical assets
  const activeSteps = useMemo(() => {
    if (isRfqMode) return RFQ_STEPS;
    if (!wizardState.nomenclatureRequiresAssets) {
      const filtered = CONTRACT_STEPS.filter(s => s.id !== 'assetSelection');
      console.log('[Wizard] activeSteps: FILTERED (no assets)', filtered.map(s => s.id));
      return filtered;
    }
    console.log('[Wizard] activeSteps: FULL (with assets)', CONTRACT_STEPS.map(s => s.id));
  ...
  const handleNomenclatureSelect = useCallback(
    (id: string | null, displayName: string | null, group?: string | null, formSettings?: any) => {
      console.log('[Wizard] Nomenclature selected:', { id, displayName, group, formSettings, is_equipment_based: formSettings?.is_equipment_based, is_entity_based: formSettings?.is_entity_based });
      ...
      // Determine if the selected nomenclature involves physical assets
      // Uses form_settings flags from master data (is_equipment_based, is_entity_based)
      // Falls back to group-name check, and defaults to true (show assets) when deselected
      const requiresAssets = id === null
        ? true // Deselected — show assets by default
        : formSettings
          ? (formSettings.is_equipment_based === true || formSettings.is_entity_based === true)
          : group === 'equipment_maintenance' || group === 'facility_property';

      console.log('[Wizard] requiresAssets =', requiresAssets);
      updateWizardState('nomenclatureRequiresAssets', requiresAssets);

      // Clear asset data when switching to a type that doesn't need assets
      if (!requiresAssets) {
