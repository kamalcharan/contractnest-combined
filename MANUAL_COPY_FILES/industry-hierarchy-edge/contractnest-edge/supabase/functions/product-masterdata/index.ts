...
serve(async (req: Request) => {
  ...
  try {
    ...
    // ✅ NEW: Industry hierarchy parameters
    const level = url.searchParams.get('level'); // '0' for segments, '1' for sub-segments, null for all
    const parentId = url.searchParams.get('parent_id'); // Filter sub-segments by parent
    ...
    if (pathname.includes('/sub-segments')) {
      // ✅ NEW: Get sub-segments for a specific parent industry
      if (!parentId) {
        return new Response(
          JSON.stringify({
            success: false,
            error: 'parent_id parameter is required for sub-segments'
          }),
          {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      ...
      response = await getSubSegments(supabase, parentId, isActive, page, limit, search);

    } else if (pathname.includes('/industries')) {
      // Get industries with pagination, search, and hierarchy support (m_catalog_industries)
      // ✅ UPDATED: Now supports level and parent_id filters
      response = await getIndustries(supabase, isActive, page, limit, search, level, parentId);
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: 'Invalid endpoint. Use /product-masterdata, /tenant-masterdata, /all-global-categories, /all-tenant-categories, /industries, /sub-segments, /all-categories, or /industry-categories'
        }),
        {
          status: 404,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
...
// ✅ UPDATED: getIndustries with hierarchy support
async function getIndustries(
  ...
  try {
    console.log(`🔍 Fetching industries - Page: ${page}, Limit: ${limit}, Search: "${search}", Level: ${level}, ParentId: ${parentId}`);
    ...
    // ✅ NEW: Filter by level (0 = segments, 1 = sub-segments)
    if (level !== null && level !== undefined && level !== '') {
      query = query.eq('level', parseInt(level));
    }

    // ✅ NEW: Filter by parent_id for sub-segments
    if (parentId) {
    ...
    // ✅ NEW: Get child counts for each industry to determine has_children
    let industriesWithChildren = industries || [];

    if (industries && industries.length > 0) {
      // Get all industry IDs to check for children
      const industryIds = industries.map((ind: any) => ind.id);

      // Count children for each industry
      const { data: childCounts, error: childError } = await supabase
        .from('m_catalog_industries')
        .select('parent_id')
        .in('parent_id', industryIds)
        .eq('is_active', true);

      if (!childError && childCounts) {
        // Create a map of parent_id -> child count
        const childCountMap: Record<string, number> = {};
        childCounts.forEach((child: any) => {
          childCountMap[child.parent_id] = (childCountMap[child.parent_id] || 0) + 1;
        });

        // Add has_children and children_count to each industry
        industriesWithChildren = industries.map((ind: any) => ({
          ...ind,
          has_children: (childCountMap[ind.id] || 0) > 0,
          children_count: childCountMap[ind.id] || 0
        }));
      } else {
        // If child count query fails, just add default values
        industriesWithChildren = industries.map((ind: any) => ({
          ...ind,
          has_children: false,
          children_count: 0
    ...
    console.log(`✅ Found ${industriesWithChildren.length} industries (${count} total)`);

    return {
      success: true,
      data: industriesWithChildren,
      pagination: {
        current_page: page,
...
async function getSubSegments(
  ...
  try {
    console.log(`🔍 Fetching sub-segments for parent: ${parentId}, Page: ${page}, Limit: ${limit}, Search: "${search}"`);

    const offset = (page - 1) * limit;

    // First, verify the parent exists and is a valid segment
    ...
    if (parentError || !parent) {
      console.log(`❌ Parent industry ${parentId} not found:`, parentError);
      return {
        success: false,
        error: `Parent industry '${parentId}' not found`,
        data: []
      };
    }
    ...
    const { data: subSegments, error, count } = await query;

    console.log('📋 Sub-segments query result:', {
      data: subSegments,
      error: error,
      count: count
    });
    ...
    if (error) {
      console.log(`❌ Failed to fetch sub-segments for parent ${parentId}:`, error);
      return {
        success: false,
        error: `Failed to fetch sub-segments for industry ${parentId}`,
        data: []
      };
    }
    ...
    console.log(`✅ Found ${subSegments?.length || 0} sub-segments for ${parentId} (${count} total)`);
    ...
    console.error(`❌ Error fetching sub-segments for ${parentId}:`, error);
    return {
      success: false,
      error: `Failed to fetch sub-segments for industry ${parentId}`,
      data: []
