...
const ServiceBlocksStep: React.FC<ServiceBlocksStepProps> = ({
  ...
  const { data: catData, isLoading: catalogLoading, isError: catalogError, refetch: retryCatalog } = useCatBlocksTest({ strict: experience });
  ...
  if (experience) return <ServicesCatalog catalog={allCatalogBlocks} selected={selectedBlocks} coverage={coverageTypes} currency={currency}
    scope={activeCoverageTabId} onScope={id => { setActiveCoverageTabId(id); setExpandedBlockId(null); }}
    loading={catalogLoading} error={catalogError || catalogConversion.error} retry={() => void retryCatalog()} sections={SECTIONS} flyByTypes={flyByMenuOptions}
    ...
    editor={(instance, block) => <ChecklistRow editorOnly colors={colors} isDarkMode={isDarkMode} currency={currency} instance={instance} block={block}
