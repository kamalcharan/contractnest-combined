...
const ContractWizard: React.FC<ContractWizardProps> = ({
  ...
  // Wizard data state
  const [experienceStep, setExperienceStep] = useState<'agreement' | 'coverage' | 'services'>('agreement');
  ...
  useEffect(() => {
    if (draftContractData?.metadata?.wizard_state && isOpen) {
      ...
      if (agreementOnly) {
        setExperienceStep(draftContractData.metadata.experience_step === 'services' ? 'services' : draftContractData.metadata.experience_step === 'coverage' ? 'coverage' : 'agreement');
