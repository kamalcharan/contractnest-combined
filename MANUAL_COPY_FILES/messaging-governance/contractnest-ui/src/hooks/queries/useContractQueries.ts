...
export const useContractOperations = () => {
  ...
  const sendNotificationMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      if (data?.suppressed) {
        // Governed send: the contract_signoff_invite automation rule is off —
        // nothing was dispatched (recorded in contract history).
        toast({
          title: 'Invite not sent',
          description:
            'Contract sign-off invites are turned off in Settings → Automations — share the acceptance link with the buyer manually.',
        });
        return;
      }
      toast({
