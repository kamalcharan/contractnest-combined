...
async function handleSendNotification(
  ...
  try {
    ...
    // Rule gate (operations-loop/022): sign-off invites are governed by
    // Settings → Automations (contract_signoff_invite) AND the channel
    // switches in /integrations. When the rule is off, record an auditable
    // history entry instead of sending — the contract itself is unaffected
    // and the caller gets success with suppressed=true (not an error).
    const { data: signoffRuleEnabled, error: ruleError } = await supabase.rpc('vani_rule_enabled', {
      p_tenant_id: tenantId,
      p_rule_key: 'contract_signoff_invite'
    });
    if (!ruleError && signoffRuleEnabled === false) {
      await supabase.from('t_contract_history').insert({
        contract_id: contractId,
        tenant_id: tenantId,
        action: 'notification_suppressed',
        changes: { rule_key: 'contract_signoff_invite', channels: ['email', 'whatsapp'] },
        performed_by_type: 'system',
        performed_by_name: 'VaNi',
        note: 'Sign-off invite suppressed — "Contract sign-off invites" is off in Settings → Automations'
      });
      return jsonResponse({
        success: true,
        suppressed: true,
        message: 'Notification suppressed — the "Contract sign-off invites" automation rule is off'
      }, 200);
    }
