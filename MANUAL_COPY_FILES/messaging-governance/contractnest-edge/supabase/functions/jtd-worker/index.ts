...
// Identity messages (owner decision 2026-07-22): password reset, forgot
// password, and user invitations are exceptions to BOTH the global
// TEST-env guardrail and the per-tenant channel kill switch below — a
// tenant that has turned off messaging (or is in test mode) must still be
// able to bring a real teammate onto the platform. Password reset itself
// never enters this queue (Supabase Auth delivers it directly; a
// non-'created' audit-only n_jtd row is inserted separately for visibility
// and is never picked up here). This is the only source_type_code
// exempted — every other message (service/payment reminders, contract
// invites, etc.) remains fully governed by rules + channels.
const GATE_EXEMPT_SOURCE_TYPES = new Set(['user_invite']);
...
async function processMessage(msg: JTDQueueMessage): Promise<void> {
  ...
  try {
    ...
    if (isGateExempt) {
      console.log(`JTD ${jtd_id} (${source_type_code}) is an identity message — exempt from the TEST-env guardrail and tenant channel kill switch`);
    }
