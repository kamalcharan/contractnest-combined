-- ============================================================================
-- Migration: Stage 3 Appointments — 014 Appointment fields on the events list
-- ============================================================================
-- Purpose (owner feedback): Service Schedule + EventCard (contract Tasks tab,
-- cockpit) must show appointment context and offer booking. Full CREATE OR
-- REPLACE of the 011 version with THREE added fields (marked STAGE 3 CHANGE):
-- appointment_id / appointment_status / appointment_scheduled_at via a LEFT
-- JOIN on t_appointments (unique active row per event — no fanout).
-- Depends on: operations-loop/011, operations-loop/012 (t_appointments)
-- Safe to re-run: Yes
         ...
         FROM (
             SELECT
                 ...
                 jsonb_build_object(
                     ...
                     ''appointment_scheduled_at'', ap.scheduled_at,
