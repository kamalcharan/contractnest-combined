...
// Saves business_name, gst_number, pan_number, business_email, address_line1, city
// to /api/tenant-profile. Logo upload via /api/tenant-profile/logo.
// GST/PAN persisted after migration 008_add_gst_pan_to_tenant_profiles.sql.
...
const BusinessDetailsStep: React.FC = () => {
  ...
  useEffect(() => {
    setForm(prev => ({
      ...
      gstNumber: (profile as any)?.gst_number || prev.gstNumber,
      panNumber: (profile as any)?.pan_number || prev.panNumber,
    }));
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      const payload: Record<string, any> = {
        ...
        // GST/PAN — persisted after migration 008
        gst_number: form.gstNumber.trim() || undefined,
        pan_number: form.panNumber.trim() || undefined,
      };
