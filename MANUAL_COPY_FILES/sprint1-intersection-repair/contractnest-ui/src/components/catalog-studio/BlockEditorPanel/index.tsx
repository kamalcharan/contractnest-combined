...
const BlockEditorPanel: React.FC<BlockEditorPanelProps> = ({
  ...
  // Sprint 1: seeded service blocks are variant_based (price varies by KT
  // variant of the linked equipment/facility) — label all three modes honestly
  const pricingModeLabel =
    pricingMode === 'resource_based' ? 'Resource Based'
    : pricingMode === 'variant_based' ? 'Variant Based'
    : 'Independent';
      ...
      {block.categoryId === 'spare' && (
        <div className="mb-4">
          ...
          <div className="space-y-1 mt-2">
            <ViewField
              label="Pricing Mode"
              value={<Badge variant="primary" colors={colors}>{pricingModeLabel}</Badge>}
              colors={colors}
            />
