...
export const catBlockToBlock = (catBlock: CatBlock): Block => {
  ...
  return {
    ...
    meta: {
      ...
      // KT lineage (Sprint 1 Task 4.2): seeded blocks carry the resource
      // template they were derived from — surfaced read-only in the UI
      is_seed: catBlock.is_seed ? 'true' : 'false',
      resource_template_id: (catBlock as any).resource_template_id || null,
      seeded_from_name: config.selectedResources?.[0]?.resource_name || null,
