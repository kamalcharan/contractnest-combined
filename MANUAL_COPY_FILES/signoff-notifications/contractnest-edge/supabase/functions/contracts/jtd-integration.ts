...
async function createContractSignoffJTD(
  ...
  try {
    ...
    // IMPORTANT: For WhatsApp templates, key ORDER matters!
    // MSG91 uses Object.values() which respects insertion order.
    // Template: {{1}}=recipient_name, {{2}}=sender_name, {{3}}=contract_title,
    //           {{4}}=contract_number, {{5}}=contract_value, {{6}}=review_link, {{7}}=cnak
    const templateData = {
      recipient_name: params.recipientName,     // {{1}}
      sender_name: params.senderName,           // {{2}}
      contract_title: params.contractTitle,     // {{3}}
      contract_number: params.contractNumber,   // {{4}}
      contract_value: params.contractValue,     // {{5}}
      review_link: params.reviewLink,           // {{6}}
      cnak: params.cnak                         // {{7}}
    };
