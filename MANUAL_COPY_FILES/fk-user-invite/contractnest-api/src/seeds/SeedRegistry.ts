...
export const SeedRegistry: Record<string, SeedDefinition> = {
  ...
  relationships: relationshipsSeedDefinition,  // FamilyKnows family relationships
  // Future seeds:
...
export const getSeedPreview = (category: string): SeedPreview | null => {
  ...
  if (category === 'sequences') {
    ...
    items = FK_RELATIONSHIP_SEED_DATA.map(item => ({
      code: item.code,
      name: item.name,
...
export {
  FK_RELATIONSHIP_SEED_DATA,
  FK_RELATIONSHIP_DISPLAY_NAMES,
  FK_RELATIONSHIPS_CATEGORY,
  relationshipsSeedDefinition,
  transformRelationshipForDB
} from './relationships.seed';
