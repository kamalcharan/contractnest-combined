...
import { Users, Package, Wrench, Truck, Building, Monitor, Box, AlertCircle, Check, Info, ChevronDown, ChevronUp, Loader2, TreePine, X } from 'lucide-react';
...
// =================================================================
// KNOWLEDGE TREE VARIANT PICKER — Slide-out panel only (controlled by parent)
// Parent manages which equipment template is open; panel just renders variants
// =================================================================
...
interface KTVariantPickerProps {
  ...
  colors: any;
  isDarkMode: boolean;
  // Controlled mode: parent tells us which equipment to show
  controlledEquipmentId: string | null;
...
const KnowledgeTreeVariantPicker: React.FC<KTVariantPickerProps> = ({
  ...
  const isPanelOpen = !!controlledEquipmentId;
  const selectedEquipmentId = controlledEquipmentId;

  // Fetch equipment templates (needed to resolve name/sub_category for the header)
  const {
  ...
  // When panel opens with a new equipment, persist the KT ref and reset variants
  useEffect(() => {
    if (controlledEquipmentId) {
      const currentRef = (formData.meta?.knowledge_tree_ref as any)?.resource_template_id;
      if (currentRef !== controlledEquipmentId) {
        onChange('meta', {
          ...formData.meta,
          knowledge_tree_ref: { resource_template_id: controlledEquipmentId },
          selectedVariants: [],
        });
      }
    }
  ...
  // Only render the slide-out panel (no inline equipment list)
  if (!isPanelOpen || !selectedEquipmentId) return null;
...
const ResourceDependencyStep: React.FC<ResourceDependencyStepProps> = ({
  ...
  // Fetch KT equipment templates for name-matching (parent level so it can resolve clicks)
  const { templates: ktEquipmentTemplates } = useResourceTemplatesBrowser({ limit: 100 });

  // Track which resource type is expanded to show its resources
  const [expandedTypeId, setExpandedTypeId] = useState<string | null>(null);

  // Track which KT equipment template is open in the variant slide-out
  const [ktPanelEquipmentId, setKtPanelEquipmentId] = useState<string | null>(
  ...
  // Helper: find KT equipment template that matches a resource type by name (case-insensitive)
  const findKTTemplateForResourceType = (resourceType: ResourceType) => {
    if (!ktEquipmentTemplates || ktEquipmentTemplates.length === 0) return null;
    const rtName = resourceType.name.trim().toLowerCase();
    return ktEquipmentTemplates.find(t => t.name.trim().toLowerCase() === rtName) || null;
  };
  ...
  // Handle resource type toggle (expand/collapse, OR open KT slide-out if equipment)
  const handleResourceTypeToggle = (typeId: string) => {
    const resourceType = resourceTypes?.find(t => t.id === typeId);
    if (resourceType) {
      // Check if this resource type matches a KT equipment template
      const matchingTemplate = findKTTemplateForResourceType(resourceType);
      if (matchingTemplate) {
        // Equipment type → open the KT variant slide-out directly
        setKtPanelEquipmentId(matchingTemplate.id);
        // Also add this resource type to the selected types so it gets tracked
        if (!selectedResourceTypeIds.includes(typeId)) {
          onChange('meta', {
            ...formData.meta,
            resourceTypes: [...selectedResourceTypeIds, typeId],
          });
        }
        return;
      }
    }
    // Non-equipment type (Labor, Team, etc.) → fall back to inline expand behavior
  ...
  // Handle KT panel close — clear panel equipment id
  const handleKTPanelClose = () => {
    setKtPanelEquipmentId(null);
  };
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4 duration-200">
      ...
      <div className="space-y-6">
            ...
            {!loadingTypes && resourceTypes && resourceTypes.length > 0 && (
              <div className="space-y-3">
                {resourceTypes.map((type) => {
                  ...
                  const matchingKTTemplate = findKTTemplateForResourceType(type);
                  const isEquipmentKT = !!matchingKTTemplate;
                  const isExpanded = expandedTypeId === type.id;
                  const selectedCount = getSelectedCountForType(type.id);
                  // For equipment (KT) types, count comes from selectedVariants linked to this template
                  const ktSelectedVariants = (formData.meta?.selectedVariants as SelectedVariant[]) || [];
                  const ktRefId = (formData.meta?.knowledge_tree_ref as any)?.resource_template_id;
                  const ktVariantCount = isEquipmentKT && ktRefId === matchingKTTemplate?.id ? ktSelectedVariants.length : 0;
                  const hasSelections = isEquipmentKT ? ktVariantCount > 0 : selectedCount > 0;
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          {isEquipmentKT && ktVariantCount > 0 && (
                            <span
                            ...
                            >
                              {ktVariantCount} variant{ktVariantCount !== 1 ? 's' : ''}
                            </span>
                          )}
                          {!isEquipmentKT && selectedCount > 0 && (
                            <span
                              className="text-xs px-2 py-0.5 rounded-full"
                              style={{
                                backgroundColor: colors.brand.primary,
                      ...
                      {/* Expanded Resources List — only for non-KT types */}
                      {isExpanded && !isEquipmentKT && (
                        <div
        ...
        {/* ═══ KNOWLEDGE TREE: Variant Slide-out Panel (controlled by parent) ═══ */}
        <KnowledgeTreeVariantPicker
          formData={formData}
          onChange={onChange}
          colors={colors}
          isDarkMode={isDarkMode}
          controlledEquipmentId={ktPanelEquipmentId}
          onControlledClose={handleKTPanelClose}
        />
