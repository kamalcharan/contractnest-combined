...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname.endsWith('/integrations')) {
      ...
      //
      // MERGED with the existing public blob, not replaced: requestData.credentials
      // only ever carries this provider's DECLARED schema fields — an
      // undeclared, derived field (e.g. org_id/mcc, silently detected from
      // an uploaded merchant QR image) is never present in that payload,
      // so a full replace on the next unrelated edit (e.g. just changing
      // payee_name) would silently wipe it. Fetching and spreading the
      // existing public blob first makes every save additive here.
      try {
        ...
        if (prov?.metadata?.config_only) {
          ...
          const { data: existingRow } = await supabase
            ...
            .maybeSingle();
          const existingPublic = existingRow?.credentials?.public || {};
          credentialsJsonb.public = { ...existingPublic, ...(requestData.credentials || {}) };
        }
