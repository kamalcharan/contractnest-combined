...
interface DynamicFormFieldProps {
  ...
  className?: string;
  disabled?: boolean;
  // Fired only for 'image' fields whose upload response includes
  // additional derived values not declared in the schema (e.g. org_id/
  // mcc silently detected from an uploaded UPI merchant QR) — the parent
  // form merges these into its own state under those keys.
  onDetectedFields?: (fields: Record<string, string>) => void;
}
...
const DynamicFormField: React.FC<DynamicFormFieldProps> = ({
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (url) {
        onChange(url);
        const { org_id, mcc } = response.data || {};
        if (org_id || mcc) {
          onDetectedFields?.({ ...(org_id ? { org_id } : {}), ...(mcc ? { mcc } : {}) });
          vaniToast.success('Merchant QR detected — payments will be classified correctly automatically');
        } else {
