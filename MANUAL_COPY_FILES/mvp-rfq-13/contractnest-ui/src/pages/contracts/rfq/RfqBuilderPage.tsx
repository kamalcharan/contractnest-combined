...
// A flyby block the buyer wants quoted. FlyBy = a STRUCTURE with no pricing —
// the buyer names what they need; the vendor fills the price when quoting.
// Types mirror the wizard's flyby blocks (FLYBY_TYPE_CONFIG): service / spare /
// text / document. No library, no pricing — user enters everything now.
// Group Session has no shared FlyBy type yet, so the RFQ carries it as a local
// flyby kind (audience = 'group'). Kept RFQ-local for now — a controlled step;
// promoting 'session' into the shared FLYBY_TYPE_CONFIG is a later increment.
type RfqFlyType = FlyByBlockType | 'session';

interface FlyByLine {
  id: string;
  type: RfqFlyType;         // service | spare | text | document | session
  name: string;
  cycle?: string;           // service — BILLING cycle (prepaid … custom)
  serviceCycleDays?: number; // service / session — SERVICE cycle: every N days (frequency)
  quantity?: number;        // service / spare — how many
  content?: string;         // text — the note the vendor should read
  fileType?: string;        // document — the file the buyer expects back
  description?: string;     // any — optional detail for the vendor
}

// Which flyby types the buyer can add, in order (icons/labels from the shared config
// for the four canonical types; Group Session is a local RFQ addition).
const FLYBY_ADD_ORDER: RfqFlyType[] = ['service', 'spare', 'text', 'document', 'session'];
const DOC_FILE_TYPES = ['pdf', 'docx', 'xlsx', 'pptx', 'txt', 'other'];

// Group Session presentation (local — mirrors the shared config's shape).
const SESSION_CFG = { icon: Users, label: 'Group Session', color: '#0EA5E9', bgColor: '#E0F2FE' };
const flybyCfg = (t: RfqFlyType) => (t === 'session' ? SESSION_CFG : FLYBY_TYPE_CONFIG[t]);
...
// Billing-cycle cards — mirror the canonical CYCLE_OPTIONS used by the service
// blocks (BlockCardConfigurable). Frequency (how often the service runs) is a
// SEPARATE "service cycle" in days, not a cadence label — matching the wizard.
const BILLING_CYCLES = [
  { id: 'prepaid', label: 'PrePaid', icon: CreditCard },
  { id: 'postpaid', label: 'PostPaid', icon: Receipt },
  { id: 'monthly', label: 'Monthly', icon: Calendar },
  { id: 'fortnightly', label: 'Fortnightly', icon: CalendarDays },
...
const RfqBuilderPage: React.FC = () => {
  ...
  const [services, setServices] = useState<FlyByLine[]>([]);
  ...
  // ── service helpers ──────────────────────────────────────────────────────
  const addFlyby = (type: RfqFlyType) =>
    setServices((prev) => [...prev, {
      id: `fb-${Date.now()}-${prev.length}`,
      type,
      name: '',
      cycle: type === 'service' ? 'prepaid' : undefined,
      serviceCycleDays: type === 'service' ? 30 : type === 'session' ? 7 : undefined,
      quantity: type === 'service' || type === 'spare' ? 1 : undefined,
      content: type === 'text' ? '' : undefined,
      fileType: type === 'document' ? 'pdf' : undefined,
    }]);
  const updateFlyby = (id: string, patch: Partial<FlyByLine>) =>
    setServices((prev) => prev.map((s) => (s.id === id ? { ...s, ...patch } : s)));
  const removeFlyby = (id: string) => setServices((prev) => prev.filter((s) => s.id !== id));
  ...
  const submit = useCallback(async () => {
    ...
    // FlyBy blocks — structure only, no pricing (the vendor quotes the price).
    const blocks = services.map((s, i) => ({
      position: i,
      source_type: 'flyby',
      // Group Session rides on a known-good flyby_type ('service') and is marked
      // via config.audience — avoids a new DB flyby_type value in this first pass.
      flyby_type: s.type === 'session' ? 'service' : s.type,
      block_name: s.name.trim(),
      block_description: s.description?.trim() || undefined,
      category_name: flybyCfg(s.type).label,
      unit_price: 0,
      quantity: s.quantity ?? 1,
      billing_cycle: s.type === 'service' ? s.cycle : undefined,
      total_price: 0,
      custom_fields: {
        config: {
          flyby_type: s.type,
          ...(s.type === 'session' ? { audience: 'group' } : {}),
          // Frequency lives on the service cycle (days), separate from billing cycle.
          ...((s.type === 'service' || s.type === 'session') && s.serviceCycleDays ? { serviceCycles: { days: s.serviceCycleDays, enabled: true } } : {}),
          ...(s.content ? { content: s.content.trim() } : {}),
          ...(s.fileType ? { fileType: s.fileType } : {}),
          ...(s.description ? { notes: s.description.trim() } : {}),
        },
      },
    }));
  ...
  return (
    <div style={{ minHeight: '100vh', background: bg, padding: '0 0 120px' }}>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '28px 20px 0' }}>
        ...
        {/* ── STEP: scope (flyby blocks) ── */}
        {stepId === 'services' && (
          <>
            <h1 style={{ fontSize: 24, fontWeight: 700, color: ink, margin: '0 0 4px' }}>What should vendors quote for?</h1>
            <p style={{ fontSize: 14, color: sub, margin: '0 0 16px' }}>Build your request from flyby blocks — a structure you name now; the vendor fills the price when they quote. No pricing here.</p>

            {/* add buttons — one per flyby type (shared FLYBY_TYPE_CONFIG) */}
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
              {FLYBY_ADD_ORDER.map((t) => {
                const cfg = flybyCfg(t);
                const Icon = cfg.icon;
                return (
            ...
            {services.map((s) => {
              const cfg = flybyCfg(s.type);
              const Icon = cfg.icon;
              return (
                <div key={s.id} style={{ ...card, marginBottom: 10, borderColor: `${cfg.color}44` }}>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <span style={{ width: 30, height: 30, borderRadius: 8, flex: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', background: `${cfg.color}18`, color: cfg.color }}>
                      <Icon className="w-4 h-4" />
                    </span>
                    <input value={s.name} onChange={(e) => updateFlyby(s.id, { name: e.target.value })}
                      placeholder={s.type === 'service' ? 'e.g. Preventive maintenance visit' : s.type === 'spare' ? 'e.g. HVAC filter kit' : s.type === 'text' ? 'e.g. Scope note / terms' : s.type === 'session' ? 'e.g. Weekly safety training session' : 'e.g. Compliance certificate'}
                      style={{ ...fieldStyle, flex: 1 }} />
                    <span style={{ fontSize: 10.5, fontWeight: 700, color: cfg.color, textTransform: 'uppercase', letterSpacing: '0.05em', flex: 'none' }}>{cfg.label}</span>
                    <button onClick={() => removeFlyby(s.id)} style={{ background: 'none', border: 'none', color: sub, cursor: 'pointer', flex: 'none' }}><X className="w-4 h-4" /></button>
                  </div>

                  {/* service → billing cycle (proper cards) + service cycle (frequency) */}
                  {s.type === 'service' && (
                    <>
                      <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: sub, margin: '12px 0 6px' }}>Billing cycle</div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                        {BILLING_CYCLES.map((cy) => {
                          const on = s.cycle === cy.id;
                          const CyIcon = cy.icon;
                          return (
                            <button key={cy.id} onClick={() => updateFlyby(s.id, { cycle: cy.id })}
                              style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 600, padding: '6px 11px', borderRadius: 8, cursor: 'pointer',
                                border: `1.5px solid ${on ? brand : ink + '22'}`, background: on ? `${brand}18` : 'transparent', color: on ? brand : sub }}>
                              <CyIcon className="w-3.5 h-3.5" /> {cy.label}
                            </button>
                          );
                        })}
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
                        <span style={{ fontSize: 12, color: sub }}>Performed every</span>
                        <input type="number" min={1} value={s.serviceCycleDays ?? 30}
                          onChange={(e) => updateFlyby(s.id, { serviceCycleDays: Math.max(1, parseInt(e.target.value, 10) || 1) })}
                          style={{ ...fieldStyle, width: 74, padding: '7px 10px', fontSize: 14 }} />
                        <span style={{ fontSize: 12, color: sub }}>days from start</span>
                      </div>
                    </>
                  )}
                  ...
                  {s.type === 'spare' && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10 }}>
                      <span style={{ fontSize: 12, color: sub }}>Quantity</span>
                      <div style={{ display: 'flex', alignItems: 'center', border: `1px solid ${ink}22`, borderRadius: 8, overflow: 'hidden' }}>
                        <button onClick={() => updateFlyby(s.id, { quantity: Math.max(1, (s.quantity ?? 1) - 1) })} style={{ width: 30, height: 32, background: ink + '0D', color: ink, border: 'none', cursor: 'pointer' }}><Minus className="w-3.5 h-3.5" style={{ margin: '0 auto' }} /></button>
                        <span style={{ width: 34, textAlign: 'center', fontSize: 14, fontWeight: 600, color: ink }}>{s.quantity ?? 1}</span>
                        <button onClick={() => updateFlyby(s.id, { quantity: (s.quantity ?? 1) + 1 })} style={{ width: 30, height: 32, background: ink + '0D', color: ink, border: 'none', cursor: 'pointer' }}><Plus className="w-3.5 h-3.5" style={{ margin: '0 auto' }} /></button>
                      </div>
                    </div>
                  )}
                  ...
                  {/* text → content */}
                  {s.type === 'text' && (
                    <textarea value={s.content || ''} onChange={(e) => updateFlyby(s.id, { content: e.target.value })} rows={2}
                      placeholder="What should the vendor know or agree to?" style={{ ...fieldStyle, marginTop: 10, resize: 'vertical', fontSize: 13 }} />
                  )}

                  {/* document → expected file type */}
                  {s.type === 'document' && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10, alignItems: 'center' }}>
                      <span style={{ fontSize: 12, color: sub }}>Expected file</span>
                      {DOC_FILE_TYPES.map((ft) => {
                        const on = s.fileType === ft;
                        return (
                          <button key={ft} onClick={() => updateFlyby(s.id, { fileType: ft })}
                            style={{ fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: 999, cursor: 'pointer', textTransform: 'uppercase',
                              border: `1.5px solid ${on ? brand : ink + '22'}`, background: on ? `${brand}18` : 'transparent', color: on ? brand : sub }}>
                            {ft}
                          </button>
                        );
                  ...
                  {/* group session → held every N days (occurrence cadence), no pricing */}
                  {s.type === 'session' && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
                      <Users className="w-3.5 h-3.5" style={{ color: SESSION_CFG.color }} />
                      <span style={{ fontSize: 12, color: sub }}>Group session · held every</span>
                      <input type="number" min={1} value={s.serviceCycleDays ?? 7}
                        onChange={(e) => updateFlyby(s.id, { serviceCycleDays: Math.max(1, parseInt(e.target.value, 10) || 1) })}
                        style={{ ...fieldStyle, width: 74, padding: '7px 10px', fontSize: 14 }} />
                      <span style={{ fontSize: 12, color: sub }}>days</span>
                    </div>
                  )}

                  {/* description — optional, for every type except text (whose content IS its text) */}
                  {s.type !== 'text' && (
                    <textarea value={s.description || ''} onChange={(e) => updateFlyby(s.id, { description: e.target.value })} rows={2}
                      placeholder="Description (optional) — details for the vendor" style={{ ...fieldStyle, marginTop: 10, resize: 'vertical', fontSize: 13 }} />
                  )}
                </div>
              );
            })}
              ...
              {services.map((s) => (
                <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: `1px solid ${line}`, fontSize: 13.5 }}>
                  <span style={{ color: ink }}>
                    <span style={{ color: flybyCfg(s.type).color, fontWeight: 700, fontSize: 11, marginRight: 6, textTransform: 'uppercase' }}>{flybyCfg(s.type).label}</span>
                    {s.name || `Unnamed ${flybyCfg(s.type).label}`}
                  </span>
                  <span style={{ color: sub, fontFamily: 'monospace' }}>
                    {s.type === 'service'
                      ? `${BILLING_CYCLES.find((x) => x.id === s.cycle)?.label || ''}${s.serviceCycleDays ? ` · ${s.serviceCycleDays}d` : ''}`
                      : s.type === 'session' ? `every ${s.serviceCycleDays ?? 7}d`
                      : s.type === 'spare' ? `×${s.quantity ?? 1}`
                      : s.type === 'document' ? (s.fileType || '').toUpperCase() : ''}
                  </span>
                </div>
              ))}
