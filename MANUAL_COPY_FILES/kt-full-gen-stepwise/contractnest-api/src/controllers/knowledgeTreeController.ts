...
class KnowledgeTreeController {
  ...
  //   existingKT: true  → activity-specific single prompt (+ Install / + Decomm) — unchanged
  //   existingKT: false → runs 3 focused steps internally, returns combined payload
  generate = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      if (existingKT) {
        // ── Activity mode (+ Install / + Decomm) — single focused prompt, unchanged ──
        ...
        // ── Full generation — 3 focused steps in sequence ──
        // Replaces the old single 16k-token prompt which was hitting max_tokens.
        // Response format is identical — UI save logic is unchanged.
        console.log(`🔄 Full KT generation — running 3 steps for "${equipmentName}"`);

        // Step 1: Variants
        const rawVariants = await knowledgeTreeGeneratorService.generateVariants({ equipmentName, subCategory, resourceTemplateId });
        if (!rawVariants.variants?.length) throw new Error('Step 1: No variants returned. Please retry.');
        const variantsPayload = resolveVariantIds(rawVariants);
        const variantRefs = variantsPayload.variants.map((v: any) => ({ id: v.id, name: v.name, capacity_range: v.capacity_range ?? null }));
        console.log(`✅ Step 1/3 — ${variantsPayload.variants.length} variants`);

        // Step 2: Spare parts (receives real variant UUIDs → proper svm entries)
        const rawParts = await knowledgeTreeGeneratorService.generateSpareParts({ equipmentName, subCategory, resourceTemplateId, variants: variantRefs });
        const partsPayload = resolveSparePartIds(rawParts);
        console.log(`✅ Step 2/3 — ${partsPayload.spare_parts?.length ?? 0} spare parts, ${partsPayload.spare_part_variant_map?.length ?? 0} variant mappings`);

        // Step 3: Checkpoints + service cycles (receives real variant UUIDs)
        const activity = serviceActivity || 'pm';
        const rawCheckpoints = await knowledgeTreeGeneratorService.generateCheckpoints({ equipmentName, subCategory, resourceTemplateId, serviceActivity: activity, variants: variantRefs });
        if (!rawCheckpoints.checkpoints?.length) throw new Error('Step 3: No checkpoints returned. Please retry.');
        if (!rawCheckpoints.service_cycles?.length) throw new Error('Step 3: No service cycles returned. Please retry.');
        const checkpointsPayload = resolveCheckpointIds(rawCheckpoints);
        console.log(`✅ Step 3/3 — ${checkpointsPayload.checkpoints.length} checkpoints, ${checkpointsPayload.service_cycles.length} cycles`);

        const combined = {
          resource_template_id: resourceTemplateId,
          variants: variantsPayload.variants,
          spare_parts: partsPayload.spare_parts ?? [],
          spare_part_variant_map: partsPayload.spare_part_variant_map ?? [],
          checkpoints: checkpointsPayload.checkpoints,
          checkpoint_values: checkpointsPayload.checkpoint_values ?? [],
          checkpoint_variant_map: checkpointsPayload.checkpoint_variant_map ?? [],
          service_cycles: checkpointsPayload.service_cycles,
          context_overlays: [],
        };

        console.log(`✅ KT complete — variants: ${combined.variants.length}, spare_parts: ${combined.spare_parts.length}, svm: ${combined.spare_part_variant_map.length}, checkpoints: ${combined.checkpoints.length}, cycles: ${combined.service_cycles.length}`);
