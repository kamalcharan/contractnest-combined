...
// Props for the inner content component
interface ContractCreateContentProps {
  onExit?: () => void;
}

// Inner component that uses the context
const ContractCreateContent: React.FC<ContractCreateContentProps> = ({ onExit }) => {
  ...
  const handleExit = async () => {
    ...
    // Use onExit callback if provided (embedded mode), otherwise navigate (standalone mode)
    if (onExit) {
      onExit();
    } else {
      navigate('/contracts');
...
// Props for the main page component — supports both standalone route and embedded usage
...
const ContractCreatePage: React.FC<ContractCreatePageProps> = ({ contractType, onExit }) => {
  const [searchParams] = useSearchParams();
  const contractTypeFromUrl = searchParams.get('type') as ContractType | null;

  // Use prop if provided (embedded in hub), otherwise fall back to URL param (standalone route)
  const resolvedType = (contractType as ContractType) || contractTypeFromUrl;

  return (
    <ContractBuilderProvider initialContractType={resolvedType || undefined}>
      <ContractCreateContent onExit={onExit} />
    </ContractBuilderProvider>
  );
};
