...
const ContractWizard: React.FC<ContractWizardProps> = ({
  ...
  const [wizardState, setWizardState] = useState<ContractWizardState>(() => ({
    ...
    ...(agreementOnly && !draftContractId ? { currency: '', durationValue: 0 } : {}),
  }));
  ...
  const saveDraftToApi = useCallback(async (stepIndex: number): Promise<boolean> => {
    ...
    if (agreementOnly && (!wizardState.buyerId || !wizardState.contractName.trim() ||
      !wizardState.currency || !Number.isInteger(wizardState.durationValue) || wizardState.durationValue <= 0 ||
    ...
    try {
      const metadata = {
        ...
        ...(agreementOnly ? { experience_chapter: 'agreement', experience_version: 2 } : {}),
  ...
  }, [wizardState, contractType, draftId, createContract, updateContract, setSilentMode, isExperience, agreementOnly, activeSteps]);
