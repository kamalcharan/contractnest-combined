...
import {
  ...
  Repeat, Pencil, Ban, X, Check, CalendarPlus, Plus, RotateCcw, Mic, UserCog, Lock,
} from 'lucide-react';
...
type ViewLevel = 'overview' | 'group' | 'occurrence';
              ...
              {rosterQuery.isLoading ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="text-[10px] uppercase tracking-wide" style={sub}>
                        <th className="px-4 py-2 font-semibold">Member</th>
                        <th className="px-3 py-2 font-semibold text-right">Attended</th>
                        <th className="px-3 py-2 font-semibold text-right">Missed</th>
                        <th className="px-3 py-2 font-semibold text-right">Subbed</th>
                        <th className="px-3 py-2 font-semibold">Status</th>
                        <th className="px-3 py-2 font-semibold">Dues</th>
                        <th className="px-4 py-2 font-semibold text-right">Contract</th>
                      </tr>
                    </thead>
                    <tbody>
                      {roster.map((m) => {
                        ...
                        return (
                          <tr key={m.contact_id} className="border-t" style={{ borderColor: line }}>
                            <td className="px-4 py-2.5">
                              <div className="flex items-center gap-2.5">
                                ...
                                <div className="min-w-0"><div className="text-[13px] font-semibold truncate" style={ink}>{m.name || '—'}</div><div className="text-[10.5px] truncate" style={sub}>{m.contract_name || ''}</div></div>
                              </div>
                            </td>
                            <td className="px-3 py-2.5 text-right text-[13px] tabular-nums" style={ink}>{m.attended}/{m.overall}</td>
                            <td className="px-3 py-2.5 text-right text-[13px] tabular-nums" style={{ color: m.over_no_show_cap ? colors.semantic.error : colors.utility.primaryText }}>{m.missed}</td>
                            <td className="px-3 py-2.5 text-right text-[13px] tabular-nums" style={{ color: m.over_substitute_cap ? colors.semantic.error : colors.utility.primaryText }}>{m.substituted}</td>
                            <td className="px-3 py-2.5">
                              {overCap ? (
                                <span className="text-[9.5px] font-bold rounded px-1.5 py-0.5 whitespace-nowrap" style={{ backgroundColor: colors.semantic.error + '1e', color: colors.semantic.error }}>
                                  {m.over_no_show_cap ? `⚠ ${m.missed}/${m.max_no_shows} no-shows` : `⚠ ${m.substituted}/${m.max_substitutes} subs`}
                                </span>
                              ) : (
                                <span className="text-[9.5px] font-bold rounded px-1.5 py-0.5 whitespace-nowrap" style={{ backgroundColor: colors.semantic.success + '1e', color: colors.semantic.success }}>OK</span>
                              )}
                            </td>
                            <td className="px-3 py-2.5">
                              <span className="text-[9.5px] font-bold rounded px-1.5 py-0.5 whitespace-nowrap" style={{ backgroundColor: (m.dues_pending ? colors.semantic.warning : colors.semantic.success) + '1e', color: m.dues_pending ? colors.semantic.warning : colors.semantic.success }}>{m.dues_pending ? 'Due' : 'Paid'}</span>
                            </td>
