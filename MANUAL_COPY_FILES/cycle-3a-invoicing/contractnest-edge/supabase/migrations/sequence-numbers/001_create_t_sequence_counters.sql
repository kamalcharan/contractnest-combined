...
BEGIN
    ...
    IF v_next_value IS NULL THEN
        -- Get config from t_category_details (includes sequence_code + form_settings)
        SELECT
            COALESCE((form_settings->>'start_value')::INTEGER, 1),
            sub_cat_name,
            COALESCE(form_settings->>'prefix', ''),
            COALESCE(form_settings->>'separator', '-'),
            COALESCE(form_settings->>'suffix', ''),
            COALESCE((form_settings->>'padding_length')::INTEGER, 4),
            COALESCE((form_settings->>'increment_by')::INTEGER, 1),
            COALESCE(form_settings->>'reset_frequency', 'NEVER'),
            cd.display_name,
            cd.description,
            cd.hexcolor,
            cd.icon_name
        INTO
            v_start_value, v_sequence_code,
            v_prefix, v_separator, v_suffix, v_padding,
            v_increment, v_reset_freq,
            v_display_name, v_description, v_hexcolor, v_icon_name
        ...
        -- Insert new counter with sequence_code + config columns (v005-compatible)
        ...
        ON CONFLICT (sequence_code, tenant_id, is_live)
        DO UPDATE SET
