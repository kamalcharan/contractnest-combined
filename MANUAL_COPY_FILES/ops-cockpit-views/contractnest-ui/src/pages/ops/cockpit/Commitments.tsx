...
import { ArrowUpRight, Search, RefreshCw, Check, Mail, MessageCircle, PhoneCall, UserPlus, PauseCircle, PlayCircle, X, List, LayoutGrid, IndianRupee } from 'lucide-react';
...
type ComingView = 'list' | 'lanes';
type Horizon = 30 | 60 | 90;

// Per-viewer conveniences only (which view, which horizon). Browser storage
// can be empty or throw in private windows — every read/write is guarded and
// the page renders fine without it.
const VIEW_KEY = 'ops.comingUp.view';
const HORIZON_KEY = 'ops.comingUp.horizon';
...
const isView = (v: unknown): v is ComingView => v === 'list' || v === 'lanes';
const isHorizon = (v: unknown): v is Horizon => v === 30 || v === 60 || v === 90;

const clean = (s: string | null | undefined) => (s || '').replace(/\s+/g, ' ').trim();
const daysFromNow = (iso: string) => Math.max(0, Math.ceil((new Date(iso).getTime() - Date.now()) / 86_400_000));
...
const OpsCommitmentsPage: React.FC = () => {
  ...
  // Coming up: how far ahead, and list or lanes. Both views render the same rows.
  const [horizon, setHorizonState] = useState<Horizon>(() => readPref(HORIZON_KEY, isHorizon, 30));
  const [comingView, setComingViewState] = useState<ComingView>(() => readPref(VIEW_KEY, isView, 'list'));
  const setHorizon = (h: Horizon) => { setHorizonState(h); writePref(HORIZON_KEY, h); };
  const setComingView = (v: ComingView) => { setComingViewState(v); writePref(VIEW_KEY, v); };

  const enabled = perspective === 'revenue';
  const wl = useCollectionsWorklist(horizon, { enabled });
  ...
  return (
    <div className="px-6 py-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-start justify-between gap-6 flex-wrap">
        <div className="min-w-0">
          ...
          <h1 className="text-[26px] sm:text-[30px] leading-snug font-medium max-w-2xl" style={ink}>
            ...
            {due.length > 0
              ? <><Num v={due.length} l="ahead" /> {due.length === 1 ? 'payment falls' : 'payments fall'} due in the next {horizon} days{dueToday ? <>, {dueToday} today</> : null}.</>
              : <>Nothing falls due in the next {horizon} days.</>}
          </h1>
        </div>
      ...
      {/* ── COMING UP ── */}
      {showAhead && (() => {
        // One row model for both views: payments and next rungs, each with the
        // days until it lands. Paging later applies per column (lanes) or per
        // section (list) on this same shape.
        type Ahead =
          | { kind: 'payment'; key: string; days: number; when: string; d: (typeof due)[number] }
          | { kind: 'rung'; key: string; days: number; when: string; r: (typeof rungsAhead)[number] };
        const matchesAhead = (name: string | null, cn: string) => !q || [name, cn].some((s) => (s || '').toLowerCase().includes(q));
        const ahead: Ahead[] = [
          ...due.filter((d) => matchesAhead(d.buyer_name, d.contract_number))
            .map((d) => ({ kind: 'payment' as const, key: `p:${d.job_id}`, days: Math.max(0, d.days_until), when: d.due_date, d })),
          ...rungsAhead.filter((r) => matchesAhead(r.buyer_name, r.contract_number))
            .map((r) => ({ kind: 'rung' as const, key: `r:${r.job_id}:${r.rung.step}`, days: daysFromNow(r.rung.due_at), when: r.rung.due_at, r })),
        ].sort((a, b) => a.days - b.days || new Date(a.when).getTime() - new Date(b.when).getTime());

        const bands: Array<{ key: string; label: string; test: (days: number) => boolean }> = [
          { key: 'today', label: 'today', test: (n) => n <= 0 },
          { key: 'soon', label: 'next 3 days', test: (n) => n >= 1 && n <= 3 },
          { key: 'fortnight', label: 'next 2 weeks', test: (n) => n >= 4 && n <= 14 },
          { key: 'rest', label: horizon > 30 ? `15–${horizon} days` : 'rest of the month', test: (n) => n >= 15 },
        ];
        ...
        const Seg: React.FC<{ on: boolean; onClick: () => void; title: string; children: React.ReactNode }> = ({ on, onClick, title, children }) => (
          <button onClick={onClick} title={title} aria-pressed={on}
            className="inline-flex items-center gap-1 px-3 min-h-[36px] rounded-full text-[11px] font-bold"
            style={on ? { backgroundColor: brand, color: '#fff' } : { color: brand, backgroundColor: 'transparent' }}>
            {children}
          </button>
        );

        const PaymentCard: React.FC<{ d: (typeof due)[number]; compact?: boolean }> = ({ d, compact }) => (
          <div className={compact ? 'rounded-xl border px-3 py-2.5' : 'px-4 py-3 flex items-center gap-4'}
            style={compact ? { borderColor: hairline, backgroundColor: colors.utility.primaryBackground } : undefined}>
            {!compact && (
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[13px] font-bold truncate" style={ink}>
                {compact && <IndianRupee size={11} style={{ display: 'inline', verticalAlign: '-1px', marginRight: 4, color: green }} />}
                {clean(d.buyer_name) || d.contract_number}
                <button onClick={() => navigate(`/contracts/${d.contract_id}`)} className="ml-2 text-[10px] font-bold align-middle" style={{ ...mono, color: brand }}>{d.contract_number}</button>
              </p>
              <p className="text-[11.5px] truncate" style={sub}>{d.cycle_label || d.block_name || ''}{compact ? ` · ${fmtDate(d.due_date)}` : ''}</p>
            </div>
            <p className={compact ? 'text-[14px] font-extrabold tabular-nums mt-1' : 'text-[15px] font-extrabold tabular-nums flex-none'} style={ink}>{fmtMoney(d.amount, d.currency)}</p>
          </div>
        );
        const RungCard: React.FC<{ r: (typeof rungsAhead)[number]; compact?: boolean }> = ({ r, compact }) => {
          const Icon = r.rung.channel === 'whatsapp' ? MessageCircle : r.rung.channel === 'call' ? PhoneCall : Mail;
          return (
            <div className={compact ? 'rounded-xl border border-dashed px-3 py-2.5' : 'px-4 py-3 flex items-center gap-4'}
              style={compact ? { borderColor: `${brand}55`, backgroundColor: colors.utility.primaryBackground } : undefined}>
              {!compact && <p className="text-[11px] font-bold w-24 flex-none" style={{ ...mono, color: colors.utility.secondaryText }}>{fmtDate(r.rung.due_at)}</p>}
              <div className="min-w-0 flex-1">
                <p className="text-[13px] truncate" style={ink}>
                  <Icon size={11} style={{ display: 'inline', verticalAlign: '-1px', marginRight: 4, color: brand }} />
                  {channelLabel(r.rung.channel)} rung {r.rung.step} · <b>{clean(r.buyer_name) || r.contract_number}</b>
                </p>
                <p className="text-[11.5px] truncate" style={sub}>day {r.rung.after_days} after {fmtDate(r.due_date)}{compact ? ` · ${fmtTime(r.rung.due_at)}` : ''}</p>
              </div>
              <p className={compact ? 'text-[12.5px] font-bold tabular-nums mt-1' : 'text-[13px] font-bold tabular-nums flex-none'} style={sub}>{fmtMoney(r.amount, r.currency)}</p>
            </div>
          );
        };
        ...
        return (
          <>
            <div className="flex items-end justify-between gap-3 flex-wrap">
              <SectionHead label="coming up" note={`next ${horizon} days · ${plural(due.length, 'payment')} · ${plural(rungsAhead.length, 'rung')}`} color={green} />
              <div className="flex items-center gap-2 mb-2.5 flex-wrap">
                <div className="inline-flex rounded-full border p-0.5" style={{ borderColor: `${brand}45` }} role="group" aria-label="Horizon">
                  {([30, 60, 90] as Horizon[]).map((h) => <Seg key={h} on={horizon === h} onClick={() => setHorizon(h)} title={`Next ${h} days`}>{h} d</Seg>)}
                </div>
                <div className="inline-flex rounded-full border p-0.5" style={{ borderColor: `${brand}45` }} role="group" aria-label="View">
                  <Seg on={comingView === 'list'} onClick={() => setComingView('list')} title="List"><List size={12} /> List</Seg>
                  <Seg on={comingView === 'lanes'} onClick={() => setComingView('lanes')} title="Lanes by when"><LayoutGrid size={12} /> Lanes</Seg>
                </div>
              </div>
            </div>

            {ahead.length === 0 ? (
              <p className="text-[13px] py-8 text-center rounded-2xl border border-dashed" style={{ ...sub, borderColor: hairline }}>
                {due.length === 0 && rungsAhead.length === 0 ? 'Nothing falls due inside the horizon.' : 'Nothing matches the search.'}
              </p>
            ) : comingView === 'list' ? (
              <div className="rounded-2xl border divide-y" style={{ borderColor: hairline, backgroundColor: colors.utility.secondaryBackground }}>
                {ahead.slice(0, 80).map((a) => a.kind === 'payment' ? <PaymentCard key={a.key} d={a.d} /> : <RungCard key={a.key} r={a.r} />)}
              </div>
            ) : (
              <div className="overflow-x-auto -mx-2 px-2 pb-2">
                <div className="grid gap-3" style={{ gridTemplateColumns: `repeat(${bands.length}, minmax(230px, 1fr))`, minWidth: bands.length * 230 + (bands.length - 1) * 12 }}>
                  {bands.map((b) => {
                    const items = ahead.filter((a) => b.test(a.days));
                    const pay = items.filter((a) => a.kind === 'payment').length;
                    return (
                      <div key={b.key} className="rounded-2xl border p-2.5 min-h-[120px]" style={{ borderColor: hairline, backgroundColor: colors.utility.secondaryBackground }}>
                        <div className="flex items-baseline justify-between px-1 mb-2">
                          <p className="text-[10px] font-bold uppercase tracking-[0.18em]" style={{ ...mono, color: b.key === 'today' ? green : colors.utility.primaryText }}>{b.label}</p>
                          <p className="text-[10px] font-bold" style={{ ...sub, ...mono }}>{items.length ? `${pay} pay · ${items.length - pay} rung` : '—'}</p>
                        </div>
                        <div className="space-y-2">
                          {items.slice(0, 40).map((a) => a.kind === 'payment' ? <PaymentCard key={a.key} d={a.d} compact /> : <RungCard key={a.key} r={a.r} compact />)}
                          {items.length > 40 && <p className="text-[11px] text-center py-1" style={sub}>+{items.length - 40} more</p>}
                        </div>
                      </div>
                    );
                  })}
