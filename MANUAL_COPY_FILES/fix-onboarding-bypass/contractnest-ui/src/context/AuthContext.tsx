...
interface AuthContextType {
  ...
  checkOnboardingStatus: (tenantOverride?: Tenant) => Promise<boolean>;
  skipOnboarding: () => void;
}
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // FIXED: Accept tenant as parameter to avoid race condition with React state updates
  const checkOnboardingStatus = async (tenantOverride?: Tenant): Promise<boolean> => {
    try {
      // Use passed tenant OR fall back to storage (NOT state, which may not have updated yet)
      const tenant = tenantOverride || storage.getCurrentTenant();
      const token = storage.getAuthToken();

      // If no token or tenant, we can't check - but for new signups, we should NOT skip
      if (!token || !tenant) {
        console.log('[Onboarding] No token or tenant available for onboarding check');
        return true; // Allow through - will be caught by ProtectedRoute
      }
      ...
      // Ensure API headers are set for this request
      api.defaults.headers.common['Authorization'] = `Bearer ${token}`;
      api.defaults.headers.common['x-tenant-id'] = tenant.id;

      console.log('[Onboarding] Checking status for tenant:', tenant.id);

      const response = await api.get(API_ENDPOINTS.ONBOARDING.STATUS);

      console.log('[Onboarding] API response:', response.data);

      // Check both response.data.data.is_complete AND response.data.needs_onboarding
      if (response.data) {
        // The API returns { needs_onboarding: boolean, data: { is_complete: boolean, ... } }
        const needsOnboarding = response.data.needs_onboarding;
        const isComplete = response.data.data?.is_complete || false;

        // If needs_onboarding is true, user hasn't completed onboarding
        if (needsOnboarding === true) {
          console.log('[Onboarding] User needs onboarding (needs_onboarding=true)');
          setHasCompletedOnboarding(false);
          return false;
        }

        // Also check is_complete flag
        if (!isComplete) {
          console.log('[Onboarding] Onboarding not complete (is_complete=false)');
          setHasCompletedOnboarding(false);
          return false;
        }

        // Onboarding is complete
        console.log('[Onboarding] Onboarding is complete');
        setHasCompletedOnboarding(true);
        sessionStorage.setItem(STORAGE_KEYS.ONBOARDING_COMPLETE, 'true');
        return true;
      }

      // If response format is unexpected, default to requiring onboarding for new users
      console.log('[Onboarding] Unexpected response format, defaulting to needs onboarding');
      setHasCompletedOnboarding(false);
      return false;
    } catch (error: any) {
      console.error('[Onboarding] Error checking onboarding status:', error);

      // IMPORTANT: On error (including 404 for no record), NEW users should go to onboarding
      // Check if this is a new tenant (created recently) vs existing tenant
      const tenant = tenantOverride || storage.getCurrentTenant();

      // If tenant is owner and error occurred, assume onboarding needed (new signup)
      if (tenant?.is_owner) {
        console.log('[Onboarding] Error occurred for owner, assuming onboarding needed');
        setHasCompletedOnboarding(false);
        return false;
      }

      // For non-owners (invited users), also assume onboarding pending
      if (tenant && !tenant.is_owner) {
        console.log('[Onboarding] Error occurred for non-owner, assuming onboarding pending');
        setHasCompletedOnboarding(false);
        return false;
      }

      // Only skip onboarding check if we truly can't determine
      console.log('[Onboarding] Could not determine status, allowing through');
      return true;
    }
  };
  ...
  const login = async (email: string, password: string, remember: boolean = false) => {
    ...
    try {
      ...
      if (!data.tenants || data.tenants.length === 0) {
        ...
        // IMPORTANT: Set storage FIRST before state, so checkOnboardingStatus can access it
        storage.setCurrentTenant(tenant);
        setCurrentTenantState(tenant);
        setIsAuthenticated(true);

        // Check onboarding status - PASS TENANT DIRECTLY to avoid race condition
        console.log('[Login] Checking onboarding status for tenant:', tenant.id, 'is_owner:', tenant.is_owner);
        const isOnboardingComplete = await checkOnboardingStatus(tenant);

        console.log('[Login] Onboarding complete:', isOnboardingComplete);

        if (!isOnboardingComplete) {
          // Check if user is owner before redirecting to onboarding
          // Only owners can complete onboarding, invited users should see pending page
          if (tenant.is_owner) {
            console.log('[Login] Owner needs to complete onboarding - redirecting to /onboarding');
            navigate('/onboarding');
          } else {
            console.log('[Login] Non-owner waiting for onboarding completion - redirecting to /onboarding-pending');
  ...
  const register = async (userData: RegisterFormData) => {
    ...
    try {
      ...
      if (!data.tenant) {
        ...
        // NEW SIGNUP: User is ALWAYS the owner and ALWAYS needs onboarding
        // No need to check should_onboard flag - new signups always require onboarding
        console.log('[Register] New signup - redirecting to onboarding');
        setHasCompletedOnboarding(false);
