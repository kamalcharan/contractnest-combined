...
import {
  ...
  FileText, Send, Zap, CalendarClock,
} from 'lucide-react';
    ...
    <div className="p-6 max-w-5xl">
            ...
            <div key={key} style={surface} className="p-5">
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-sm" style={{ color: dim }}>
                  {unlimited ? 'unlimited' : `left to ${blurb}`}
                </span>
              </div>
      ...
      <div style={surface} className="p-5 mb-5">
        ...
        <p className="text-xs mb-4" style={{ color: dim }}>
          One pool per channel, shared across every contract. Credits are spent only
          when a notification is sent — they never expire.
        </p>
          ...
          {CHANNELS.map(({ key, label, dot }) => {
            ...
            return (
              <div key={key}>
                ...
                {/* The grant rate is the thing a tenant can act on: create a
                    contract, get more credits. Worth showing next to the pool. */}
                {rate ? (
      ...
      {/* Top-ups deliberately not here yet: the 19 rows in t_bm_topup_pack
          have a NULL channel, so they cannot be shown against the pools
          above, and the owner has not yet ruled on whether packs are
          authored there or as catalog-studio metering blocks. */}
    </div>
  );
};
