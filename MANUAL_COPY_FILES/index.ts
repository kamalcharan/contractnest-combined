...
async function seedSequences(
  ...
  try {
    ...
    if (seedData && Array.isArray(seedData) && seedData.length > 0) {
      ...
      // Get or create the sequence_numbers category for this tenant
      ...
      if (existingCategory) {
        ...
        console.log('[Sequences] Found existing category:', categoryId);
      } else {
        // Create the category master entry (like roles.ts pattern)
        console.log('[Sequences] Creating sequence_numbers category for tenant:', tenantId);
        const { data: newCategory, error: createError } = await supabase
          ...
          .insert({
            ...
            is_active: true,
            description: 'Document and record numbering sequences configuration',
            icon_name: 'Hash',
            order_sequence: 10,
            tenant_id: tenantId,
            is_live: isLive
        ...
        if (createError) {
          console.error('[Sequences] Error creating category:', createError);
          throw new Error(`Failed to create sequence_numbers category: ${createError.message}`);
        }

        categoryId = newCategory.id;
        console.log('[Sequences] Created category:', categoryId);
      }
      const seeded: string[] = [];
