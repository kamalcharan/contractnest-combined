...
// Updated: Added centralized field validation constants
...
const BasicInfoStep: React.FC<BasicInfoStepProps> = ({ blockType, formData, onChange }) => {
  ...
  const nameCharCount = getCharCountDisplay(nameLength, BLOCK_FIELD_LIMITS.name.max);
  ...
  const handleTagsChange = useCallback((value: string) => {
    const tags = value
      .split(',')
      .map(t => t.trim().slice(0, BLOCK_FIELD_LIMITS.tags.maxLength))
      .filter(t => t)
      .slice(0, BLOCK_FIELD_LIMITS.tags.maxCount);
    onChange('tags', tags);
  }, [onChange]);

  // Handle name change with validation
  const handleNameChange = useCallback((value: string) => {
    onChange('name', value.slice(0, BLOCK_FIELD_LIMITS.name.max));
  }, [onChange]);

  // Handle SKU change with validation
  const handleSkuChange = useCallback((value: string) => {
    onChange('meta', { ...formData.meta, sku: value.slice(0, BLOCK_FIELD_LIMITS.sku.max) });
  }, [formData.meta, onChange]);
        ...
        <div className="lg:col-span-3 space-y-5">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  maxLength={BLOCK_FIELD_LIMITS.name.max}
                  className="w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2"
                  style={{
                    ...inputStyle,
                    borderColor: nameCharCount.isError ? colors.semantic.error : inputStyle.borderColor,
                  }}
                  required
                />
                <p
          ...
          <div className="p-5 rounded-xl border" style={cardStyle}>
            <RichTextEditor
              ...
              required={true}
              maxLength={BLOCK_FIELD_LIMITS.description.max}
              showCharCount={true}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    maxLength={BLOCK_FIELD_LIMITS.sku.max}
                    className="w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2"
                    style={inputStyle}
                    required
                  />
                  <p className="text-xs mt-1 text-right" style={{ color: colors.utility.secondaryText }}>
                    {((formData.meta?.sku as string) || '').length}/{BLOCK_FIELD_LIMITS.sku.max}
                  </p>
                </div>
                <div>
          ...
          <div className="p-5 rounded-xl border" style={cardStyle}>
            ...
            />
            <p className="text-xs mt-2" style={{ color: colors.utility.secondaryText }}>
              Tags help organize and filter blocks in your library (max {BLOCK_FIELD_LIMITS.tags.maxCount} tags, {BLOCK_FIELD_LIMITS.tags.maxLength} chars each)
            </p>
          </div>
        </div>
          ...
          {(isServiceBlock || isSpareBlock) && (
            <div className="p-5 rounded-xl border" style={cardStyle}>
              <RichTextEditor
                ...
                required={false}
                maxLength={BLOCK_FIELD_LIMITS.terms.max}
                showCharCount={true}
                allowFullscreen={true}
