...
//
// Express replaces the first EIGHT of those with TWO screens and hands off
// into the existing chain at vani-consent, which carries the tenant through
...
//   → /onboarding/vani-consent  (existing, untouched, and everything after it)
//
...
//
// WHY vani-consent AND NOT resource-pick
// --------------------------------------
...
// which surfaced as "no service blocks found". Now express asks the same
// question itself and emits the identical route-state payload
// ResourcePickStep emits, so resource-pick is genuinely redundant rather
// than skipped, and nothing downstream can tell the difference.
//
...
export const EXPRESS_HANDOFF_PATH = '/onboarding/vani-consent';
...
export const EXPRESS_STEPS = [
  ...
  { id: 'serve', label: 'What you service', path: '/start/serve' },
  { id: 'workspace', label: 'Your workspace', path: EXPRESS_HANDOFF_PATH },
] as const;
