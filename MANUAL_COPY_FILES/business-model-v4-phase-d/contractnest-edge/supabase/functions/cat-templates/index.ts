...
async function handleGetPlanTemplates(
  ...
  const buildQuery = (withLatest: boolean) => {
    let q = supabase
      ...
      .eq('is_active', true)
      // Exclude packs. Nothing about a plan's own category is enforced here
      // (plans predate this constant and are not tagged consistently), but a
      // pack MUST never render as a plan card — it has no limits and its
      // one_time grant would be mislabeled as a per-creation rate.
      .neq('category', 'topup_pack')
...
//
// The credit-pack catalogue every tenant can buy from — same platform-tenant
// template shape as /plans, filtered to category = 'topup_pack' so a pack
// never gets listed as a plan or vice versa. purchase_topup_template itself
// re-derives "is this actually a pack" from the block shape (a one_time
// grant, no limits/per_creation rate) independently of this category tag —
// the tag is what makes LISTING cheap, not what makes a purchase safe.
// ============================================================================
async function handleGetPackTemplates(
  ...
  const packs = (data || [])
    ...
    // A pack template with no one_time grant is mis-authored (missing its
    // metering block) — dropped here rather than shown as a $0-credit pack.
    .filter((p: any) => Object.keys(p.grants).length > 0);
