...
const ContractWizard: React.FC<ContractWizardProps> = ({
  ...
  // Skip assetSelection for pure service delivery contracts (no equipment/facilities)
  const activeSteps = useMemo(() => {
    if (isRfqMode) return RFQ_STEPS;
    if (wizardState.nomenclatureGroup === 'service_delivery') {
      return CONTRACT_STEPS.filter(s => s.id !== 'assetSelection');
    }
    return CONTRACT_STEPS;
  }, [isRfqMode, wizardState.nomenclatureGroup]);
  ...
  const handleNomenclatureSelect = useCallback(
    (id: string | null, displayName: string | null, group?: string | null) => {
      ...
      // Clear asset data when switching to service_delivery (no equipment/facilities)
      if (group === 'service_delivery') {
