...
const GroupSessionsPage: React.FC = () => {
  ...
  const markDuePaid = useMarkDuePaid();
  ...
  const renderOccurrence = () => {
    ...
    return (
      <>
        ...
        <p className="text-sm mt-1 mb-4" style={sub}>{fmtDate(o.date)} · one session occurrence</p>

        {/* per-session statistics */}
        {(() => {
          const total = att.length;
          const present = data?.present_count ?? 0;
          const marked = att.filter((m) => m.present).length; // present-marked
          const absent = total - marked;
          const pct = total > 0 ? Math.round((present / total) * 100) : 0;
          const duesDue = att.filter((m) => m.dues_pending).length;
          const stat = (label: string, value: React.ReactNode, tone?: string) => (
            <div className="flex-1 min-w-[92px]"><div className="text-xl font-bold tabular-nums" style={{ color: tone || colors.utility.primaryText }}>{value}</div><div className="text-[10px] uppercase tracking-wide" style={sub}>{label}</div></div>
          );
          return (
            <Card style={cardStyle} className="mb-4">
              <CardContent className="p-4 flex gap-4 flex-wrap">
                {stat('Present', present, colors.semantic.success)}
                {stat('Absent', absent, absent > 0 ? colors.semantic.warning : undefined)}
                {stat('Attendance', `${pct}%`, pctColor(pct))}
                {stat('Roster', total)}
                {stat('Dues due', duesDue, duesDue > 0 ? colors.semantic.warning : undefined)}
              </CardContent>
            </Card>
          );
        })()}
          ...
          <>
            ...
            <Card style={cardStyle}>
              ...
              {(d?.billing?.length ?? 0) === 0 ? (
                ...
                <div key={i} className="flex items-center justify-between gap-3 px-4 py-2.5 border-b" style={{ borderColor: line }}>
                  ...
                  <div className="flex items-center gap-2 flex-none">
                    ...
                    {b.event_id && (b.status === 'paid' ? (
                      <button disabled={markDuePaid.isPending} onClick={() => markDuePaid.mutate({ billingEventId: b.event_id!, paid: false })} className="text-[11px] font-medium px-2 py-1 rounded border" style={{ ...sub, borderColor: colors.utility.primaryText + '22' }}>Undo</button>
                    ) : (
                      <button disabled={markDuePaid.isPending} onClick={() => markDuePaid.mutate({ billingEventId: b.event_id!, paid: true })} className="text-[11px] font-semibold px-2.5 py-1 rounded inline-flex items-center gap-1" style={{ backgroundColor: colors.semantic.success, color: '#fff' }}><Check size={12} /> Mark collected</button>
                    ))}
                  </div>
                </div>
              ))}
