...
/** Chair marks a due collected (paid) directly — cash/offline, no QR. */
export const useMarkDuePaid = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (vars: { billingEventId: string; paid: boolean }) =>
      unwrap(await api.post(API_ENDPOINTS.GROUP_SESSIONS.DUE_PAID(vars.billingEventId), { paid: vars.paid })),
    onSuccess: (_d, vars) => {
      toast.success(vars.paid ? 'Marked paid' : 'Marked unpaid');
      queryClient.invalidateQueries({ queryKey: gsDashboardKeys.all });
    },
    onError: (e) => toast.error(`Could not update the due: ${gsErr(e)}`),
  });
};
