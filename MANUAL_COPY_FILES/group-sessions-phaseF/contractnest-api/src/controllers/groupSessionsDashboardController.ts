...
class GroupSessionsDashboardController {
  ...
  /** POST /api/group-sessions/due/:billingEventId/paid  body:{ paid? } */
  markDuePaid = async (req: AuthRequest, res: Response): Promise<void> => {
    const tenantId = this.tenantId(req);
    const billingEventId = req.params.billingEventId;
    if (!tenantId || !billingEventId) { sendError(res, ERROR_CODES.VALIDATION_ERROR, 'Tenant and billing event id are required', 400); return; }
    const result = await groupSessionsDashboardService.markDuePaid(tenantId, billingEventId, req.body?.paid !== false);
    if (!result.success) { sendError(res, ERROR_CODES.INTERNAL_ERROR, result.error?.message || 'Failed to update due', 500); return; }
    sendSuccess(res, result.data);
  };
}
