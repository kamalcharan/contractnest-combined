...
const ChecklistRow: React.FC<ChecklistRowProps> = ({
  ...
  const handleRequiresCycleToggle = (yes: boolean) => {
    ...
    if (yes) {
      ...
      onUpdate({
        ...
        config: { ...instance.config, serviceCycles: { ...(instance.config as any)?.serviceCycles, days: undefined, anchorWeekday: undefined } } as any,
      });
    }
  };
  ...
  // the cadence lands before saving.
  const sampleDates = React.useMemo(() => {
    ...
    const out: Date[] = [];
    for (let i = 0; i < count; i++) out.push(hasAnchor ? addD(first, i * everyNWeeks * 7) : addD(start, i * instance.serviceCycleDays!));
    return out;
  }, [instance?.serviceCycleDays, anchorWeekday, instance?.quantity]);
