...
// ─── Route: POST /delete ────────────────────────────────────────────
// Deletes all KT data for a resource_template_id including snapshot history.
...
async function deleteKnowledgeTree(body: any, isAdmin: boolean) {
  ...
  try {
    ...
    // Also delete all snapshot history
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      switch (path) {
        ...
        default:
          return errorResponse(`Unknown path: /${path}. Valid POST: save, delete, snapshot, restore`, 404);
      }
    }
