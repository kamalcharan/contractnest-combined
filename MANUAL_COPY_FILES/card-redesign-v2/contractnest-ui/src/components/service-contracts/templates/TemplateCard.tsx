...
const TemplateCard: React.FC<TemplateCardProps> = ({
  ...
  const handleAction = (actionId: string, e: React.MouseEvent) => {
    ...
    switch (actionId) {
      ...
      case 'settings':
        ...
        break;
      case 'smart-form':
        // Navigate to smart form for this template
  ...
  // ── Dropdown menu items (overflow only) ──
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <button
          ...
          onMouseEnter={(e) => {
            if (!disabled) {
              (e.currentTarget as HTMLElement).style.backgroundColor = withOpacity(tenantPrimary, 0.1);
              (e.currentTarget as HTMLElement).style.color = tenantPrimary;
              (e.currentTarget as HTMLElement).style.borderColor = withOpacity(tenantPrimary, 0.3);
            }
          }}
          onMouseLeave={(e) => {
            (e.currentTarget as HTMLElement).style.backgroundColor = 'transparent';
            (e.currentTarget as HTMLElement).style.color = colors.utility.secondaryText;
            (e.currentTarget as HTMLElement).style.borderColor = colors.utility.secondaryText + '30';
          }}
        >
  ...
  return (
    <TooltipProvider delayDuration={200}>
      ...
      >
        ...
        {/* ── Footer Actions — 5 icon buttons with tooltips + CTA + overflow dropdown ── */}
        {showActions && (
          <div
          ...
          >
            ...
            {/* Primary CTA + 3-dot overflow */}
            <div className="flex items-center gap-2">
              <button
                onClick={(e) => handleAction(isSelected ? 'select' : (cardContext.mode === 'management' ? 'create-contract' : 'select'), e)}
                className="px-3 py-1.5 text-xs font-medium rounded-lg transition-opacity hover:opacity-90 flex items-center gap-1.5"
                style={{
                  backgroundColor: isSelected ? (colors.semantic?.success || '#10B981') : tenantPrimary,
                  color: '#FFFFFF',
                }}
                disabled={isSelected}
              >
                <CheckCircle className="w-3.5 h-3.5" />
                {isSelected ? 'Selected' : (cardContext.mode === 'management' ? 'Create Contract' : 'Use Template')}
              </button>
              ...
              {getDropdownItems().length > 0 && (
                <div className="relative" ref={dropdownRef}>
                  ...
                  {showDropdown && (
                    <div
                      className="absolute right-0 bottom-full mb-1 w-40 rounded-lg border shadow-lg z-20 py-1"
                      style={{
