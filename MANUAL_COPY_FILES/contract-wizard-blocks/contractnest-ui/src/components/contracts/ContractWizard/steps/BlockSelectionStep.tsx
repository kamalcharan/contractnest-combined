...
const BlockSelectionStep: React.FC<BlockSelectionStepProps> = ({
  ...
  // Fetch blocks
  const { hierarchy, isLoading, error, refetchHierarchy } = useBlocks({
    ...
    hierarchy.forEach((category) => {
      category.masters?.forEach((master) => {
        blocks.push({
          ...
          icon: master.icon || 'Package',
          price: (master.config as any)?.price || 0,
          currency: (master.config as any)?.currency || currency,
          categoryId: category.id,
          categoryName: category.name,
          categoryColor: category.icon ? '#3B82F6' : '#6B7280',
  ...
  const handleAddBlock = useCallback(
    ...
    [selectedBlocks, onBlocksChange, isBlockSelected, addToast]
  );
  ...
  if (error) {
    return (
      ...
      >
        ...
        >
          <LucideIcons.AlertCircle className="w-8 h-8" style={{ color: colors.semantic.error }} />
        </div>
          ...
          >
            ...
            <div className="relative">
              ...
              >
                <span>
                  {selectedCategory
                    ? hierarchy.find((c) => c.id === selectedCategory)?.name
                    : 'All Categories'}
                </span>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        >
                          <IconComponent className="w-5 h-5" style={{ color: colors.brand.primary }} />
                        </div>
