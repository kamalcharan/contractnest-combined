...
import { ArrowUpRight, Check, Mail, MessageCircle, PhoneCall, UserPlus, PauseCircle, PlayCircle, RefreshCw, X, IndianRupee, CalendarClock, History, Wrench, CalendarCheck, Play, CheckCircle2 } from 'lucide-react';
...
export const kindLabel = (c: BoardCard, meId?: string): string => {
  switch (c.kind) {
    ...
    case 'visit_in_progress': return 'Visit in progress';
    case 'visit_overdue': return 'Visit overdue';
    case 'visit_today': return 'Visit today';
    case 'visit_scheduled': return c.slot_state === 'confirmed' ? 'Visit confirmed' : c.slot_state === 'proposed' ? 'Slot proposed' : 'Visit scheduled';
...
export const evidence = (c: BoardCard, ladder?: LadderInfo, meId?: string): string => {
  ...
  if (isVisit(c)) {
    const v = c.visit;
    if (v?.block_name) bits.push(`${v.block_name}${v.sequence && v.of ? ` · visit ${v.sequence} of ${v.of}` : ''}`);
    ...
    else bits.push(c.slot_state === 'confirmed' ? 'slot confirmed with the customer' : c.slot_state === 'proposed' ? 'slot proposed, awaiting the customer' : 'no slot agreed yet');
    return bits.join(' · ');
  }
...
type ActionKey =
  ...
  // services lane
  | 'assign_visit' | 'schedule' | 'confirm_slot' | 'start_visit' | 'complete_visit';
...
export const actionsFor = (c: BoardCard): { actions: ActionKey[]; primary: ActionKey | null } => {
  ...
  switch (c.kind) {
    ...
    case 'visit_scheduled': {
      ...
      const primary: ActionKey | null = c.slot_state === 'proposed' ? 'confirm_slot' : c.kind === 'visit_scheduled' ? (c.slot_state === 'none' ? 'schedule' : null) : 'start_visit';
...
const JobCard: React.FC<JobCardProps> = ({ card: c, compact, busy, locked, team, ladder, meId, actions }) => {
  ...
  const [panel, setPanel] = useState<'assign' | 'pause' | 'followup' | 'assign_visit' | 'schedule' | 'complete' | null>(null);
  ...
  const kc: string =
    ...
    : c.kind === 'payment_ahead' || c.kind === 'rung_ahead' || c.kind === 'visit_today' ? green
  ...
  const button = (k: ActionKey) => {
    ...
    switch (k) {
      ...
      // services
      case 'assign_visit': return <Btn key={k} primary={p} onClick={() => { setPanel(panel === 'assign_visit' ? null : 'assign_visit'); setAssignTo(c.visit?.assigned_to || meId || ''); }} icon={<UserPlus size={s} />} label={c.visit?.assigned_to ? 'Reassign' : 'Assign'} title="Technician for this visit" />;
      ...
      case 'start_visit': return <Btn key={k} primary={p} onClick={() => actions.onStartVisit(c)} icon={<Play size={s} />} label="Start visit" title="Opens a service ticket and marks the visit in progress" />;
      case 'complete_visit': return <Btn key={k} primary={p} onClick={() => setPanel(panel === 'complete' ? null : 'complete')} icon={<CheckCircle2 size={s} />} label="Mark done" title="Completes the ticket and the visit" />;
      default: return null;
    }
  };
    ...
    <div className="rounded-2xl border px-4 py-3.5" style={{ backgroundColor: colors.utility.secondaryBackground, borderColor: hairline }}>
      <div className="flex items-center gap-4">
        ...
        <div className="text-right flex-none">
          {isVisit(c)
            ? <p className="text-[12px] font-bold truncate max-w-[160px]" style={ink}><Wrench size={12} style={{ display: 'inline', verticalAlign: '-2px', marginRight: 4, color: kc }} />{c.visit?.sequence && c.visit?.of ? `Visit ${c.visit.sequence}/${c.visit.of}` : 'Visit'}</p>
            : <p className="text-lg font-extrabold tabular-nums" style={ink}>{c.amount != null ? fmtMoney(c.amount, c.currency) : ''}</p>}
          <p className="text-[10px] truncate max-w-[160px]" style={{ ...sub, ...mono }}>{isVisit(c) ? (c.visit?.assigned_to_name || 'unassigned') : (c.invoice_number || c.block_name || '')}</p>
        </div>
        {pill}
      </div>
