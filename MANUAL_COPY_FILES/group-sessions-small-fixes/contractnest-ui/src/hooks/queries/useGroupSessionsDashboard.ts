...
export interface GsSessionRow {
  ...
  /** True when the block's catalog config carries an attendance policy
   * (max no-shows / max substitutes) — set in Catalog Studio. */
...
export interface GsOccAttendanceMember {
  ...
  present: boolean;
  dues_pending: boolean;
  /** From this member's own signed contract policy, scoped to their own
   * join-date window — lets the chair see who's already over cap while
   * marking attendance that day, not just after the fact. */
  missed: number;
  max_no_shows: number | null;
  max_substitutes: number | null;
  over_no_show_cap: boolean;
