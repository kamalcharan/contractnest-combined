...
import {
  ...
  Repeat, Pencil, Ban, X, Check, CalendarPlus, Plus, RotateCcw, Mic, UserCog, Lock, Search, ShieldCheck,
} from 'lucide-react';
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <div className="font-semibold truncate" style={ink}>{s.name}</div>
                      {s.policy_configured && <span title="Attendance policy configured"><ShieldCheck size={13} style={{ color: colors.semantic.success }} /></span>}
                    </div>
                    <div className="text-xs" style={sub}>{s.occurrences_total} sessions</div>
                  </div>
                </div>
          ...
          <Card style={cardStyle}>
            ...
            {occAttQuery.isLoading ? (
              ...
              <div key={m.contact_id} className="flex items-center gap-3 px-4 py-2.5 border-b" style={{ borderColor: line }}>
                ...
                {(m.over_no_show_cap || m.over_substitute_cap) && (
                  <span
                    title={m.over_no_show_cap ? `${m.missed}/${m.max_no_shows} no-shows` : `Over substitute cap`}
                    className="text-[9.5px] font-bold rounded px-1.5 py-0.5 inline-flex items-center gap-1"
                    style={{ backgroundColor: colors.semantic.error + '1e', color: colors.semantic.error }}
                  >
                    <AlertTriangle size={11} /> {m.over_no_show_cap ? `${m.missed}/${m.max_no_shows}` : 'Over cap'}
                  </span>
                )}
