...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const register = useCallback(async (data: RegisterData) => {
    try {
      ...
      // Store tenant (FamilyKnows always creates a tenant during registration)
      if (tenant) {
        ...
        // Fallback: use first tenant from tenants array
        const defaultTenant = userTenants[0];
        console.log('Using tenant from tenants array:', defaultTenant.id);
        storageItems.push([STORAGE_KEYS.TENANT_ID, defaultTenant.id]);
        storageItems.push([STORAGE_KEYS.CURRENT_TENANT, JSON.stringify(defaultTenant)]);
        setCurrentTenantState(defaultTenant);
        setTenants(userTenants);
      } else {
        console.warn('WARNING: No tenant returned from registration!');
        setTenants([]);
      }
