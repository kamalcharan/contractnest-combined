...
export async function handleRegister(supabase: any, data: RegisterData) {
  ...
  try {
    ...
    // Always create a tenant for FamilyKnows users
    // If workspace_name is provided, use it; otherwise use a default name
    const familySpaceName = workspace_name || `${first_name || email.split('@')[0]}'s Family`;
    console.log('Creating family space:', familySpaceName);

    const workspaceCode = generateWorkspaceCode(familySpaceName);

    const { data: tenant, error: tenantError } = await supabase
      .from('t_tenants')
      .insert({
        name: familySpaceName,
