...
| `jtd_escalate_payment_call` | `p_job_id, p_assign_to uuid, p_note?, p_due_at timestamptz?` (011) | Creates the open `payment_call_due` task assigned to a user (the *call* rung), advances the step. `p_due_at` is the task's `scheduled_at` (default now); **assigning yourself with a date = "Follow up"** (`business_context.task_kind='follow_up'`, history action `follow_up_set`) — owner decision 2026-09-17: a follow-up is a self-assigned JTD task, not a bookmark, and distinct from an appointment (customer + slot). The board anchors an open call on its due date. | Duplicate rung; `call_already_open` (one open call/follow-up per job — Log a call closes it); `due_in_past`. The 7-argument overload was dropped in 011. |
