...
- Autonomy dial per domain (Off · Propose · Auto) and the money cap live in `n_jtd_tenant_config` (`vani_enabled`, `vani_auto_execute_types`; add `autonomy jsonb` + `money_cap` when built). *Propose* = VaNi creates the Needs-you card and stops; *Auto* = VaNi invokes the tool and the card appears under *What happened*.
