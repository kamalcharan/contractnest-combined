...
- **Sprints 5+6 (combined) — Group Session Chair Assignment — applied live, CORRECTED after a wrong turn.** Owner picked tenant `BBB` as the base. First pass reasoned from `t_contract_events`/`t_appointments` alone (without searching for an existing Group Sessions backend) and built a brand-new parallel table + 2 RPCs to solve "assign one instructor per occurrence, fan to every member." Owner then showed a live Group Sessions dashboard (`/group-sessions`, **Test environment** — the earlier live-only DB queries had missed it entirely) proving a full, mature backend already exists: `bbb-foundation/021` onward, ~26 `gs_*` RPCs. Occurrences already live in `t_group_session_schedule` (one row per tenant+block+date, generated by `gs_generate_schedule()` from the catalog block's cadence config, decoupled from `t_contract_events` — member contracts carry the block via `t_contract_blocks`, they do **not** spawn one event per member, confirming the owner's instinct that 50 events-on-a-date would be a mess was already correctly avoided by the existing system). Attendance (`t_session_attendance`), roster, QR self/guest/substitute check-in, dues-paid marking, and a full dashboard all already existed and are live. The first-pass table/RPCs were **reverted live** (0 rows ever existed, 0 dependents, confirmed before dropping) — see `sprint5-6-group-session-occurrence` batch folder, now removed.
  - **The one real gap**, found by reading the real `gs_dash_occurrences()` definition: `t_group_session_schedule` had no column for who chairs/hosts an occurrence — exactly what the owner asked for. Fixed with `ALTER TABLE ... ADD assigned_to, assigned_to_name` (no new table) + one new RPC `gs_schedule_assign()` matching the existing `gs_schedule_status`/`gs_schedule_move` convention exactly, + an additive change to `gs_dash_occurrences()` surfacing the two new fields.
  - Verified live against BBB's real "Saturday Cadence" (27 active member contracts, not the ~10 first sampled — `LIMIT 10` had undercounted): assigning one occurrence (2026-07-31) left the other 26 occurrences' `assigned_to` untouched; test assignment then cleared back to null, no fake data left behind.
  - `t_contract_events` remains the unified cadence layer for `service`/`billing`/`spare_part`; Group Sessions are their own parallel cadence family with their own already-built occurrence/attendance/roster system, not a variant of `t_contract_events` at all — a correction to the earlier framing, which assumed Group Sessions would eventually route through `t_contract_events`. Also debated and **declined**: merging Group Session occurrences directly into `t_contract_events` (a "universal cadence table" with a type discriminator) — technically feasible (nullable `contract_id` + `CHECK`), but would mean rewriting ~26 already-live `gs_*` RPCs + the working dashboard for a conceptual-purity gain that doesn't even solve the attendance-table question (`t_session_attendance` still can't fold into `t_contract_event_assets` either way — attendance is per (occurrence, member) with no per-member event to attach to, deliberately avoided). Owner agreed: not worth the rewrite risk now; a read-layer `UNION` view can give VaNi a unified feed later without touching the live system.
  - **Follow-up, same session — Group Session Chair Appointment (031), applied live.** Owner's next requirement: assigning a chair should also create a real appointment for that person. Unlike the many-contract fan-out case (correctly ruled out — `t_appointments` cannot represent one appointment spanning 50 members' contracts), this is genuinely 1:1 (one occurrence → one chair → one appointment) — exactly `t_appointments`' intended cardinality. The only blocker was `contract_id`/`event_id` both being `NOT NULL`. Fixed: both made nullable, new nullable `group_session_occurrence_id` (FK), a `CHECK` enforcing exactly one link type is ever set, a partial unique index (one active appointment per occurrence). `gs_schedule_assign()` extended to upsert the appointment in the same call — `status='accepted'` directly (no negotiation stage; the date's fixed by cadence, the tenant is assigning, not proposing slots); `assigned_to=NULL` now also soft-deletes the appointment. New RPC `gs_schedule_assign_default()` — "set as default chair going forward," applies to every future non-cancelled occurrence of a block in one call; a later single-occurrence `gs_schedule_assign()` still overrides just that one date (owner: "default to all, but might change at a session"). Verified live against BBB's real Saturday Cadence: single-assign created a correctly-linked `accepted` appointment (`contract_id`/`event_id` both null, `group_session_occurrence_id` set, `is_live` correctly inherited); unassign soft-deleted it; `gs_schedule_assign_default` correctly bulk-assigned all 26 future occurrences (the 1 past occurrence excluded) with 26 appointment rows; all test data fully cleaned up (zero residue confirmed) before this was called done.
  - **Not yet done**: UI — "Assign chair" (single + "set as default") action on the Group Sessions dashboard, using the existing `common/ContactPicker.tsx` (`classifications: buyer/vendor/partner/team_member` — no new picker needed) plus a small inline "Appointment Card" (Accepted status, editable) showing the result; the API layer (route/controller/service/hook) to reach the new RPCs from the UI; Sprint 5(b) data repair (stale statuses, orphaned test events, the 29 stuck `requested` appointments — platform-wide, unrelated to Group Sessions); Sprint 6 1:1 appointment kanban work (needs a tenant with real appointment data — BBB has zero).
  - **Process lesson, recorded so it isn't repeated**: search the codebase (routes, `serviceURLs.ts`, controllers) for an existing feature by name *before* proposing new architecture for it, and check both Test and Live environments when a live-data query comes back suspiciously empty — don't assume "zero rows" means "unbuilt."
...
- **Sprints 5–6, combined per owner's call (19 Jul)** — architecture debated and settled, Phase 1 of the POA shipped (see Done below); Phases 0/2/3/4/5 not started.
