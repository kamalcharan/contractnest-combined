...
**Plan**: 2 Sessions lane → ~~3 Services lane (+ Who, Assign)~~ done above, **retire Event Schedule** still pending → ~~4 Appointments~~ done above (the chase *ladder* remains) → 5 swap `/ops/cockpit` to render `Commitments`, delete `/next` + the preview entry → then the contract visit card + Visit screen (replacing `ServiceExecutionDrawer`) as its own batch. A public technician page (`/visit/:token`, check-in pattern) is the natural step after that and needs a new token grant.
