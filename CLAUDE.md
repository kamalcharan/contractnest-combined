...
### HANDOVER 2026-09-17 → next session: the BUYER side (expense perspective) of Ops

**Read this first.** Everything Ops built in September (`ops-*`, `commitments-register`, `collections-invoice-aware`, `ops-menu-flatten`) was designed, probed and worded for the **seller / revenue** side. The owner's closing question — "if I log in as Buyer will this work?" — was answered from code, not from a screen: it *runs*, it is not *built for* that side. Next session's focus is making the buyer side coherent. Nothing has been coded for it yet; analysis first, "go" before code, as always.

**State of the staged work (owner has NOT tested any of it yet).** Six batches under `MANUAL_COPY_FILES/`, to be copied in this order: `ops-services-lane` → `ops-appointments-loop` → `ops-cockpit-swap` → `commitments-register` → `collections-invoice-aware` → `ops-menu-flatten`. Each has a `COPY_INSTRUCTIONS.txt` with a signia checklist. Migrations `jtd-nucleus/014`–`017` are LIVE (source-of-record files in the batches; do not re-run). Submodule `main` heads: api 968ac34, ui 7ed13cb, edge 1e43573; two page deletions (`pages/operations/services`, `pages/operations/appointments`) are pending in the owner's local ui checkout and go into Phase 2. Phase 2 commands only after "tested, working". Parent branch `claude/tenant-bbb-data-issues-buwxqm` (last commit a517da0). Owner action still open: register a MSG91 positional WhatsApp template + an email template for `visit_slot_request` and set `provider_template_id` on the two rows.

**What "Buyer" is (verified in code).** `perspective` in `AuthContext` is `'revenue' | 'expense'`. `initializePerspective()` sets **expense** when the tenant's business type is `buyer`, revenue for `seller`/`both`. Lite tiers fix it: **`cnak` = buyer side = expense**, `rfq` = seller side = revenue (`liteAccess.ts`; the tier comes from `t_tenant_onboarding.onboarding_type + is_completed`, cleared when lite onboarding completes). Switching sides is gated by readiness (`sideReadiness.ts`: revenue needs ≥1 catalog block, expense needs ≥1 own registry asset). The old cockpit's data hooks filter by `contract_type = 'client' | 'vendor'` from the perspective — that is the buyer's data model: on the expense side the tenant is the buyer, their contracts are `vendor` contracts, their money page is **To Pay** (`pages/to-pay/index.tsx`, 194 lines, on `usePayables` / `get_tenant_payables`), and menu items carry `expenseOnly` / `revenueOnly` flags (`industryMenus.ts`: Money In + Invoices revenue-only, To Pay expense-only).

**What a buyer sees today (from code, untested on a screen):**
- `/ops/cockpit` → `Home.tsx` renders the **old cockpit** (`pages/ops/cockpit/index.tsx`, 1,521 lines: awaiting acceptance, RFQ tracker, CNAK claim CTA) — owner decision at the swap; `Commitments.tsx` itself shows "You're on the expense side" and a link to To Pay if reached directly.
- `/ops/services` (Commitments Register) has **no perspective guard**. Events (`get_contract_events_list`) are tenant-scoped, so a buyer sees the services and billing events of their own vendor contracts — correct data, wrong words: the lane says **"Collections"** for money they must *pay*; invoice numbers and whole-invoice rows are absent because `useReceivables` is disabled on expense (correct — those would be payables, `get_tenant_payables`, which the register does not read); the **Follow-ups** lane is always empty (call tasks are created only from the revenue board); the **Appointments** lane shows their vendor visits' slots — arguably the most useful lane for a buyer; every "On the board" link points at the revenue board, which refuses them; the status pill works (`update_contract_event` is tenant-scoped).
- **Menu** (after `ops-menu-flatten`): Ops Cockpit · Commitments Register · Group Sessions · Contacts · Equipment Registry · Facility Registry are top-level with no perspective flags, so a buyer sees all six; the lite `cnak` flavour already listed the cockpit and the register.
- **Environment** is handled everywhere (UI sends `x-environment` on every call; board/tasks/activity controllers, events/appointments/finance edge functions and the 017 invoice rows all key on `is_live`; the send RPC inherits the invoice's flag). No work needed there.

**Questions to settle with the owner before designing (in this order):**
1. **Who is the buyer persona in practice** — a full tenant onboarded as `buyer` (facilities manager with vendor AMCs), the lite `cnak` claimant who received a contract, or both? The lite claimant has no catalog, no team, and one or two contracts; the full buyer may have dozens of vendors. The answer decides whether the buyer needs a *board* or just a *register + To Pay*.
2. **What needs a buyer *now*?** Candidates from the same data: vendor services due this week (overdue / today / scheduled — the visit is coming *to* them), slots to confirm (the customer-side of `visit_slot_*` — today that lives on the public `/slot/:token` page; a logged-in buyer could answer in-app), bills falling due (To Pay already owns this), contracts awaiting *their* acceptance, RFQ quotes to compare. Decide whether the expense side gets its own `jtd_ops_board` perspective (a `p_side` parameter: lanes Services-received · Payments-to-make · Requests) or whether the old cockpit + To Pay + a re-worded register is enough for MVP.
3. **Vocabulary on the expense side**: "Collections" → "Payments" (or "To pay"); "Services" stays (a service they receive); "Follow-ups" hidden or repurposed (a buyer's follow-up is with a vendor — different tool); "On the board" → To Pay / the contract page.

**Recommended first batch (small, UI only, safe regardless of the answers above):** in `pages/ops/register/index.tsx` on `perspective === 'expense'`: lane label Payments, read invoice numbers from `usePayables` instead of `useReceivables` (check the payables payload has an `events`/`invoices` shape like receivables — `PayablesResponse` in `useFinanceQueries.ts` has `by_vendor` + `invoices`; verify whether it carries per-event rows before relying on it), hide the Follow-ups lane, point "On the board" at `/to-pay` or the contract page; in `industryMenus.ts` decide whether Ops Cockpit / Commitments Register get `expenseOnly`/`revenueOnly` variants. Then, if the owner wants a real buyer board, extend `jtd_ops_board` with a side parameter rather than a second reader — the row model (lane · kind · anchor · bucket) already fits "services coming to me" and "bills I owe"; the tools differ (a buyer confirms a slot, declares a payment, raises a request; never nudges or assigns a technician). Spec §5 must record the decision; §11 already says a new lane needs its rows in `n_jtd` with `contract_id` for the timeline.

**Facts that will save time:** the customer-side slot answers already exist as RPCs (`visit_slot_resolve/respond`, public by token) — an in-app buyer version is the same RPCs called with the tenant's session; payment declarations from a buyer already exist (`/api/payments/declarations`, `useConfirmPaymentDeclaration` on the seller side confirms them); the old cockpit's expense flavour is the only place the RFQ tracker and CNAK claim CTA live, so retiring it for buyers needs a home for both; `LITE_CAPABILITIES` in `liteAccess.ts` gates creation surfaces for the lite buyer — read it before adding any action to a buyer page.
