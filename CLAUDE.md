...
### DONE 2026-09-18 — the TIMEBOARD, rebuilt to the PLAYGROUND shell (batch `ops-timeboard`, staged, UI only) — POA batch 3
Owner: "remove availability from settings/users (review post MVP) · start batch 3" → first cut kept only the grids; owner: "you gave me a beautiful UX and what you built for timeboard is entirely different" → rebuilt the same day to the playground (`MANUAL_COPY_FILES/playground/timeboard-playground.html`): top bar (‹ Today › · range · Day/Week/Agenda · **View as** · search · **Find a slot**), strip, then **rail** (Who: everyone + each person with avatar, load bar, hours/day off, clashes; the **VaNi autonomy dial** Draft / Ask me / Auto — a localStorage preference, no backend) · **board** (**To place** tray of every unslotted service, draggable; the grid: Day = person columns + Unassigned + VaNi, Week = day columns, Agenda) · **side** (the selected card = the board's own `JobCard` with every verb, the **confirm bar** under it after a drop with the clash preview and the playground copy — "Move / Move & confirm / Keep as is", a confirmed slot: "Move & ask / Move, keep confirmed" — Find a slot; otherwise **VaNi suggests** from the counts with buttons on Ask me/Auto and text only on Draft, and **Your day**). Drop → `jtd_assign_visit` (if another person) → `jtd_schedule_visit` → toast with **Undo** (re-schedules the previous slot). Find a slot = modal (What · Who · From → next free times, 14 days, client-side from `get_team_availability` + timed slots). No migration, no API change: readers `jtd_plan` (023) + `get_team_availability` (024). Files: `pages/ops/timeboard/index.tsx`, `components/ops/timeboard/{model.ts, Grid.tsx, Agenda.tsx}`, `App.tsx` (route `/ops/timeboard`), `industryMenus.ts` (superset of `ops-menu-flatten`). Verified: tsc 1170 = pristine on the assembled state, vite build passes; grids rendered in a throwaway harness (screenshots checked), the rebuilt shell not rendered (no auth here). Actions wiring is duplicated a third time (Ops · Plan · Timeboard) — a shared hook is the obvious refactor, not done (no unsolicited refactoring). **After the owner's first screenshot (signia: 27 unslotted, 22 carried since August)**: the tray is grouped by contract + block, collapsed by default (×N chips, oldest due, click to open, height-capped), the side panel is 400px with the compact card, and VaNi suggests leads with the backlog line + "Find a slot for the oldest". **Decisions taken without an explicit answer**: only services drag; the dial's Auto = the 023 batch tools run at once (real autonomy = batch 4); Draft hides every button; one free slot per person per day.

**FOUND 2026-09-18 — every react-hot-toast call in the app is SILENT.** Owner: "I created 2 slots but they are not visible — I don't even know if data is saved; integrate standard loader and toast." Live check: both slots were saved (n_jtd appointment items, confirmed, 19 Sep 10:00 IST = the card's Schedule default "tomorrow 10:00", a day off screen). The silence: `App.tsx` mounts `VaNiToastProviderWithGlobal` and has react-hot-toast's `<Toaster>` commented out ("Replaced with VaNiToast"); only `pages/public/VikunaHome.tsx` mounts one. **66 files import react-hot-toast** — every `toast.success/error` in them renders nothing. The standard is `vaniToast` from `components/common/toast/VaNiToast.tsx` (`success/error/warning/info/vani(title, {message, duration, action:{label,onClick}, dismissible})`, `dismiss(id)`; `duration: 0` = sticky). Fixed in batch `ops-timeboard` for `useCollectionsQueries.ts` (`useToolMutation` → every Ops verb on the board, the Plan tab and the Timeboard), `PlanView.tsx` and the Timeboard (sticky "Saving…" toast + side-panel spinner, jump to the slot's day when off screen, Schedule panel defaults to the day on screen via a new optional `JobCard.defaultSlotAt`). **Still silent**: `Commitments.tsx` (ops-expense-board), `LogCallSheet`/`HistoryDrawer` if they toast, and the ~60 other files — a one-line sweep (`import toast from 'react-hot-toast'` → `vaniToast`, JSX toasts → `action`) is the fix, batch by batch; or mount `<Toaster>` again next to the VaNi provider as a stop-gap. Ask the owner which.

### DONE 2026-09-23 — perspective toggle readiness is the PERSONA, not a row count (batch `perspective-persona-readiness`, staged, UI only)
Owner (signia, persona `both` since 15 Aug): switched to Expense, was told the side wasn't set up, completed the Expense activation, and was asked to complete it again on every toggle. Cause: `sideReadiness.ts` (batches `revenue-side-activation` / `expense-side-activation`, merged) decided "is this side set up" by **counting rows** — ≥1 catalog block for Revenue, ≥1 OWN registry asset for Expense — while `seed_onboarding_registry_assets` writes own assets **test-only** (`is_live=false`, by design), so in Live the probe saw 0 forever (signia: 5 client assets live, 4 own assets test, 0 own live). Owner: **"readiness link to persona not an asset"** — an Expense tenant RECEIVES contracts and may own nothing (the `buyer` tenant behind CN-1010). Built: `isSideReady(side, persona)` — seller → Revenue on / Expense offered; buyer → the reverse; both or unknown → both on (fail open); synchronous, no probe, no cache, `'checking'` state and the sequence ref removed. `TenantContext` now passes `profile.persona || business_type_id` to `initializePerspective` (it only ran when `business_type_id` was set — null for signia, so the persona never reached AuthContext), which records `tenantPersona` on every call. The activation walk itself: rail follows the ACTIVATED side (`OnboardingLayout` reads the hand-off), `VaniWorkingStep` carries the activated leg as route `persona` (Back on "Your assets" → Building, not Pricing), new done **Screen 9D** ("Expense side is on." · "Go to my Expense side →" / "Stay in Revenue for now") replacing 9C's seller-contract CTA for a `both` tenant; the CTA writes a one-shot **landing perspective** (`sideActivation.setLandingPerspective`, sessionStorage) that `initializePerspective` takes on the hard reload — otherwise `both` always lands on Revenue. Verified: tsc 1161 = pristine, vite build passes. **Note for the record**: the environment-blind data probe is gone; if a data-based "empty side" nudge is ever wanted, it belongs on the side's own empty states, not on the toggle.

### ON HOLD (owner, 2026-09-26) — industry in VaNi onboarding; research done, nothing built
Owner (after signia's Expense activation): "as an Expense user, when onboarding is happening we need to capture their industry, and the equipment or anything for select will be visible based on the industry" → discussion only → **"let us keep industry in onboarding on hold for now."** Findings, so nobody re-researches: **nobody is asked an industry on the express path**, sellers included (`/onboarding` → `/start` → `/start/serve`; the long form's "Which industries do you serve?" `industry-selection` / `served-industries` steps are unreachable for a new signup). `ServiceableStep` shows the WHOLE template catalog unscoped (`useGlobalTemplates` walks every page with an `industry_ids` override) and **derives "Industries you serve" backwards from the picks** (`deriveServedIndustries` → home industry) — right for sellers (the lift-AMC case: `lifts_elevators`/`hvac` have zero linked templates), **wrong for buyers**: signia picked "ATM Machine" as something it OWNS and now lists Financial Services as an industry it SERVES. *Your industry* (`t_tenant_profiles.industry_id`) is written only by the long form's branding step and the Business Profile page — empty for every express tenant. Data supports industry-first for buyers: 26 of 29 root industries have linked equipment (2–14 each), 52/52 facility templates have a home industry, 108/122 equipment templates are linked (the rest universal, cross-joined onto every root by `v_resource_templates_by_industry`); **sub-segments (Banking, Hotels…, 43 rows) carry no links** and would need rolling up to the parent. Proposal on the table when resumed: one "What's your business?" screen saved as *Your industry*; sellers continue to "What do you service?" unchanged; buyers continue to "What do you own?" scoped by `industry_ids` (no API/edge change) and write nothing to served; repair signia (drop Financial Services from served, set its own industry). Open decisions: everyone or buyers only · one industry or several · sub-segments rolled up or roots only.

### HANDOVER for the next session (2026-09-18, written for a fresh model; read this before anything else)
**State**: everything since 16 Sep is on parent branch `claude/wonderful-knuth-kb61tu` (commits through the Timeboard rebuild), staged under `MANUAL_COPY_FILES/<batch>/`, **unmerged** into the submodules; all migrations through `jtd-nucleus/024` are LIVE. The owner copies, tests and merges (Phase 2 only after "tested, working"). Copy order for the UI batches: `ops-expense-board → ops-items-on-jtd → ops-plan-view → ops-availability → ops-menu-flatten → ops-timeboard` (later batches are supersets of earlier copies of the same files).
**Verification standard**: assemble those batches into `contractnest-ui` (`cp -r MANUAL_COPY_FILES/<b>/contractnest-ui/. contractnest-ui/` in that order), `npx tsc --noEmit -p tsconfig.json | grep -c "error TS"` must be **1170** (= pristine main), `npx vite build` must pass, then `git checkout . && git clean -fd` in the submodule. API: `npx tsc --noEmit` = 0. Never commit inside a submodule; commit the parent with the two trailers (`Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>` and the `Claude-Session:` line from the system reminder).
**The POA the owner approved** (messaging workstream M1–M4 folded in): batch 1 Plan tab ✅ (+ Status filter ✅) · batch 2 availability ✅ (per-user editor removed, post-MVP) · batch 3 Timeboard ✅ (rebuilt to the playground) · **batch 4 next**: VaNi suggestions for real (a reader that proposes moves/assignments/placements with reasons, the dial's Auto acting on its own + "What VaNi did", and the **visit-slot ladder** ask → remind → escalate on Automation Rules behind `vani_is_enabled()`, exactly like `payment_reminder`) — **needs M1** (owner registers MSG91 templates: `visit_slot_request` WA + email, `service_visit_scheduled/started/completed` WA, `contract_accepted_email`; set `provider_template_id` on the global `n_jtd_templates` rows) **and M2** (owner adds an n8n HTTP node forwarding MSG91 delivery reports to `…/functions/v1/msg91-webhook`; then tighten the extractor to the real payload) · batch 5: buyer-side timeboard + "Request a visit" (expense board, `jtd_ops_board_expense`).
**Smaller open items**: an API route for `jtd_item_reschedule` so follow-ups can be dragged; per-block `config.duration` in the Block Wizard; drop `t_appointments_legacy_022`; a shared JobCard actions hook (ask first); production redeploy of API/UI (owner); the owner may report first-run issues on the Timeboard shell — fix in the same batch folder.
**Working rules that bit this session**: analysis first, wait for "go"; Phase 1 = copy commands only; unmerged batches must be supersets of earlier unmerged copies of the same file; PL/pgSQL changes via `jtd__rewrite_fn` anchor rewrites with a post-check, probed whole in one rolled-back transaction (`RAISE EXCEPTION 'PROBE OK …'`) before `apply_migration`; IST everywhere; a clash is a warning, never a refusal; the plan is for everyone, the leverage is VaNi's.
---
