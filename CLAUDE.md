...
**Plan**: 2 Sessions lane → ~~3 Services lane (+ Who, Assign)~~ done above, **retire Event Schedule** still pending (`/ops/services`, plus the lite menus' "Service Events"/"Event Schedule" entries and the Briefing's "Open Service Schedule" action) → ~~4 Appointments~~ done above (the chase *ladder* remains) → ~~5 swap~~ done above (expense side still on the old cockpit) → then `/ops/cockpit` to render `Commitments`, delete `/next` + the preview entry → then the contract visit card + Visit screen (replacing `ServiceExecutionDrawer`) as its own batch. A public technician page (`/visit/:token`, check-in pattern) is the natural step after that and needs a new token grant.
