...
**⚠️ BATCHES E AND F MERGE TOGETHER OR NOT AT ALL** — E (`evidence-storage-cleanup`) deletes `useStorageManagement.ts`, which the pristine `TicketEvidencePanel.tsx` still imports; only F (`evidence-storage-freeze`) replaces that panel with the broker version. Assembling every batch except F leaves exactly one new error, TS2307 on that import; adding F clears it (0 new, 6 removed, `vite build` passes). Noted at the top of both batches' COPY_INSTRUCTIONS. Service-evidence upload is the one path the owner has not tested ("changes are flowing into it"), so merging the pair ships it unverified — migration 008 and edge `service-execution` v6 are already live either way.
...
4. **Unmerged batches must carry byte-identical SUPERSETS of every shared file.** Four batches shipped four different `firebaseStorageAdmin.ts` and none was a superset, so whichever landed last decided which exports went missing — the owner hit `[2305, 2305]` from a copy command I gave. Now all copies are identical and six copy orders were verified. `MANUAL_COPY_FILES/check-imports.py` checks relative named imports against the exporting module **without needing node_modules** — it found that bug in seconds. **Fixing one file was not fixing the class.** A sweep across all 11 evidence batches for files shipped by more than one of them found **five more** carrying the same divergence: `App.tsx` (4 copies), `evidenceStorageService.ts` (4), `storageFormat.ts` (3), API `index.ts` (3), `evidenceController.ts` (2) and `useEvidenceQueries.ts` (2 — the older copy lacked `useUploadIdentityAsset`, so copying `evidence-storage-component` after `evidence-storage-migrate` would have handed the owner a third `[2305]` crash, in five files at once). All are now the newest version byte-for-byte; re-assembling the batches in **reverse** order gives zero new tsc errors. **The check is one command** — list every file each unmerged batch ships, keep those appearing in more than one, and `diff` them; do it before handing over any multi-batch copy order.
