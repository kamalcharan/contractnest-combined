...
**Next stitches, in order (owner: "tenant-context API is the starting point; rest stitched on")**: Automation Rules page reads the flag for a tacit status line (+ the ladder-editor review: `payment_reminder` is one pre-due email per invoice, ever; array fields render as "21" and can't be edited; template constraints say `minLength/maxLength` while `update_vani_rule` checks `min_items/max_items`; group-session rule values are decorative — `gs_run_session_notifications` never reads them, only the worker's on/off gate is honoured); VaNi landing reads the flag; `vaniEntitlementService` → `vani_is_enabled()` and `VANI_ENTITLEMENT_MODE` removed; plan writers set the column where they set `addon_vani_ai`; engines: `vani_rule_enabled() AND vani_is_enabled()`, group-session cron check, jtd-worker blocks **system-originated** jobs only (human-invoked tools keep working); retire `n_jtd_tenant_config.vani_enabled`. Ten tenants receive automation today with no subscription (BBB, signia, Hygene Services, Trinity Tecnitions, Freedom Services, Value Elevators, hubb, flow1, stw, vikuna) — before the engine stitch, decide who else gets the flag or their reminders stop. Spec: `specs/OPS-JTD-TOOLS-SPEC.md` §7.
...
**Phase 1 (this batch, staged, not yet merged)**: `pages/ops/cockpit/Commitments.tsx` — the future cockpit body — mounted at a **temporary** route `/ops/cockpit/next` with a **temporary** "Ops (preview)" submenu entry. Collections lane only: instalments from `useReceivables` (same `get_tenant_receivables` payload as Money In, so figures agree), overdue rows one-per-buyer, not-yet-due group-session fees aggregated per due date + cycle, other contracts one-per-instalment; declared payments from `gs_pending_declarations` (Confirm → existing `useConfirmDeclaration`; guest fees without an adhoc invoice can't be confirmed and say so) and `/api/payments/declarations` (Confirm → existing `useConfirmPaymentDeclaration`). Lane and Who toggles deliberately absent until later lanes. Existing pages untouched.
**Plan**: 2 Sessions lane → 3 Services lane (+ Who, Assign; retire Event Schedule) → 4 Appointments lane (retire the board) → 5 swap `/ops/cockpit` to render `Commitments`, delete `/next` + the preview entry → then the contract visit card + Visit screen (replacing `ServiceExecutionDrawer`) as its own batch. A public technician page (`/visit/:token`, check-in pattern) is the natural step after that and needs a new token grant.
